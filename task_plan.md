...
| 17. iOS/iPadOS scene、PiP 与连续性 | in_progress | OpenSpec `integrate-mobile-scene-pip-continuity`进度`19/36`；4.3已完成真实display-layer renderer client与单槽backpressure sink，下一项4.4接入production PiP controller/content source |
...
阶段17 OpenSpec `integrate-mobile-scene-pip-continuity`已进入`in_progress`，权威进度`20/36`。1.x值合同和确定性矩阵、2.1至2.6 actual UIKit scene/window/geometry/drawable/video/input ownership、3.1至3.5 actual-window mobile EDR观察与renderer接线均已完成。4.1定义generation-scoped main-actor PiP client合同，4.2以同一`CVPixelBuffer`生成保留时间与颜色元数据的ready sample buffer，4.3通过真实`AVSampleBufferDisplayLayer.sampleBufferRenderer`实现单槽latest-compatible backpressure sink，4.4通过nullable Objective-C构造边界创建production sample-buffer content source/controller，接通controller/playback delegates、actual possibility KVO、typed playback state及exactly-once restore/skip completion。4.4验收为focused `10/10`、expanded `54/54`、完整macOS `839 total / 838 passed / 1 explicit Keychain skip`、四平台generic Debug与8/8 strict/generator/repository门通过。下一项4.5实现generation-scoped prepare/start/stop/failure/restore/playback/invalidation/replacement reducer orchestration；6.6物理iPhone/iPad、系统PiP、Stage Manager、external display、visible EDR、power与live Sunshine证据保持独立pending。
...
- 阶段17 `integrate-mobile-scene-pip-continuity`权威进度`20/36 in_progress`；2.1至2.6完成actual mobile surface lifecycle/geometry/drawable/video/input绑定，3.1至3.5完成actual-window mobile EDR读取与renderer接线，4.1至4.4完成injectable PiP client、不复制的sample adapter、真实display-layer backpressure sink以及production AVKit content source/controller/delegate/KVO adapter。下一项4.5实现generation-scoped native lifecycle reducer orchestration；当前generic build和离线测试仍不证明live Stage Manager、系统PiP、background、visible mobile EDR、signed/physical设备或live Sunshine。
