...
| 15. 原生 HDR/EDR 管线 | in_progress | OpenSpec `implement-native-hdr-edr-pipeline`进度`27/33`；5.5 native HDR status/settings UI完成，下一项6.1 normal tests/Keychain skip验证 |
...
阶段15 OpenSpec `implement-native-hdr-edr-pipeline`权威进度`27/33`。5.5以无payload的observable presentation状态驱动stream overlay与Settings当前output/fallback文案，显式提供VoiceOver label/value，并以`ViewThatFits`覆盖compact宽度。最终focused `4/4`、完整macOS `616 total / 615 passed / 1 Keychain skip / 0 failed`、五平台Debug Metal build、simulator不变与repository gates通过。下一项6.1为在live-host与真实Keychain路径禁用时重新确认normal tests及唯一允许skip；live Sunshine HDR、compositor EDR signaling及物理亮度/颜色/跨显示器证据仍未完成。
...
- 阶段15 `implement-native-hdr-edr-pipeline` 权威进度`26/33`；5.4 application integration gate已完成并通过任务级验收，5.5 accessibility-safe native HDR status/settings UI已启动。恢复复核确认`HEAD == origin/main == f03151d`且工作树clean；macOS已更新至27.0，但工具链仍为Xcode 26.4/Swift 6.3/SDK 26.4。
- production graph现在以session/media/decoder generation和presentation revision连接negotiated/decoded metadata、真实lifecycle display snapshot/current headroom、user preference、resolver与actual Metal surface transition，并以presenter UUID lease隔离diagnostic replacement ownership；5.4在同一应用流程验证连续revision、stale frame与clean stop。该离线证据不证明compositor实际进入EDR、live Sunshine HDR、物理亮度/颜色或跨显示器视觉一致性；5.5 status/settings UI和6.5物理显示器验收保持未完成。
