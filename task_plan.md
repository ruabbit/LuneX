...
| 16. 空间音频运行接线 | in_progress | OpenSpec `integrate-spatial-audio-runtime`进度`22/35`；runtime到AppModel接线与recovery/replacement回归矩阵已完成，下一项完整7.1 application integration gate |
...
阶段16已在macOS 27.0/Xcode 26.4更新后恢复并进入`in_progress`。OpenSpec `integrate-spatial-audio-runtime`权威进度`22/35`：1.x至3.x的channel-layout、session-owned graph、平台策略、entitlement、route/capability adapter和observer矩阵已完成；4.1至4.4完成`SessionAudioRuntime`串行恢复、processor/factory ownership、current-generation media environment转发及AppModel actual state/desired preference绑定，4.5完成route/interruption、capability downgrade/recovery、preference、underrun/concealment、stale completion、stop/restart与same-session generation replacement回归矩阵。下一项4.6新增一条跨negotiated 7.1 PCM、environment graph、media readiness、route downgrade、entitlement fallback、reconnect replacement、diagnostics与clean stop的application integration gate。实现和测试继续显式移除`LUNEX_RUN_KEYCHAIN_TEST`并使用Debug文件fallback；在具体任务要求前不创建、启动、安装、运行或关闭任何simulator。AirPods/head tracking、可听多声道定位、真实route切换与signed entitlement行为仍是6.6真机硬件证据，不能由离线测试、属性赋值、编译或模拟器替代。
