...
| 16. 空间音频运行接线 | in_progress | OpenSpec `integrate-spatial-audio-runtime`进度`23/35`；完整7.1 application integration gate已覆盖运行接线、replacement与clean stop，下一项设置默认值、持久化、迁移与active-stream更新 |
...
阶段16已在macOS 27.0/Xcode 26.4更新后恢复并进入`in_progress`。OpenSpec `integrate-spatial-audio-runtime`权威进度`23/35`：1.x至3.x的channel-layout、session-owned graph、平台策略、entitlement、route/capability adapter和observer矩阵已完成；4.1至4.5完成`SessionAudioRuntime`串行恢复、processor/factory ownership、current-generation media environment转发、AppModel actual state/desired preference绑定及recovery/replacement矩阵；4.6以合法WAVE 7.1 PCM穿过`AppModel -> NativeSessionMediaEnvironment -> NativeSessionAudioProcessor -> SessionAudioRuntime -> AudioEngineClient`，覆盖三路readiness、missing-entitlement fixed spatial、route downgrade、reconnect replacement、迟到旧generation拒绝与两代clean stop。下一项5.1新增向后兼容的spatial-audio/head-tracking设置默认值、持久化、旧JSON迁移和active-stream更新。实现和测试继续显式移除`LUNEX_RUN_KEYCHAIN_TEST`并使用Debug文件fallback；在具体任务要求前不创建、启动、安装、运行或关闭任何simulator。AirPods/head tracking、可听多声道定位、真实route切换与signed entitlement行为仍是6.6真机硬件证据，不能由离线测试、属性赋值、编译或模拟器替代。
...
- 阶段16 `integrate-spatial-audio-runtime`权威进度`23/35`；4.3至4.6已完成current-generation environment forwarding、AppModel actual state/preference binding、恢复/replacement矩阵与合法WAVE 7.1 application integration gate。当前执行5.1设置默认值、持久化、旧JSON迁移与active-stream更新；已完成的离线门仍不证明AirPods head tracking、visionOS硬件可听行为、signed entitlement、真实route transition、物理声道输出或live Sunshine播放。
