...
| 14. macOS 原生输入与生命周期闭环 | in_progress | OpenSpec `integrate-macos-native-input-lifecycle`进度`4/29`；共享坐标合同与完整确定性矩阵已完成，下一步定义闭合生命周期directive |
...
阶段14 OpenSpec `integrate-macos-native-input-lifecycle`权威进度`4/29`。1.4补齐fit四侧黑边拒绝、fill crop边界、1x/2x backing等价、全部非正几何、resize与旧revision隔离矩阵；focused `13/13`、完整macOS `380 passed + 1 Keychain skip`通过。坐标foundation小节完成，下一项为2.1从active/visible/focused/drawable状态定义闭合render、video-processing、presentation与input lifecycle directive。每项继续独立实现、验收、提交和推送。
...
- 阶段14 `integrate-macos-native-input-lifecycle` 当前权威进度`4/29`；坐标foundation和完整确定性矩阵完成，下一项为2.1闭合生命周期directive。
