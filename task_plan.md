...
| 16. 空间音频运行接线 | in_progress | OpenSpec `integrate-spatial-audio-runtime`进度`16/35`；graph、平台策略、entitlement、route capability及有界语义通知monitor已完成，下一项平台adapter/observer测试矩阵 |
...
阶段16已在macOS 27.0/Xcode 26.4更新后恢复并进入`in_progress`。OpenSpec `integrate-spatial-audio-runtime`已创建并通过strict validation，权威进度`16/35`；1.1完成真实audio graph/SDK/证明边界盘点，1.2完成不可变mono/stereo/WAVE 5.1/7.1声道布局合同，1.3完成graph/platform/route/spatial/head-tracking/fallback/revision不可变snapshot与确定性resolver，1.4使RTSP negotiated config、decoder PCM、pipeline config及concealment共享同一个canonical layout identity，`channelCount`只作为派生值，1.5完成resolver/history边界。2.1新增不可变`SpatialAudioGraphIntent`，把偏好、平台、route、entitlement与semantic revision作为纯值传入`AudioEngineClient`，由client返回actual `SpatialAudioRuntimeSnapshot`；pipeline拒绝revision/layout/route或activation不一致的结果，failure/stop清除actual状态，`SessionAudioRuntime`在start/rebuild复用同一intent。2.2让production graph connection与PCM buffer共享显式mono/stereo/WAVE 5.1/WAVE 7.1 Core Audio layout的interleaved Int16 `AVAudioFormat`，并在copy前验证单一buffer、exact channel count、frame length和byte ownership。2.3由`AVAudioEngineClient`唯一拥有environment node，对eligible stereo/5.1/7.1连接`player -> environment -> mainMixer`，使用`.ambienceBed`且只在`.auto`属于current output applicable algorithms时选择它；disabled路径保持direct mixer，旧孤立controller已移除。2.4加入同步injectable environment builder和闭合graph fallback readback；mono、用户关闭、unsupported route、unsupported algorithm与部分graph configuration failure都会清理environment连接并继续`player -> mainMixer` PCM，resolver按确定优先级发布精确fallback且不虚报activation。2.5在macOS/iOS/tvOS仅对用户开启且embedded entitlement已授予的environment graph请求listener head tracking，并使用属性实际读回；visionOS 26不编译listener API，而是设置并读回output node的`.headTracked`或`.fixed` intended spatial experience，fallback/reconfigure/stop统一恢复`.bypassed`或关闭listener。2.6补齐同声道错序拒绝、三种eligible layout schedule前后graph readback、head-tracking reconfigure清理、visionOS resolver策略、algorithm/partial graph failure、重复stop和client/adapter ARC释放测试矩阵。3.1新增typed injectable embedded entitlement query：macOS通过公开Security `SecTask`读取并只接受literal CFBoolean true；iOS/tvOS/visionOS因公开SDK不暴露该API而默认`.unreadable`，禁止私有符号推断。3.2新增generator-owned macOS/iOS/tvOS独立entitlement plist与Debug/Release `CODE_SIGN_ENTITLEMENTS`，visionOS和test target不设置；四平台两配置unsigned generic-device build均通过，但不证明profile已授予。3.3新增可注入移动家族`AVAudioSession` adapter并由production engine owner使用：配置playback/moviePlayback、按声道布局声明multichannel、把preferred output channel裁剪到当前maximum、读取port `isSpatialAudioEnabled`、失败回滚并在stop时清除声明及停用。3.4新增macOS actual-output capability resolver：实际engine output format有效且environment-to-mixer连接与`.auto` applicable algorithm完整时为supported，typed graph fallback为unsupported，direct/unconfigured或证据不足保持unknown；类型不接收output name，观测结果的revision/rebuild接线保留4.1。3.5新增纯值route capability reader、移动家族platform notification source和`bufferingNewest` monitor；route/interruption/media-services/spatial-capability事件归约为1...64容量、单调且语义去重的revision，media-services reset即使没有先收到lost也产生一次恢复转换，stop/deinit和generation token阻止晚到callback。下一项3.6补齐entitlement/platform/multichannel/capability/equivalence/missing-route/output-name/deactivate/observer-cleanup测试矩阵。实现和测试继续显式移除`LUNEX_RUN_KEYCHAIN_TEST`并使用Debug文件fallback；在具体任务要求前不创建、启动、安装、运行或关闭任何simulator。AirPods/head tracking、可听多声道定位、真实route切换与entitlement行为必须保留为真机硬件证据，不以resolver、属性赋值、编译或模拟器结果替代。
...
- 阶段16 `integrate-spatial-audio-runtime`权威进度`10/35`；2.5平台listener/visionOS strategy已通过focused `41/41`、expanded `76/76`、四平台App build、strict和generator门，下一项2.6完整graph/resource测试矩阵。该证据不证明AirPods head tracking、visionOS硬件可听行为、signed entitlement、route notification或物理声道输出，也不得回填阶段13至15的live/hardware缺口。
