...
| 17. iOS/iPadOS scene、PiP 与连续性 | in_progress | OpenSpec `integrate-mobile-scene-pip-continuity`进度`15/36`；3.4已完成原子EDR-to-SDR fallback、screen-move reconfiguration、revision exhaustion与observer幂等teardown，下一项3.5补齐扩展回归矩阵 |
...
阶段17 OpenSpec `integrate-mobile-scene-pip-continuity`已进入`in_progress`，权威进度`15/36`。1.x值合同和确定性矩阵完成；2.1至2.6完成actual mobile surface callback、current-generation attachment/lifecycle owner、连续UIKit geometry发布、drawable/video/input原子绑定及扩展组合矩阵。3.1至3.2完成actual-window EDR读取与attached-screen通知owner；3.3完成current-generation mobile display revision到既有HDR render identity与Metal surface的接线；3.4把observer输出升级为typed snapshot/exhaustion event，revision overflow时一次性清除token与screen ownership，coordinator原子清除display/headroom并关闭render resolution，晚到同generation snapshot不能重开renderer，只有replacement surface generation可以恢复。最终focused `71/71`、expanded `83/83`、完整macOS `804 total / 803 passed / 1 explicit Keychain skip`、四平台generic Debug、strict/generator/repository与固定iOS 26.4 simulator只读门通过。下一项3.5补齐window-screen ownership、normalization、notification filtering、screen move、foreground restore、render-mode transition、stale frame与resource release矩阵；6.6物理iPhone/iPad、PiP、Stage Manager、external display、visible EDR、power与live Sunshine证据保持独立pending。
...
- 阶段17 `integrate-mobile-scene-pip-continuity`权威进度`18/36 in_progress`；2.1至2.6完成actual mobile surface lifecycle/geometry/drawable/video/input绑定，3.1至3.5完成actual-window mobile EDR读取与renderer接线，4.1完成generation-scoped main-actor injectable PiP client边界，4.2完成不复制、不另建decoder的current-generation `CVPixelBuffer`到`CMSampleBuffer` adapter。下一项4.3实现`AVSampleBufferDisplayLayer` readiness backpressure、latest-compatible-frame单槽、format/discontinuity flush、failure recovery和pixel-buffer release；当前generic build和离线测试仍不证明live Stage Manager、系统PiP、background、visible mobile EDR、signed/physical设备或live Sunshine。
