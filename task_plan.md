...
| 17. iOS/iPadOS scene、PiP 与连续性 | in_progress | OpenSpec `integrate-mobile-scene-pip-continuity`进度`12/36`；3.1已完成actual-window `UIScreen` EDR reader，下一项3.2接入attached-screen通知与重采样owner |
...
阶段17 OpenSpec `integrate-mobile-scene-pip-continuity`已进入`in_progress`，权威进度`12/36`。1.x值合同和确定性矩阵完成；2.1至2.6完成actual mobile surface callback、current-generation attachment/lifecycle owner、连续UIKit geometry发布、drawable/video/input原子绑定及扩展组合矩阵。3.1新增injectable main-actor actual-window `UIScreen` EDR reader，复用唯一finite/bounded normalizer并发布typed detached/SDR/EDR/invalid/unavailable状态；focused `5/5`、expanded `88/88`、完整macOS `798 total / 797 passed / 1 explicit Keychain skip`、四平台generic Debug、strict/generator/repository与固定simulator只读门通过。下一项3.2实现attached-screen mode/brightness通知及foreground、trait、attachment重采样、replacement/cancellation/stale rejection；3.3 renderer接线和6.6物理iPhone/iPad、PiP、Stage Manager、external display、visible EDR、power与live Sunshine证据保持独立pending。
...
- 阶段17 `integrate-mobile-scene-pip-continuity`权威进度`11/36 in_progress`；2.1至2.6完成actual mobile surface callback、current-generation attachment/lifecycle owner、连续geometry、drawable/video/input原子绑定和完整组合回归矩阵。invalid、detach、stale、source/mode mismatch与drawable失败均fail closed，iOS actual surface关闭自动drawable resize，tvOS/visionOS路径不变。下一项3.1实现actual-window mobile EDR reader；当前证据仍不证明live Stage Manager、PiP、background、mobile EDR或真机。
