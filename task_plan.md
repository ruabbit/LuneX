...
| 17. iOS/iPadOS scene、PiP 与连续性 | in_progress | OpenSpec `integrate-mobile-scene-pip-continuity`进度`24/36`；5.1 actual-state continuity policy已完成，下一项5.2统一serialized mobile media owner |
...
阶段17 OpenSpec `integrate-mobile-scene-pip-continuity`已进入`in_progress`，权威进度`24/36`。1.x值合同和确定性矩阵、2.x actual UIKit scene/window/geometry/drawable/video/input ownership、3.x actual-window mobile EDR观察与renderer接线、4.x generation-scoped sample-buffer PiP runtime及跨层回归均已完成。5.1让application continuity action复用既有path resolver，只接受generation-matched native-confirmed PiP+operational sink或active/permitted audio；capability、preference和Info.plist declaration只作为资格门。5.1验收为focused `38/38`、expanded `104/104`、完整macOS `870 total / 869 passed / 1 explicit Keychain skip`、四平台generic Debug与8/8 strict/generator/repository门通过。下一项5.2通过一个serialized mobile media generation owner统一scene、audio、PiP、foreground renderer、decoder/control、pause/stop与foreground restoration；6.6物理iPhone/iPad、系统PiP、Stage Manager、external display、visible EDR、power与live Sunshine证据保持独立pending。
...
- 阶段17 `integrate-mobile-scene-pip-continuity`权威进度`24/36 in_progress`；2.x actual mobile surface lifecycle/geometry/drawable/video/input、3.x actual-window mobile EDR、4.x generation-scoped sample-buffer PiP runtime和5.1 actual-state continuity policy均已完成。下一项5.2统一serialized mobile media generation owner；当前generic build和离线测试仍不证明live Stage Manager、系统PiP、background、visible mobile EDR、signed/physical设备或live Sunshine。
