...
| 17. iOS/iPadOS scene、PiP 与连续性 | in_progress | OpenSpec `integrate-mobile-scene-pip-continuity`进度`11/36`；2.6已完成scene/geometry/binding/renderer/input扩展矩阵，下一项3.1接入actual-window `UIScreen` EDR reader |
...
阶段17 OpenSpec `integrate-mobile-scene-pip-continuity`已进入`in_progress`，权威进度`11/36`。1.x值合同和确定性矩阵完成；2.1至2.5完成actual mobile surface callback、current-generation attachment/lifecycle owner、连续UIKit geometry发布及drawable/video/input原子绑定。2.6补齐rotation、safe-area、trait连续revision、scene/coordinate revision分离、replacement/late work和renderer/input teardown组合矩阵；focused `3/3`、expanded `91/91`、完整macOS `793 total / 792 passed / 1 explicit Keychain skip`、四平台generic Debug、strict/generator/repository与固定simulator只读门通过。下一项3.1实现injectable actual-window `UIScreen` EDR reader及typed detached/SDR/EDR/invalid状态；6.6物理iPhone/iPad、PiP、Stage Manager、external display、EDR、power与live Sunshine证据保持独立pending。
