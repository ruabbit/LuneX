...
| 14. macOS 原生输入与生命周期闭环 | in_progress | OpenSpec `integrate-macos-native-input-lifecycle`进度`24/29`；normal suite已确认唯一显式Keychain skip且live-host XCTest仍缺失，下一步执行6.2 Debug/Release五平台构建门 |
...
阶段14 OpenSpec `integrate-macos-native-input-lifecycle`权威进度`24/29`。6.1从`db11c35`干净基线和全新DerivedData执行normal suite，结构化通过`470 total / 469 passed / 1 Keychain skip / 0 failed`；测试树只有Keychain opt-in，没有可被禁用后冒充通过的live-host XCTest，阶段13 9.2继续保持缺失。下一项6.2为macOS Debug/Release及固定iPhone/iPad/tvOS/visionOS构建门。
...
- 阶段14 `integrate-macos-native-input-lifecycle` 当前权威进度`24/29`；normal suite确认`469 passed + 1 explicit Keychain skip`且live-host XCTest仍缺失，下一项6.2为Debug/Release五平台构建门。
