...
| 17. iOS/iPadOS scene、PiP 与连续性 | in_progress | OpenSpec `integrate-mobile-scene-pip-continuity`进度`2/36`；1.1 inventory和1.2 scene/window geometry值合同完成，下一项1.3为PiP/continuity状态合同 |
...
阶段17 OpenSpec `integrate-mobile-scene-pip-continuity`已进入`in_progress`，权威进度`2/36`。proposal、三个capability specs、cross-module design和36项tasks已完成规划门；1.1完成inventory/API/proof boundary，1.2新增平台无关immutable scene/window geometry合同、finite bounds、closed unavailable state、semantic deduplication与checked revision，focused/full/四目标build通过。下一项1.3定义PiP/continuity状态合同；6.6物理iPhone/iPad、PiP、Stage Manager、external display、EDR、power与live Sunshine证据保持独立pending。
