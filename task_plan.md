...
| 15. 原生 HDR/EDR 管线 | in_progress | OpenSpec `implement-native-hdr-edr-pipeline`进度`12/33`；typed uniforms与bounded pipeline cache完成，下一项3.3 injectable Metal renderer |
...
阶段15 OpenSpec `implement-native-hdr-edr-pipeline`权威进度`12/33`。3.2已冻结32-byte Swift/Metal uniform ABI，从validated frame/configuration/source-peak生成typed values，并以同步线程安全LRU cache按input layout、mapping mode和output pixel format拥有真实pipeline state。focused `36/36`、完整macOS `529 total / 528 passed / 1 Keychain skip / 0 failed`和五平台零诊断build通过。下一项3.3实现injectable generation/revision-owned Metal renderer；当前证据不代表production presenter、GPU readback或物理HDR完成。
...
- 阶段15 `implement-native-hdr-edr-pipeline` 权威进度`12/33`；3.2 typed uniforms与bounded pipeline cache完成，下一项3.3 injectable generation/revision-owned Metal renderer。
