...
| 16. 空间音频运行接线 | in_progress | OpenSpec `integrate-spatial-audio-runtime`进度`6/35`；纯值graph intent与actual runtime snapshot已贯穿engine client、pipeline和rebuild，下一项显式AVAudioFormat声道布局 |
...
阶段16已在macOS 27.0/Xcode 26.4更新后恢复并进入`in_progress`。OpenSpec `integrate-spatial-audio-runtime`已创建并通过strict validation，权威进度`6/35`；1.1完成真实audio graph/SDK/证明边界盘点，1.2完成不可变mono/stereo/WAVE 5.1/7.1声道布局合同，1.3完成graph/platform/route/spatial/head-tracking/fallback/revision不可变snapshot与确定性resolver，1.4使RTSP negotiated config、decoder PCM、pipeline config及concealment共享同一个canonical layout identity，`channelCount`只作为派生值，1.5完成resolver/history边界。2.1新增不可变`SpatialAudioGraphIntent`，把偏好、平台、route、entitlement与semantic revision作为纯值传入`AudioEngineClient`，由client返回actual `SpatialAudioRuntimeSnapshot`；pipeline拒绝revision/layout/route或activation不一致的结果，failure/stop清除actual状态，`SessionAudioRuntime`在start/rebuild复用同一intent。production当前仍只报告truthful nonspatial mixer，不提前声称environment graph。下一项2.2构造带显式channel layout的interleaved `AVAudioFormat`并验证buffer-list ownership，不提前接environment node或平台route notification。实现和测试继续显式移除`LUNEX_RUN_KEYCHAIN_TEST`并使用Debug文件fallback；在具体任务要求前不创建、启动、安装、运行或关闭任何simulator。AirPods/head tracking、可听多声道定位、route切换与entitlement行为必须保留为真机硬件证据，不以resolver、属性赋值、编译或模拟器结果替代。
...
- 阶段16 `integrate-spatial-audio-runtime`权威进度`6/35`；2.1纯值graph intent、actual snapshot传播、rebuild复用、fail-closed一致性与清理门已通过focused/expanded测试、四平台App build、strict和generator门，下一项2.2。production仍为truthful nonspatial mixer；该证据不证明environment graph、route/head tracking或硬件输出，也不得回填阶段13至15的live/hardware缺口。
