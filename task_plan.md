...
| 16. 空间音频运行接线 | in_progress | OpenSpec `integrate-spatial-audio-runtime`进度`24/35`；空间音频设置默认值、持久化、旧JSON迁移与active-stream更新已完成，下一项稳定privacy-bounded diagnostics |
...
阶段16已在macOS 27.0/Xcode 26.4更新后恢复并进入`in_progress`。OpenSpec `integrate-spatial-audio-runtime`权威进度`24/35`：1.x至3.x的channel-layout、session-owned graph、平台策略、entitlement、route/capability adapter和observer矩阵已完成；4.1至4.6完成runtime到AppModel的generation-owned接线、恢复/replacement矩阵与合法WAVE 7.1 application gate；5.1新增`AppSettings.audio`，逐键兼容旧JSON缺失值，默认值直接来自`.nativeDefault`，真实JSON repository可迁移/保存/重载，AppModel从同一持久化desired值启动并在active media generation内更新。下一项5.2以稳定、privacy-bounded状态替换free-form spatial diagnostics，覆盖active/fixed/head-tracked/visionOS/fallback/entitlement/route-layout/recovery/graph-failure。实现和测试继续显式移除`LUNEX_RUN_KEYCHAIN_TEST`并使用Debug文件fallback；在具体任务要求前不创建、启动、安装、运行或关闭任何simulator。AirPods/head tracking、可听多声道定位、真实route切换与signed entitlement行为仍是6.6真机硬件证据，不能由离线测试、属性赋值、编译或模拟器替代。
...
- 阶段16 `integrate-spatial-audio-runtime`权威进度`24/35`；4.3至4.6已完成current-generation environment forwarding、AppModel actual state/preference binding、恢复/replacement矩阵与合法WAVE 7.1 application integration gate，5.1已完成设置默认值、持久化、旧JSON迁移与active-stream更新。当前执行5.2稳定privacy-bounded spatial diagnostics；已完成的离线门仍不证明AirPods head tracking、visionOS硬件可听行为、signed entitlement、真实route transition、物理声道输出或live Sunshine播放。
