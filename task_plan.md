...
2026-08-26起由OpenSpec `prioritize-macos-product-completion`和`docs/macos-first-completion-plan.md`覆盖旧的阶段轮转顺序。M0已完成权威审计与计划迁移；M1 Task 2.1已完成production `VideoReceiveProvider`/`AudioReceiveProvider`、RTSP negotiated configuration、默认runtime接线及取消/teardown的确定性验收。Task 2.2已通过严格自验：登记`tanmy-white`广告的协议/codec能力，将`Desktop`指定为无破坏测试app，不使用Sunshine package-version allowlist，并将另外两个已知离线host的timeout记为预期状态。当前Task 2.3的Qt identity导入、wire ID continuity、production pinned mTLS与严格双重opt-in live harness均已完成确定性验收；production pinned-mTLS `/applist`已取得唯一`Desktop`，matching busy状态已按`/resume`被Sunshine接受，free状态才走`/launch`，普通断开、失败、取消、replacement与reconnect cleanup保持零`/cancel`。精确提交`c105414`的live回执已跨过OPTIONS、DESCRIBE和三次SETUP并暴露确定性缺口：LuneX遗漏ANNOUNCE和PLAY，导致Sunshine尚未注册stream session时过早连接ENet。当前实现已修正为`SETUP audio/video/control -> ANNOUNCE -> PLAY -> ENet`，并以真实vendored-ENet loopback验证首包丢弃重传、48 channels、exact connect-data、可靠channel-47收发和disconnect；focused `32/32`、related `74/74`、fresh normal `1310/1308/2/0`均通过。下一步完成产品build、generator/OpenSpec/repository gate并提交精确SHA，再执行一次bounded live gate。Task 2.3保持pending，不以协商或一次session启动代替持续媒体、实际输入、重连、远程终止、重复停止与clean teardown验收。
...
| M1 Production session与live transport | in_progress (Task 2.3; `/resume` accepted, ANNOUNCE/PLAY fixed deterministically) | production video/audio receiver、server-advertised capability inventory、identity/mTLS接线、server-info端口、macOS Local Network/Bonjour声明及显式live harness已完成确定性验收；production pinned-mTLS catalog与matching-busy `/resume`已获live接受，每请求RTSP TCP及`ANNOUNCE -> PLAY -> ENet`顺序已通过完整离线门，待精确SHA live持续视频、可听同步音频、输入、重连、终止、零`/cancel`本地停止全矩阵 |
