...
| 16. 空间音频运行接线 | in_progress | OpenSpec `integrate-spatial-audio-runtime`进度`21/35`；graph、平台策略、route monitor、runtime/processor ownership、media-generation转发与AppModel状态/偏好绑定已完成，下一项完整recovery/replacement回归矩阵 |
...
阶段16已在macOS 27.0/Xcode 26.4更新后恢复并进入`in_progress`。OpenSpec `integrate-spatial-audio-runtime`权威进度`21/35`：1.x至3.x的channel-layout、session-owned graph、平台策略、entitlement、route/capability adapter和observer矩阵已完成；4.1完成`SessionAudioRuntime`串行spatial-policy rebuild与interruption latest-wins，4.2完成processor/factory对共享route monitor、内存态preferences、单调policy/graph revision及有界semantic audio event stream的所有权，4.3完成current-generation media environment转发、snapshot和stale session/sequence/graph/generation拒绝，4.4完成AppModel actual audio state与未持久化desired preference的current-generation绑定，并在stop/failure/reconnect/replacement清理actual state。下一项4.5补齐route-during-interruption、capability downgrade/recovery、preference change、underrun/concealment、stale completion、stop/restart与generation replacement回归矩阵。实现和测试继续显式移除`LUNEX_RUN_KEYCHAIN_TEST`并使用Debug文件fallback；在具体任务要求前不创建、启动、安装、运行或关闭任何simulator。AirPods/head tracking、可听多声道定位、真实route切换与signed entitlement行为仍是6.6真机硬件证据，不能由离线测试、属性赋值、编译或模拟器替代。
...
- 阶段16 `integrate-spatial-audio-runtime`权威进度`19/35`；4.2 processor/factory route monitor、内存态preferences、graph generation与bounded semantic event stream ownership已通过focused `8/8`、expanded `88/88`、完整macOS `698 total / 697 passed / 1 Keychain skip`、四平台App build、strict和generator门。下一项4.3转发current-generation audio state并拒绝stale processor/rebuild事件；该证据仍不证明AirPods head tracking、visionOS硬件可听行为、signed entitlement、route transition可听同步或物理声道输出。
