...
| 14. macOS 原生输入与生命周期闭环 | in_progress | OpenSpec `integrate-macos-native-input-lifecycle`进度`25/29`；五平台Debug/Release warnings-as-errors零诊断构建通过，下一步执行6.3 analyzer/sanitizer/resource/dependency门 |
...
阶段14 OpenSpec `integrate-macos-native-input-lifecycle`权威进度`25/29`。6.2最终从隔离DerivedData完成macOS与固定iPhone/iPad/tvOS/visionOS的Debug/Release共十个warnings-as-errors build-only，日志零Swift/Clang/Xcode诊断；未使用的AppIntents metadata rule通过SwiftBuild公开开关跳过。simulator前后规范化快照逐字节一致，固定实例唯一、可用且全部`Shutdown`，全局`Booted=0`。下一项6.3为OpenSpec/generator/dependency/analyzer/sanitizer/malloc/resource门。
...
- 阶段14 `integrate-macos-native-input-lifecycle` 当前权威进度`25/29`；五平台Debug/Release十构建零诊断通过且simulator状态不变，下一项6.3为深度质量门。
