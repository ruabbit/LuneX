...
| 14. macOS 原生输入与生命周期闭环 | in_progress | OpenSpec `integrate-macos-native-input-lifecycle`进度`3/29`；renderer与input mapper已共享immutable snapshot，下一步补完整确定性坐标矩阵 |
...
阶段14 OpenSpec `integrate-macos-native-input-lifecycle`权威进度`3/29`。1.3让`StreamRenderState`发布的immutable snapshot同时驱动Metal destination/crop和`InputMapper`，fit letterbox拒绝、fill crop映射与drawable revision不再各自重算；focused `17/17`、完整macOS `374 passed + 1 Keychain skip`和五平台Debug构建通过。下一项为1.4补齐backing scale、invalid geometry、resize与stale revision在内的完整确定性矩阵。每项继续独立实现、验收、提交和推送。
...
- 阶段14 `integrate-macos-native-input-lifecycle` 当前权威进度`3/29`；renderer与`InputMapper`已经共享同一immutable coordinate snapshot，下一项为1.4完整确定性坐标矩阵。
