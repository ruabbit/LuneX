...
| 17. iOS/iPadOS scene、PiP 与连续性 | in_progress | OpenSpec `integrate-mobile-scene-pip-continuity`进度`3/36`；1.1 inventory、1.2 scene/window geometry和1.3 PiP/continuity值合同完成，下一项1.4为mobile EDR值合同 |
...
阶段17 OpenSpec `integrate-mobile-scene-pip-continuity`已进入`in_progress`，权威进度`3/36`。proposal、三个capability specs、cross-module design和36项tasks已完成规划门；1.1完成inventory/API/proof boundary，1.2新增平台无关immutable scene/window geometry合同，1.3新增generation-owned PiP capability/lifecycle/frame-sink/restoration状态机和actual-state continuity resolver；focused/expanded/full/四目标build通过。下一项1.4定义mobile EDR capability/headroom和display-revision合同；6.6物理iPhone/iPad、PiP、Stage Manager、external display、EDR、power与live Sunshine证据保持独立pending。
