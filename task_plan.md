...
| 16. 空间音频运行接线 | in_progress | OpenSpec `integrate-spatial-audio-runtime`进度`8/35`；eligible stereo/5.1/7.1已走session-owned ambience-bed environment graph，下一项typed nonspatial fallback |
...
阶段16已在macOS 27.0/Xcode 26.4更新后恢复并进入`in_progress`。OpenSpec `integrate-spatial-audio-runtime`已创建并通过strict validation，权威进度`8/35`；1.1完成真实audio graph/SDK/证明边界盘点，1.2完成不可变mono/stereo/WAVE 5.1/7.1声道布局合同，1.3完成graph/platform/route/spatial/head-tracking/fallback/revision不可变snapshot与确定性resolver，1.4使RTSP negotiated config、decoder PCM、pipeline config及concealment共享同一个canonical layout identity，`channelCount`只作为派生值，1.5完成resolver/history边界。2.1新增不可变`SpatialAudioGraphIntent`，把偏好、平台、route、entitlement与semantic revision作为纯值传入`AudioEngineClient`，由client返回actual `SpatialAudioRuntimeSnapshot`；pipeline拒绝revision/layout/route或activation不一致的结果，failure/stop清除actual状态，`SessionAudioRuntime`在start/rebuild复用同一intent。2.2让production graph connection与PCM buffer共享显式mono/stereo/WAVE 5.1/WAVE 7.1 Core Audio layout的interleaved Int16 `AVAudioFormat`，并在copy前验证单一buffer、exact channel count、frame length和byte ownership。2.3由`AVAudioEngineClient`唯一拥有environment node，对eligible stereo/5.1/7.1连接`player -> environment -> mainMixer`，使用`.ambienceBed`且只在`.auto`属于current output applicable algorithms时选择它；disabled路径保持direct mixer，旧孤立controller已移除。下一项2.4实现mono、用户关闭、route/algorithm/graph failure的完整typed nonspatial fallback；平台route notification和head tracking仍保留后续任务。实现和测试继续显式移除`LUNEX_RUN_KEYCHAIN_TEST`并使用Debug文件fallback；在具体任务要求前不创建、启动、安装、运行或关闭任何simulator。AirPods/head tracking、可听多声道定位、route切换与entitlement行为必须保留为真机硬件证据，不以resolver、属性赋值、编译或模拟器结果替代。
...
- 阶段16 `integrate-spatial-audio-runtime`权威进度`8/35`；2.3 session-owned ambience-bed environment graph已通过focused `21/21`、expanded `72/72`、四平台App build、strict和generator门，下一项2.4 typed fallback。该证据不证明head tracking、visionOS intended experience、route notification或硬件输出，也不得回填阶段13至15的live/hardware缺口。
