...
| 15. 原生 HDR/EDR 管线 | in_progress | OpenSpec `implement-native-hdr-edr-pipeline`进度`26/33`；5.4 application integration gate完成，下一项5.5 native HDR status/settings UI |
...
阶段15 OpenSpec `implement-native-hdr-edr-pipeline`权威进度`26/33`。5.4以单一application gate串联活动session/media/decoder、actual presentation source、presenter surface/runtime、diagnostics与stop，覆盖SDR、HDR EDR、same-display headroom降级/恢复、metadata change、cross-display revision、stale frame与clean stop。focused `1/1`、扩大矩阵`100/100`、完整macOS `612 total / 611 passed / 1 Keychain skip / 0 failed`、五平台Debug Metal build、simulator不变与repository gates通过。下一项5.5为accessibility-safe native HDR status/settings UI；live Sunshine HDR、compositor EDR signaling及物理亮度/颜色/跨显示器证据仍未完成。
...
- 阶段15 `implement-native-hdr-edr-pipeline` 权威进度`26/33`；5.4 application integration gate已完成并通过任务级验收，下一项5.5为accessibility-safe native HDR status/settings UI。
