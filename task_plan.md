...
## 2026-08-28 M1 silent-audio readiness repair

- **状态：** `in_progress`；唯一 PID `34308` 的真实 App 已稳定复现 `Connecting: Waiting for Audio`，OpenSpec Task 2.3 继续 pending。
- **实时证据：** Computer Use 正常双击 `Desktop`；只读抓包证明 video UDP 47998 与 control UDP 47999 活跃，audio socket 以 Sunshine custom 20-byte ping 每 500 ms 向 UDP 48000 发送，但静音期间没有返回 audio RTP。不是网络不可达、TCC 或 spatial entitlement。
- **修复：** 产品不再要求首个 decoded/scheduled Opus packet 才标记 audio ready；同 generation 的 session audio runtime 成功发布 `.running` 即发布 audio readiness。真实 receive/decrypt/jitter/decode/schedule 路径保持运行，任何后续 terminal failure 仍然致命且保留有限诊断。
- **验收门：** 新增无音频包 deterministic readiness 回归；依次执行 focused、related、完整 macOS、generator/OpenSpec、必要的共享 generic compatibility，提交推送后精确替换唯一 App 实例并真实复验。
- **记录错误：** 第一次同步 source/test/planning 的原子 patch 因 `task_plan.md` 锚点实际位于 `findings.md` 而在修改前验证失败，零文件被该次 patch 修改；随后按各文件真实尾部拆分应用。
- **focused R1 错误：** 测试执行前编译失败，因为两处 `XCTAssertEqual` autoclosure 直接包含 `await iterator.next()`；产品源码没有编译诊断。已把异步结果先绑定局部变量，再从新 xcresult 重跑，R1 不计通过证据。
- **focused R2：** fresh warnings-as-errors `/tmp/LuneX-SilentAudioReady-Focused-R2-20260828/Focused.xcresult` 通过 `2/2`；证明 running audio runtime 无需首个网络音频包即可发布 readiness，且 regressive/stale runtime 事件仍被拒绝。下一步完整 SessionMediaEnvironment + AppModel/audio related matrix。
- **related gate：** `/tmp/LuneX-SilentAudioReady-Focused-R2-20260828/Related.xcresult` 结构化通过 `192 total / 191 passed / 1 skipped / 0 failed`，覆盖完整 `SessionMediaEnvironmentTests`、`AppModelWorkflowTests`、`NativeSessionAudioProcessorTests` 与 `AudioRuntimeRecoveryTests`。唯一 skip 为显式 live Sunshine 测试 `testLiveTanmyWhiteProductionAcceptanceWhenExplicitlyEnabled()`；三个真实 opt-in 均 unset，未访问 Keychain、host 或 Simulator。
- **full macOS gate：** `/tmp/LuneX-SilentAudioReady-Full-20260828-R1/Full.xcresult` 权威 summary 通过 `1356 total / 1354 passed / 2 skipped / 0 failed / 0 expected failures`；build diagnostics 为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。两个 skip 精确为 live Sunshine 与 real Keychain 显式验收，三个 opt-in 均 unset；下一步 generator 与共享 target build compatibility。
- **universal R1 错误：** generic macOS product build 在编译前因 entitlement 需要 development certificate 而退出 65；项目未配置 team，generic destination 未自动选择本机 identity。该结果不计源码失败或签名验收；保持 entitlement 不变，按仓库 `build_and_run.sh` 的既有 compile-only 策略用 `CODE_SIGNING_ALLOWED=NO` 在 fresh root 重跑双架构门。
- **generator/build compatibility：** 权威 generator 双跑前/后 project hash 均为 `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`。fresh unsigned compile-only universal R2 成功，macOS executable 精确为 `x86_64 arm64`；iOS/iPadOS、tvOS、visionOS generic Debug 三项均为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。未启动或查询 Simulator，非 macOS 结果不推进 frozen product status。
- **pre-commit repository gate：** OpenSpec strict `12/12`，主 change 仍为 `7/27 ready` 且 Task 2.3 pending；baseline 与 `origin/main` 同步，精确 5 个预期文件，diff hygiene、privacy、零 remote-cancel、Debug identity fallback `0700/0600`、零 test process 和单一旧 App PID 全部通过。验收后立即提交推送，再从 pushed SHA 精确替换唯一实例执行 live 复验。
- **pushed/live product result：** silent-audio 修复已以 pushed SHA `29a27d1` 启动唯一 PID `42117`；真实 Desktop 立即越过 `Waiting for Audio` 且未断开，但停在 `Waiting for Video`。Diagnostics 证明初始 `drawable unavailable` 随后恢复 active；重新聚焦 8 秒仍无首帧。
- **direct live discriminator：** 同一 SHA 的 direct XCTest 在 test-owned nonzero drawable 下 `1/1` 通过，2.226 秒内完成真实首帧、30 帧增长、audio running、输入往返、本地 stop 和 clean teardown。因此 transport、multi-FEC、VideoToolbox、audio 与 presentation publication 正常，剩余差异是产品窗口启动时丢弃初始 IDR 后的 lifecycle resume recovery。
- **下一修复：** `VideoDecodePipeline` 在 lifecycle resume 的首个完整 predicted access unit 上允许且只允许一次补充 IDR 请求，匹配 moonlight-common-c 的成功帧后重请求语义；普通 packet-loss coalescing、停止、generation 与故障行为不放宽。先增加确定性有界重试回归，再重走门禁和立即提交推送。
- **本轮工具错误：** 初次源码检索包含不存在的 `Sources/LuneXVideo`；首个日志命令误命中 zsh `log` builtin；Computer Use 截图脚本在已完成关闭 Diagnostics/等待动作后引用未持久化的 `fs` binding；live `xcodebuild` 因 Xcode test runner 过滤自定义 opt-in 而仅 skip。四项均未修改产品/host/Keychain/Simulator；分别改用实际 source 目录、`/usr/bin/log`、重新 import、既有 direct `xcrun xctest` 路径。
- **focused/related：** lifecycle resume IDR focused `1/1` 通过；related `173 total / 172 passed / 1 live skip / 0 failed`，structured diagnostics 为 `succeeded/0/0/0`。普通 packet-loss coalescing、stop race、media generation、AppModel lifecycle/presentation/teardown 均保持通过。
- **真实产品复验：** 当前 worktree 经标准脚本精确替换为唯一 PID `45402`；Computer Use 双击 Desktop 后 5 秒内稳定显示远程桌面，控制层明确为 `Streaming / SDR / Fixed spatial`，不再停在 audio/video waiting 或断开。控件已恢复隐藏，唯一实例保持运行供用户查看。下一步 full/build/generator/OpenSpec gate 后立即 commit/push。
- **fresh full：** lifecycle-resume IDR 最终源码在 `/tmp/LuneX-VideoResumeIDR-Full-20260828-R1/Full.xcresult` 通过 `1356 total / 1354 passed / 2 exact opt-in skips / 0 failed / 0 expected failures`；structured build 为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。两个 skip 仍精确为 live Sunshine 与 real Keychain，Task 2.3 因可听同步、真实可见输入反馈、reconnect、remote termination、重复 stop 等完整矩阵尚未完成而保持 pending。
- **下一产品批次：** 用户指出真实串流浮层的 `Direct pointer / SDR / Fixed spatial` 只读标签和常驻 entitlement 说明没有操作价值。macOS P0 将把浮层改为会话工作台：显示当前主机/应用与本次串流 profile，提供会话内真实生效的 pointer、Fit/Fill、HDR 与 spatial audio 控制，只在异常时显示简短状态；不添加未接入 runtime 的假静音开关，不把诊断详情重新塞入主界面。
- **IDR final compatibility：** generator 双跑前/后 `project.pbxproj` SHA-256 均为 `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`。`/tmp/LuneX-VideoResumeIDR-Compatibility-20260829-R1` 的 unsigned compile-only macOS Debug 为精确 `x86_64 arm64` universal；iOS/iPadOS、tvOS、visionOS generic Debug 串行通过，四份 xcresult 均为 `succeeded/0/0/0`。未启动或查询 Simulator，非 macOS 结果不推进冻结平台状态。
- **IDR pre-commit gate：** OpenSpec strict `12/12`，主 change `7/27 ready` 且 Task 2.3 pending；HEAD 与 `origin/main` 同为 `29a27d1`，精确 5 个预期文件且无 untracked，三个真实 opt-in unset，Debug fallback `0700/0600`，零 `xcodebuild/xctest` 且只有一个既有产品 PID `1564`。立即提交推送，不替换当前 App。
- **IDR commit/push：** lifecycle-resume IDR batch 已提交并推送为 `cf7ebd4`，本地与 `origin/main` 同步且工作树 clean；没有替换当前单一产品实例。
- **macOS overlay implementation plan：** 仅修改 macOS `StreamStatusOverlay`，冻结平台保持现状。标题使用当前 app/host，profile 明确展示配置的 resolution/frame rate/bitrate；pointer 与 Fit/Fill 通过现有 input/render didSet 即时应用，HDR 通过现有 render resolution refresh 即时应用，spatial audio 通过 `updateSpatialAudioPreferences` 实时应用；四项均调用 settings repository 持久化。正常 streaming 不显示 general diagnostics 或 entitlement prose，连接态保留有界 actionable message。
- **overlay 工具错误：** 首轮只读 formatter 探索包含不存在的 `README*`，zsh 在命令展开阶段以 `no matches found` 退出；没有格式化、源码、runtime、App、host、Keychain 或 Simulator 副作用。后续使用 `rg --files` 或明确存在路径，不重复该 glob。
- **overlay related R1：** `228 total / 225 passed / 2 explicit opt-in skips / 1 failed`；唯一失败为旧 `HDRPresentationStatusTests` source contract 仍要求已被本次主动退役的只读 `HDR presentation` overlay label。runtime、persistence、input、HDR、audio processor 和 media-generation 测试全部通过。修订契约为可操作 `HDR and EDR` toggle + actual output accessibility value 后先隔离复核，再重跑同一 related 矩阵。
- **overlay focused/related：** 初始 overlay source contract `32/32` 通过；HDR 修订隔离 `1/1` 通过；related R2 `228 total / 226 passed / 2 exact opt-in skips / 0 failed`，structured build `succeeded/0/0/0`。两个 skip 精确来自显式 live Sunshine 与 real Keychain gates；pointer/render/HDR/spatial preference/persistence/generation ownership 均覆盖。OpenSpec strict 仍为 `12/12`。
- **overlay full：** fresh `/tmp/LuneX-MacOverlay-Full-20260829-R1/Full.xcresult` 通过 `1356 total / 1354 passed / 2 exact opt-in skips / 0 failed / 0 expected failures`，structured build `succeeded/0/0/0`。Task 5.1 仍保持 pending，等待真实窗口视觉/交互验收及该任务的其余工作流矩阵。
- **overlay compatibility/generator：** `/tmp/LuneX-MacOverlay-Compatibility-20260829-R1` 的 unsigned compile-only macOS Debug 精确含 `x86_64 arm64`；iOS/iPadOS、tvOS、visionOS generic Debug 均成功，四份 structured result 为 `succeeded/0/0/0`。generator 双跑前/后哈希均为 `e412926...7524bf`。未启动或查询 Simulator，下一步仅启动一份 macOS 产品做 Computer Use 验收。
- **overlay live acceptance：** 标准脚本只启动 PID `49417`。Computer Use 双击 `Desktop` 后真实视频持续呈现；显示控件后 UI 明确为 `Desktop on tanmy-white`、`2560x1440 @ 144 fps - 100 Mbps`、Direct/Relative、Fit/Fill、HDR/EDR、Spatial audio、hide、disconnect，正常串流无可见 entitlement/general diagnostic prose。临时 `Fit -> Fill` 立即使远程画面填满窗口且 AX `Fill=1`，恢复 `Fit` 后 AX `Fit=1` 且 settings JSON 持久化 `scaleMode=fit`。Task 5.1 仍 pending，因为该 task 还包含其他 workflow/multiwindow/acceptance 行。
- **Computer Use 小错误：** 首次复用截图脚本中的临时 `fs2` binding 返回 `is not defined`；没有 UI action 或设置副作用。重新 import 为新 binding 后只读截图成功，未重复启动 App 或 session。
- **用户位置反馈：** 第一版 action overlay 虽然内容正确，但 `.frame(maxHeight:)` 未指定内部 alignment，限制高度 frame 顶部对齐后其内容仍默认垂直居中，导致真实 panel 悬在画面约三分之一处并遮挡远程内容。修复为 wide top-center + explicit inner top alignment，将宽度从最高 `1040` 收紧为 `760`；隐藏入口同步 top-center，compact 仍 bottom。当前错误位置版本未提交。
- **position focused R1：** `32 total / 31 passed / 1 failed`；唯一失败是新增 test 过度禁止整个 `StreamWorkspaceView` 的所有 `.topLeading`，误伤与 overlay policy 无关的合法 alignment。产品编译成功。断言收窄为只禁止旧 `layout == .compact ? .bottom : .topLeading` 策略后隔离重跑，不修改定位实现。
- **position focused/live R2：** 收窄后的失败 case `1/1`、structured build `succeeded/0/0/0`。标准脚本精确替换唯一 App 后，Computer Use 再次双击 Desktop 并显示控件；真实 screenshot 证明 panel 位于 titlebar 下方 top-center、宽度 760pt，头像/用户名/密码框保持可见，全部控件无裁切/重叠。错误位置已关闭；Task 5.1 仍因其余矩阵 pending。
...
- **post-push 产品崩溃：** pushed `6206165` 的 PID `15431` 在 catalog Return 启动后于 `launchSelectedApp()` 首个 await 返回处以 `EXC_BAD_ACCESS` 崩溃；Computer Use 读取状态时按其合同自动重启为 PID `15740`，同一 catalog launch 再次在相同 stack 崩溃。两份 `.ips` 均指向 `ObservationTracking._AccessList.addAccess -> activeProductSessionOwner.getter -> productSessionLaunchIsCurrent`，而非 display-link callback、network 或 Sunshine。UI 动作的 launch task 将改为 detached MainActor task，阻断 SwiftUI Observation task-local 跨 suspension 继承，再重走 focused/full/product gate。
- **detached launch 确定性验收：** 首次 focused R1 因使用不存在的方法选择器只完成严格编译并执行 `0` tests，不计通过；正确 related R2 通过 `108 total / 107 passed / 1 live-host opt-in skip / 0 failed`，fresh full R1 通过 `1383 total / 1381 passed / 2 opt-in skips / 0 failed`，两份有效 xcresult 的 structured build diagnostics 均为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。下一步立即 commit/push，再只启动一个产品实例越过原崩溃点进行 live discriminator 验收；OpenSpec Tasks 2.3、3.1、3.2、9.1 保持 pending。
- **产品编译纠正：** pushed `5f24f57` 的标准 `LuneX-macOS` Debug build 在 `Task.detached { @MainActor in ... }` 处被 Swift 6 region-based isolation checker 以“不知道如何检查该 pattern”的编译器诊断拒绝，产品未启动。测试 target 的完整通过不能替代产品 scheme；detached MainActor closure 改为显式捕获 `[appModel, workspace]` 后必须先通过产品 build，再重跑 related/full，随后独立 commit/push。
- **产品编译 R2：** 给 `@MainActor` detached closure 增加显式 capture list 后，产品 scheme 仍在同一 actor closure pattern 上报相同编译器诊断。R2 失败证据保留；最终候选移除 closure-level `@MainActor` attribute，让 detached `@Sendable` closure 显式捕获 actor-isolated AppModel，并由 `launchSelectedApp` 的类型隔离自动 hop 回 MainActor，从而同时阻断 Observation task-local 继承并避开编译器缺陷。
- **最终确定性门禁：** plain detached closure 候选的 strict product compile R3 成功且 structured diagnostics `0/0/0`；broader related R3 通过 `139 total / 138 passed / 1 live skip / 0 failed`，fresh full R2 通过 `1383 total / 1381 passed / 2 opt-in skips / 0 failed`。下一步独立 commit/push 后使用标准脚本启动唯一产品实例，越过原 crash suspension point 并收集同一新 session 的 presentation/input discriminator。
- **detached 真实复验失败：** pushed `ad4fb4c` 从标准脚本启动唯一 PID `19270`；Computer Use 对 `Desktop` 双击后进入 stream workspace，但约数秒内再次以相同 `ObservationTracking._AccessList.addAccess -> activeProductSessionOwner.getter -> launchSelectedApp` stack 崩溃。状态读取按工具合同自动重启为替代 PID `19771`，新报告为 `LuneX-macOS-2026-08-31-080537.ips`。这推翻了单纯 TaskLocal 继承假设；下一候选在 SwiftUI 回调返回后用 `DispatchQueue.main.async` 进入下一主循环，再创建 MainActor Task，保留全部 Observation 字段语义。
- **next-turn 确定性门禁：** strict product compile R4 成功且 structured diagnostics `0 errors / 0 warnings / 0 analyzer warnings`；related R4 通过 `139 total / 138 passed / 1 live skip / 0 failed`；fresh full R3 通过 `1383 total / 1381 passed / 2 opt-in skips / 0 failed`。下一步立即 commit/push，标准脚本替换当前自动重启的旧 PID 后只保留一个产品实例，并以新的 crash-report cutoff 复验同一 suspension point。
- **next-turn 真实复验失败：** pushed `78f0596` 唯一 PID `21091` 在 Computer Use 双击后退出，且先用 `ps/pgrep` 确认退出再读取任何 UI 状态，因此没有工具自动重启。`081000.ips` 的 fault 已移动到 SwiftUI `ScrollEnvironmentStorage -> ObservationTracking._AccessList.addAccess`，不再包含 AppModel launch getter，证明外围 Task 只改变了最先触碰损坏 Observation state 的位置。回归边界仍指向 display-link：当前实现会在首帧前的 SwiftUI catalog-to-stream 过渡期立即以 source/display cadence 启动 callback。下一候选在有效 decoded presentation contract 前不创建或启用 display link，decoder clear 时立即 invalidate 并暂停。
- **display-link readiness focused R1 失败记录：** `/tmp/LuneX-MetalDisplayReadiness-Focused-R1.xcresult` 在测试 fixture 编译阶段因新建 `MacStreamInputCaptureView` 漏传现有 `sampleHandler` 而失败；产品 strict build `/tmp/LuneX-MetalDisplayReadiness-Product-R1.xcresult` 已成功。修复仅补齐测试依赖，R1 保留为失败证据，不计通过。
- **display-link readiness focused R2：** fresh warnings-as-errors `/tmp/LuneX-MetalDisplayReadiness-Focused-R2.xcresult` 结构化通过 `44/44/0/0`，build 为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。新 case 已实际验证首帧前零 display-link attachment、首帧后 144 Hz attachment 与 contract clear 后 invalidate/detach。
- **display-link readiness related R1：** fresh serial warnings-as-errors `/tmp/LuneX-MetalDisplayReadiness-Related-R1.xcresult` 结构化通过 `345 total / 344 passed / 1 explicit live-host skip / 0 failed / 0 expected failures`，build 为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。覆盖 AppModel workflow、lifecycle/render policy、AppKit surface、Metal presenter/latest-frame delivery、VideoToolbox recovery、HDR/EDR 与 media environment ownership。
- **display-link readiness full R1：** fresh serial macOS `/tmp/LuneX-MetalDisplayReadiness-Full-R1.xcresult` 结构化通过 `1384 total / 1382 passed / 2 exact opt-in skips / 0 failed / 0 expected failures`，build 为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。两个 skip 仍为 live Sunshine 与 real Keychain；确定性证据不勾选 2.3、3.1、3.2、4.1 或 9.1。
- **display-link readiness repository gate：** 产品 strict build structured diagnostics 为 `succeeded/0/0/0`；OpenSpec all strict `12/12`；权威 generator 双跑前/后 `project.pbxproj` SHA-256 均为 `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`；`git diff --check` 通过，三个真实 opt-in 均 unset，零 `xcodebuild/xctest` 与零产品 App。七个预期文件立即独立 commit/push。
- **readiness 提交与 live 判别：** 批次已提交推送为 `d0e36ce`。标准脚本从该 pushed SHA 只启动 PID `24495`；Computer Use 只双击一次 `Desktop`，8 秒后原 PID 仍存活、零新 crash report、真实视频正常呈现。首次诊断为 `3121 published / 3019 presented / 101 superseded`、latest `9.6 ms`、present max `18.4 ms`；该 snapshot 在 Diagnostics 成为 key 后正确显示 `144 source / 144 display / 15 requested`。
- **后台返回与工具边界：** Computer Use 切到 Finder 10 秒再返回，同一 PID、stream workspace 和更新中视频均保持，没有回 Library。混合前/后台累计为 `12359/11376/982`，latest `18.3 ms`、present max `21.6 ms`。Computer Use 的坐标 drag 与 coordinate click 各一次返回 `noWindowsAvailable`，不计产品输入失败，也不宣称 Relative 物理方向已验收。
- **下一独立批次：** readiness 已证明原 Observation crash 不需要 catalog 的 `DispatchQueue.main.async`/detached workaround。恢复原始结构化 `Task` 启动，移除不必要的主队列周期延迟，用 source contract、product build、related/full 与同一单实例 live 路径验证。
- **structured catalog launch 确定性门：** strict product build `/tmp/LuneX-CatalogStructuredTask-Product-R1.xcresult` 为 `succeeded/0 errors/0 warnings/0 analyzer warnings`；focused `/tmp/LuneX-CatalogStructuredTask-Focused-R1.xcresult` 通过 `32/32`；related R1 通过 `200 total / 199 passed / 1 live-host skip / 0 failed`；full R1 通过 `1384 total / 1382 passed / 2 exact opt-in skips / 0 failed`。OpenSpec all strict `12/12`、diff hygiene 通过，唯一运行产品仍为旧 pushed PID `24495`。立即 commit/push，不在推送前替换 live session。
- **structured catalog launch live 验收：** cleanup 已提交推送为 `1e6dcdc`；标准脚本替换后只有 PID `26877`。Computer Use 仅双击一次 `Desktop`，8 秒后原 PID 仍存活、零新 crash report并持续呈现真实视频。有界诊断为 control max `2.7 ms`、video receive/audio receive discard 均 0、`2592 published / 2503 presented / 88 superseded`、latest `13.4 ms`、present current/max `6.6/12.3 ms`。当前唯一 App 留在 stream workspace 供用户体验。
- **新诊断真实性缺口：** 真实视频已显示且 presentation 计数不断增长，但 on-demand Diagnostics 的 `Video decode` 仍显示 `0 access units, 0 frames, 0 failures`。该行不能用于区分 receive/decode/presentation；下一批审计 telemetry owner/publication path，修复当前 generation 的实际 decode snapshot，不从 presentation 反推虚构值。
- **输入/视频判别批次：** 用户物理确认当前 Relative 上下镜像，因此撤销 `bc3716a` 的错误 Y 反转，以真实主机结果覆盖先前仅确定性的坐标假设。活动 AppKit 捕获表面临时关闭系统 mouse-event coalescing，最后一个 owner 释放后恢复原值；已有第一层小队列继续像 Moonlight-Qt 一样仅合并相邻兼容 motion，并保留 Relative 总位移。decode snapshot 新增真实 submitted access-unit 与 current-generation decoded-frame 总数，现有 drops/failures 不再冒充总量。
- **focused R1 失败记录：** `/tmp/LuneX-InputDecode-Focused-R1.xcresult` 在产品源编译时失败，Xcode 26 Swift overlay 要求可写 class property 使用 `NSEvent.isMouseCoalescingEnabled`，而非 Objective-C header 基名 `mouseCoalescingEnabled`；零测试运行。已只修正 API overlay 名称，R1 不计通过，fresh R2 待跑。
- **focused R2：** fresh warnings-as-errors `/tmp/LuneX-InputDecode-Focused-R2.xcresult` 通过 `57/57/0/0`。实际执行覆盖 AppKit active-capture coalescing acquire/restore、所有 moved/dragged Relative Y 入口、decode submitted/current-generation decoded totals、drop/failure隔离及AppModel realtime snapshot聚合；下一门为输入、VideoToolbox、media environment、Metal presentation与diagnostics related matrix。
- **related R1 编排拒绝：** 首次 related 命令因包含预清理 `/tmp` 证据路径的 `rm -rf`，在创建任何进程前被执行层拒绝；零 `xcodebuild`、零文件删除、零测试与零 runtime 副作用。改用从未存在的 R2 DerivedData/result路径直接运行，不重复清理操作。
- **related R2：** fresh serial `/tmp/LuneX-InputDecode-Related-R2.xcresult` 通过 `406 total / 405 passed / 1 explicit live-host skip / 0 failed`，structured build 为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。覆盖完整macOS capture/coordinator/provider、VideoToolbox decode、generation-owned media environment、Metal presenter、AppModel workflow与runtime diagnostics；进入真实产品scheme严格build。
- **product R1：** unsigned strict Debug `/tmp/LuneX-InputDecode-Product-R1.xcresult` 完整构建 `LuneX-macOS` 成功，structured diagnostics 为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`；该证据覆盖SwiftUI Diagnostics产品编译但不冒充签名/head-pose或live验收。进入fresh full macOS suite。
