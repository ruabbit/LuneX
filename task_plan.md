...
| 14. macOS 原生输入与生命周期闭环 | in_progress | OpenSpec `integrate-macos-native-input-lifecycle`已apply-ready（`0/29`）；推进`NSEvent`、cursor capture、焦点释放、统一transform、decoder/renderer后台节流 |
...
阶段14 OpenSpec `integrate-macos-native-input-lifecycle`已完成proposal/design与`macos-native-input-capture`、`macos-session-lifecycle-control`、`stream-coordinate-transform`三项spec，权威进度`0/29`并通过strict validation。当前第一项为1.1只读清点AppKit event/modifier/shortcut/cursor/coordinate/multi-window所有权合同；每项继续独立实现、验收、提交和推送。
...
- 阶段14 `integrate-macos-native-input-lifecycle` 当前权威进度`0/29`且apply-ready；下一项为1.1 macOS输入与多窗口合同清单。
