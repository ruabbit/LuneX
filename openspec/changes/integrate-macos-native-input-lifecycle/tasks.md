...
- [ ] 1.1 Inventory macOS event, modifier, shortcut, cursor, coordinate, and multi-window ownership contracts without changing runtime behavior
- [ ] 1.2 Implement immutable revisioned stream coordinate snapshots and a shared fit/fill video-rectangle resolver
- [ ] 1.3 Make renderer presentation and `InputMapper` consume the same resolved coordinate contract
...
- [ ] 2.1 Define closed render, video-processing, presentation, and input lifecycle directives from active/visible/focused/drawable state
- [ ] 2.2 Add generation-scoped lifecycle application to the session media environment without exposing providers to SwiftUI
...
- [ ] 4.1 Implement an injectable main-actor cursor owner with balanced hide/show and relative association restore
- [ ] 4.2 Implement a flipped first-responder stream capture view for key, modifier, repeat, and reserved-shortcut samples
