...
- [ ] 1.1 Audit the existing `AppModel`, root scenes, workflow views, endpoint parser, diagnostics, tests, and platform targets; record the exact compatibility and proof boundaries before production edits.
...
- [ ] 2.1 Migrate host selection, first-use presentation, refresh state, and manual-add draft/result state from global fields to the owning workspace.
...
- [ ] 2.6 Recompose the SwiftUI host, pairing, and catalog surfaces for first-use, loading, empty, cached, failed, retry, confirmation, and completed states.
