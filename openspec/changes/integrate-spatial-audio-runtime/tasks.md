...
- [x] 2.5 Apply macOS/iOS/tvOS listener head tracking and visionOS 26 output-node intended spatial experience through compile-safe platform strategies
- [x] 2.6 Add graph topology, schedule/readback, layout mismatch, algorithm fallback, head-tracking readback, visionOS strategy, partial-failure, stop, idempotence, and resource-release tests
...
- [x] 3.2 Add generator-owned macOS, iOS, and tvOS head-pose entitlement files/build settings while preserving visionOS platform separation and unsigned buildability
...
- [x] 4.6 Add one application integration gate spanning negotiated 7.1 PCM, environment graph state, media readiness, route downgrade, entitlement fallback, reconnect replacement, diagnostics, and clean stop
