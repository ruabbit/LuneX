...
- 2026-08-26：继续Task 2.3，恢复确认`HEAD == origin/main == 9357a69`、工作树clean、OpenSpec `7/27 next 2.3`。当前HEAD live bundle warnings-as-errors build-for-testing通过；18:12单次gate仍返回busy/current Desktop并在旧门控下于catalog前skip，本地state不变。现收窄为busy允许只读pinned-mTLS catalog、session仍严格要求free和双opt-in，下一步focused离线验收后执行一次catalog acceptance。
- 2026-08-26：18:14 CST唯一revised catalog acceptance通过`1/1`、0.053秒：production pinned-mTLS `/applist`成为current且精确包含唯一`Desktop` ID `881448767`；四个本地文件SHA和`0600`权限前后完全相同，0 launch/resume/input/cancel/stop。identity reuse与catalog live row已通过；actual App稳定签名TCC以及session/media/input/reconnect/termination仍pending，Task 2.3不勾选。
- 2026-08-26：本轮续接按用户此前明确要求再次调用`create_goal`，控制面仍因旧`blocked`目标被视作unfinished而拒绝替换；未虚假完成旧目标且无仓库/runtime副作用，继续以macOS-first OpenSpec和planning files执行。
- 2026-08-26：catalog admission变化后的fresh macOS normal在三项真实opt-in显式unset下结构化通过`1287 total / 1285 passed / 2 skipped / 0 failed`；两个skip精确为一次性Keychain和live-host门控，compiler/xcresult diagnostics全零，仅1条无AppIntents依赖的metadata advisory。未访问Keychain、host或Simulator，下一步执行strict/generator/diff/process最终门并提交推送独立catalog acceptance批次。
- 2026-08-26：首轮final repository wrapper已通过OpenSpec `11/11`、apply `7/27 next 2.3`、generator双跑稳定、六文件scope、diff/privacy/permission/opt-in门，但末尾进程awk因正则分隔符冲突报`nonterminated character class`且命令替换掩盖exit；该轮不计完整final gate。将只补跑修正后的进程、当前diff与已保存证据一致性检查，不重复normal/generator/live或Simulator操作。
- 2026-08-26：corrected final gate通过：活动`xcodebuild/xctest=0`、三个真实opt-in unset、normal结构化`1287/1285/2/0`且skip精确为Keychain/live、OpenSpec strict `11/11`、apply `7/27 next 2.3`、generator三哈希同为`f9a54a2e...6000de`、六文件scope、diff/privacy/permission边界通过。Task 2.3保持pending，catalog acceptance批次可独立提交推送。
- 2026-08-26：catalog admission批次以`f9cefd74f8584de123541724b952e4b0d4650208`提交并推送，三方SHA一致且工作树clean。随后在该精确提交上只执行一次正确`:47989/serverinfo` preflight，返回`tanmy-white / SUNSHINE_SERVER_BUSY / currentgame=881448767`；按fail-closed门控未运行Desktop session，0 launch/input/cancel/stop。Task 2.3仍pending，等待既存session自然结束后下一次明确续接再做单次preflight。
- 2026-08-26：用户纠正busy不等于Sunshine单实例/单线程。复核Moonlight-qt、Moonlight iOS与Sunshine源码后确认原free-host session gate错误：free应`/launch`，matching busy `Desktop`应`/resume`，different busy app应无干扰拒绝；普通Disconnect、consumer cancellation、replacement与failure cleanup均应零`/cancel`。开始修订production provider、cancellation/recovery/live harness测试、OpenSpec、live matrix与planning authority；Task 2.3在确定性回归和matching-busy live验收前保持pending。
- 2026-08-26：busy/resume修正首轮fresh warnings-as-errors focused执行`41 total / 40 passed / 0 skipped / 1 failed`；唯一失败是`SessionRecoveryTests`仍保留reconnect exhaustion后应有一次remote cancel的旧断言，而production正确得到0。已改为断言零stop及teardown `.notRequested`，失败bundle不计验收，下一轮使用全新DerivedData/result bundle。
- 2026-08-26：fresh focused第二轮`41/41 passed / 0 skipped / 0 failed`，覆盖initial free launch、matching busy resume、different busy/missing state拒绝、provider实际`resume=1/launch=0/stop=0`、RTSP、全部cancellation/recovery及live双opt-in配置；源码warning/error为0，只有既有无AppIntents依赖metadata advisory。三个真实opt-in均unset，未访问Keychain/live host/Simulator。AppModel审计确认产品路径只调用coordinator `prepare`，实际HTTP start由session provider唯一拥有，不存在上层重复`/launch`；local stop只走provider本地teardown。
- 2026-08-26：完整macOS normal通过`1292 total / 1290 passed / 2 skipped / 0 failed`，两项skip精确为一次性Keychain与live-host门控；macOS Debug/Release及iOS/iPadOS、tvOS、visionOS generic Debug unsigned warnings-as-errors build全部成功，非macOS仅证明共享源码兼容，未操作Simulator。首轮repository wrapper在shell执行前因隐私正则引号嵌套产生zsh parse error，0 OpenSpec/generator/build副作用；将拆分模式后完整重跑。
- 2026-08-26：corrected repository gate通过OpenSpec strict `11/11`、apply `7/27 next Task 2.3`、generator三哈希稳定`f9a54a2e...6000de`、diff check、三个opt-in unset、活动xcodebuild/xctest为0及隐私扫描。focused、normal、macOS Debug/Release、iOS/iPadOS、tvOS、visionOS七份xcresult均为`succeeded / 0 error / 0 warning / 0 analyzer warning`；Task 2.3仍未勾选，准备提交确定性修正后在精确SHA上运行matching-busy live gate。
- 2026-08-26：提交前单change OpenSpec命令实际返回`items[0].valid=true`，但首个wrapper误读旧顶层`.valid`并在状态打印前退出；直接按当前JSON结构复核为`1/1 valid`且`git diff --check`通过，无源码、生成器或runtime副作用。
- 2026-08-26：busy/resume确定性修正已提交推送为`c8888952551a468830f67a1781c81ad85037d6c3`且三方SHA一致、工作树clean。精确SHA fresh live bundle build为`succeeded/0/0/0`；唯一双opt-in live gate在3.372秒进入`.failed`、未到streaming，wrapper仍完成本地清理且四个state文件SHA/`0600`前后完全相同。现有失败消息只有phase，不能区分resume HTTP、RTSP或媒体失败；不盲目重复，先以透明production client recorder补固定issue/diagnostic及launch/resume/cancel计数回执。
- 2026-08-26：live receipt recorder首个`apply_patch`因同一文件在一个patch中出现两个独立update operation而被工具原子拒绝，零修改；已合并为单一operation后应用，不改变production代码。
- 2026-08-26：live receipt recorder fresh warnings-as-errors focused通过`1/1`且build结构化`succeeded/0 error/0 warning/0 analyzer warning`，三个真实opt-in均unset；透明actor仅包装production HTTP client并将在失败/成功回执中记录固定launch/resume/cancel计数及privacy-bounded issue/diagnostic code。准备独立提交推送后从精确SHA只执行第二次live诊断尝试。
- 2026-08-26：recorder提交推送为`5568e4fe6802ddd9f757c04bd10b6de0e9a9a6e4`，精确SHA live bundle build为`succeeded/0/0/0`。第二次唯一live gate在1.971秒失败，明确回执`launch=0/resume=1/cancel=0`、`issue=stream_interrupted`、`diagnostic=transport_failed`、`subsystem=stream.transport`，四state文件仍完全不变。这证明matching busy实际走`/resume`且本地failure cleanup零`/cancel`；尚不能区分RTSP bootstrap、SDP、network或control枚举，现补透明SessionControlProvider stage/error recorder后再决定第三次尝试。
- 2026-08-26：SessionControl stage/error recorder首轮focused在0 tests前被warnings-as-errors拒绝，两处`await self.record`因forwarding Task继承actor isolation而无异步操作；production无诊断、未访问live host/Keychain/Simulator。删除多余await后将从全新证据目录重跑。
- 2026-08-26：SessionControl recorder fresh focused第二轮通过`1/1`，build结构化`succeeded/0 error/0 warning/0 analyzer warning`。wrapper透明转发真实provider全部协议操作，receipt只记录固定event stage与最终Error类型/描述；三个真实opt-in unset。准备提交推送并以精确SHA执行第三次、具备分层错误证据的live尝试。
- 2026-08-26：SessionControl recorder已提交推送为`74056ca92d1067742969496ab83517e3c7db6494`。精确SHA第三次bounded live gate回执为`launch=0/resume=1/cancel=0`、`controlEvents=launch_accepted`、`controlFailure=NetworkChannelError.posixFailure(96)`；四个本地state文件仍为`0600`且SHA逐字不变。该结果证明Sunshine接受matching-busy附加客户端的`/resume`且普通失败清理零`/cancel`，失败位于首个RTSP事务、早于`rtspReady`。
- 2026-08-26：复核Sunshine与`moonlight-common-c`确认每个RTSP OPTIONS/DESCRIBE/SETUP都创建独立TCP连接并读到peer close；LuneX production `NetworkRTSPConnection`错误地在`connect()`建立单个channel并跨全部事务复用。另确认`NWConnectionDriver.receive()`在nonempty bytes与terminal `NWError`同时返回时丢弃bytes，可直接解释live `ENODATA(96)`。Task 2.3继续pending，当前进入per-transaction channel、terminal-byte precedence、encrypted sequence continuity与active-channel cancellation修复，不重复live尝试直至确定性和完整build门通过。
- 2026-08-26：已完成首轮production/test补丁；`NetworkRTSPConnection`改为每事务fresh channel并保留session级加密sequence，`NWConnectionDriver`优先返回同callback内nonempty terminal bytes，`NetworkByteChannel`返回最后数据后标记closed。首轮warnings-as-errors focused在0 tests前仅因取消测试的`Task`闭包间接捕获`XCTestCase self`而编译失败；production无诊断、真实opt-in unset。已在Task外预构造Sendable request，下一轮使用fresh DerivedData，不复用失败bundle。
- 2026-08-26：修正后新增focused结构化通过`5/5`，扩大NetworkChannel/RTSPBootstrap/SessionCancellation/SessionRecovery/live-config矩阵结构化通过`59/59`；两份build均`succeeded / 0 error / 0 warning / 0 analyzer warning`。人工并发审阅补强transaction finish token：response解析后关闭channel期间若session cancel胜出，事务现在返回cancelled而不发布已取消session的成功响应。该补强需从fresh focused重新验证后才进入normal/build门。
- 2026-08-26：finish-token补强后的fresh最终related结构化再次通过`59/59`，0 skip/failure/expected failure，build为`succeeded / 0 error / 0 warning / 0 analyzer warning`；三个真实opt-in unset且未访问Keychain/host/Simulator。下一步运行独立fresh serial normal，再执行macOS Debug/Release和共享变化所需的冻结平台generic compatibility builds。
- 2026-08-26：fresh serial macOS normal结构化通过`1297 total / 1295 passed / 2 skipped / 0 failed / 0 expected failure`，build为`succeeded / 0 error / 0 warning / 0 analyzer warning`；两个skip精确为live Sunshine opt-in和一次性真实Keychain，普通测试继续使用Debug文件fallback。下一步顺序执行macOS Debug/Release和iOS/iPadOS、tvOS、visionOS generic compatibility builds，不操作Simulator lifecycle。
- 2026-08-26：macOS Debug/Release及iOS/iPadOS、tvOS、visionOS generic Debug五份warnings-as-errors产品build全部结构化为`succeeded / 0 error / 0 warning / 0 analyzer warning`；两个macOS executable均为`x86_64 arm64` universal。非macOS结果只证明共享networking修改的编译兼容，未创建、启动、查询或操作Simulator，不推进冻结平台产品状态。下一步执行OpenSpec/generator/diff/privacy/opt-in/process repository gate。
- 2026-08-26：repository gate已确认generator三次稳定SHA-256 `f9a54a2e4b9881e4730b88075074670c8486091683490c48c8ff7924130600de`且工程零diff、OpenSpec strict `11/11`、当前change `7/27`且2.3 pending、精确9文件scope、diff/privacy/zero-remote-cancel/zero-persistent-channel形状、三个opt-in unset、零活动xcodebuild/xctest以及三方Git基线一致。OpenSpec首个jq汇总因子表达式缺括号发生precedence错误；验证已成功且只重新解析保存JSON，不重复命令。下一步完整人工diff审阅与最终只读gate。
- 2026-08-26：人工diff审阅确认transaction-local channel/buffer、session-local endpoint/key/sequence、terminal-byte-first、incomplete-close fail-closed、finish token cancellation及普通teardown零`/cancel`均成立。最终只读gate输出`FINAL_RTSP_REPOSITORY_GATE_OK`，复核精确9文件、`59/59`、`1297/1295/2/0`、五build全零诊断、两macOS universal、stable generator、OpenSpec `11/11`与`7/27 pending`、opt-in/process/coverage/fallback权限边界；准备独立提交推送，不勾选2.3。
- 2026-08-26：per-request RTSP修复已以`92e9d9e7fb19ecda0361f6d0fd5d2253a0ae7b16`提交推送且三方SHA一致。精确SHA双重opt-in live gate记录`launch=0/resume=1/cancel=0`、`controlEvents=launch_accepted`、`controlFailure=SunshineRTSPNegotiationError.descriptionTooLarge`；原首笔`ENODATA(96)`已消失，OPTIONS/DESCRIBE已到达。四个本地state文件保持`0600`且SHA/尺寸前后不变，活动xctest收敛为0。
- 2026-08-26：上轮live wrapper的`xctest ... | tee`没有`pipefail`，shell错误报告exit `0`；XCTest本身明确失败，该尝试按failure receipt记录并不重跑。后续live wrapper必须传播xctest真实exit status。
- 2026-08-26：读取Sunshine、`moonlight-common-c`和LuneX parser合同后定位新根因：Sunshine DESCRIBE以peer close界定无`Content-Length`的SDP；LuneX在header完成时提前发布empty-body response，之后`SunshineSessionDescriptionParser`将empty body误合并到`descriptionTooLarge`。开始实现plaintext close-delimited transaction parsing与确定性回归；Task 2.3继续pending。
- 2026-08-26：已实现独立`decodeCloseDelimitedExact`、plaintext terminal-aware transaction parsing、empty ENODATA到`.closed`映射、channel closed-state保留及SDP `.emptyDescription`类型。fresh focused warnings-as-errors结构化通过`56/56`，0 skip/failure/expected failure且build零error/warning/analyzer warning；取消竞态用例证明terminal response解析后仍由session cancel胜出。三opt-in unset，未访问Keychain/live host/Simulator。
- 2026-08-26：人工审阅将空`ENODATA -> .closed`从全transport收窄到TCP，避免改变UDP语义；收窄前focused/related成功证据不作最终候选。收窄后fresh focused通过`56/56`，related通过`78/78`，两份build均零structured diagnostics，三opt-in unset且零active build/test进程。
- 2026-08-26：fresh serial macOS normal结构化通过`1306/1304/2/0`，两skip精确为真实Keychain和live Sunshine opt-in，build零structured diagnostics。随后串行macOS Debug/Release及三冻结平台generic Debug产品build `5/5`通过且零error/warning/analyzer warning；macOS两配置均为`x86_64 arm64`。未访问Keychain/host或操作Simulator。
- 2026-08-26：repository wrapper首轮已验证OpenSpec strict `11/11`和generator三次SHA稳定，但next-task jq误用不存在的`.status`字段，将已完成1.1打印为next。权威apply JSON使用`.done` boolean；最终门改用`select(.done == false)`从头复核。该错误只影响状态打印，零仓库/runtime副作用。
- 2026-08-26：修正next-task后的首轮最终repository wrapper又因`rg -c`零匹配输出空字符串而在privacy断言提前退出；本轮不计完整最终门。独立诊断确认实际匹配0、三opt-in unset、fallback `0700/0600`、零build/test process；改用`awk`count从头复核。
- 2026-08-26：第二轮最终wrapper超过10秒yield后，上层丢失运行中session id且最终marker不可读；当前零git/OpenSpec/build/test进程，但该轮不计最终门。改为30秒yield和checkpoint输出从头重跑。
- 2026-08-26：30秒yield完整repository gate通过；补入OpenSpec close-delimited定界规则后的post-spec门再次通过`FINAL_POST_SPEC_CLOSE_DELIMITED_GATE_OK`。最终证据为strict `11/11`、change `7/27 next 2.3 pending`、stable generator `f9a54a2e...6000de`、13文件scope、privacy 0、focused `56/56`、related `78/78`、normal `1306/1304/2/0`、五build零structured diagnostics、三opt-in unset、零process与Git三方基线一致。Task 2.3未勾选，准备独立提交推送。
- 2026-08-26：close-delimited批次已以`c105414288ce3b2978f839ecf697c7d9b81a52da`提交推送，精确SHA fresh warnings-as-errors live bundle build零structured diagnostics。唯一双重opt-in gate使用`pipefail`正确返回xctest `1`，回执为`launch=0/resume=1/cancel=0`、`controlEvents=launch_accepted,rtsp_ready`、`controlFailure=ENetTransportError.connectionFailed`。四state文件mode/size/SHA前后不变，零active xctest。不重跑，转入ENet control connect根因审计。
- 2026-08-26：ENet根因审计确认bridge的地址族、port、48 channels和connect-data形状与upstream一致；真正的确定性协议缺口是LuneX在三次SETUP后直接连接ENet，完全遗漏ANNOUNCE和PLAY。Sunshine只在ANNOUNCE中分配并注册stream session，缺失时control server无法按connect-data匹配并会主动拒绝peer，精确解释当前`connectionFailed`。开始实现`SETUP -> ANNOUNCE -> PLAY -> ENet`，并补实际vendored ENet回环/首包丢弃重传测试；不触碰live host、Keychain或Simulator。
- 2026-08-27：`ANNOUNCE -> PLAY -> ENet`实现的fresh warnings-as-errors构建通过；首轮focused为`29/30`，唯一失败是HDR codec fixture未声明新必需的control-v2能力而先收到`unsupportedControlEncryption`，生产代码无失败。补fixture后扩展能力真实性和失败边界：SDP feature mask从错误的`3`收窄为实际支持的session-ID位`2`，ENet server显式观察disconnect，并新增missing control-v2与ANNOUNCE/PLAY rejection不会连接ENet的回归。最终串行focused通过`32/32`，0 failure；三真实opt-in unset，未访问Keychain/live host或操作Simulator。下一步fresh related regression。
- 2026-08-27：首轮fresh related执行`74`项，其中16个断言/意外失败均来自`SessionCancellationTests`和`SessionRecoveryTests`仍使用旧五事务成功脚本：DESCRIBE未广告control-v2且缺CSeq 6/7，导致production正确提前失败，取消用例等待后续阶段超时。本轮不计验收；仅更新两个测试helper为七事务握手，不更改production或时序断言。全新串行related随后结构化通过`74/74`，0 skip/failure/expected failure，build为`succeeded / 0 error / 0 warning / 0 analyzer warning`；三opt-in unset且未访问Keychain/live host/Simulator。下一步fresh serial macOS normal。
- 2026-08-27：fresh serial macOS normal `/private/tmp/LuneX-M1-2_3-announce-normal.x0FG3C/Normal.xcresult`结构化通过`1310 total / 1308 passed / 2 skipped / 0 failed / 0 expected failure`，build为`succeeded / 0 error / 0 warning / 0 analyzer warning`。两个skip精确为`testLiveTanmyWhiteProductionAcceptanceWhenExplicitlyEnabled()`和`testRealKeychainIdentityRoundTripWhenExplicitlyEnabled()`；普通测试继续使用Debug文件identity fallback，未访问Keychain/live host或操作Simulator。下一步串行产品build与repository gate。
- 2026-08-27：fresh `/private/tmp/LuneX-M1-2_3-announce-builds.1rnyR3`串行完成macOS Debug/Release、iOS/iPadOS generic Debug、tvOS generic Debug与visionOS generic Debug `5/5`，五份structured build均为`succeeded / 0 error / 0 warning / 0 analyzer warning`；两个macOS executable均为`x86_64 arm64` universal。非macOS只计共享兼容，未查询、创建、启动或操作Simulator。下一步generator/OpenSpec/repository gate。
- 2026-08-27：ANNOUNCE/PLAY repository pre-gate通过：权威Ruby generator三次project hash稳定`783a7494...5944`，OpenSpec strict `11/11`、change `7/27 next 2.3 pending`，`git diff --check`、16文件scope、隐私扫描、三个真实opt-in unset和零build/test process均通过。人工审阅确认三SETUP后才ANNOUNCE、成功后才PLAY、成功后才ENet，capability/feature mask不夸大；普通teardown源码无`cancelRemoteSession:true`，不调用Sunshine `/cancel`。下一步fetch/remote基线与最终只读门后提交推送。
- 2026-08-27：首轮final wrapper在输出remote/OpenSpec/project/normal/精确skip checkpoint后无final marker提前结束，本轮不计完整验收。独立只读诊断确认skip字节序列、五build structured diagnostics、macOS两universal、零process、零`cancelRemoteSession:true`、16文件scope及untracked whitespace均实际通过；不重复任何build/test/generator/live，改用显式JSON和分段checkpoint重跑最终门。
- 2026-08-27：第二轮final wrapper已通过baseline/OpenSpec/diff/project/normal/五build/universal checkpoint，但在boundary以`rg`统计零`cancelRemoteSession:true`时被`pipefail`的零匹配exit 1中断，重复了历史已知包装器错误；产品与证据无失败且无runtime副作用。本轮不计完整验收，最终轮固定使用`awk`文件扫描，不再使用零匹配`rg`管道。
- 2026-08-27：纯`awk`修正后的完整repository gate输出`FINAL_ANNOUNCE_REPOSITORY_GATE_OK`：remote baseline `c105414`、OpenSpec `11/11`与`7/27 next 2.3 pending`、stable project `783a7494...5944`、normal `1310/1308/2/0`、五build零structured diagnostics、macOS双universal、三opt-in unset、零process/remote-cancel/privacy及16文件scope全部通过。Task 2.3保持pending，准备独立提交推送。
- 2026-08-27：ANNOUNCE/PLAY批次已提交推送为`05aa8771a15446ae82d925782fc6947b9dc4901b`，三方SHA一致且工作树clean；精确SHA fresh live bundle build为`succeeded/0/0/0`。唯一双opt-in gate正确返回xctest `1`，4.703秒回执为`launch=0/resume=1/cancel=0`、`controlEvents=launch_accepted,rtsp_ready,negotiated,channels_1,video_color_metadata`、`controlFailure=none`。四state文件`0600`且mode/size/SHA前后完全不变，零active xctest；不重跑。
- 2026-08-27：新live失败位于negotiated media启动后。根因是Network.framework为每个完整UDP datagram返回`isComplete=true`，通用`NetworkByteChannel`却沿用TCP EOF语义把channel改为`.closed`，下一次receive以`.invalidState`终止媒体环境。开始将complete receive按TCP/UDP分流并补连续datagram与真实UDP loopback状态回归；Task 2.3继续pending。
- 2026-08-27：fresh UDP focused首轮build structured diagnostics为`succeeded/0/0/0`，`NetworkChannelTests`执行`17/16/1/0`。唯一失败是新加的TCP loopback测试假设payload与EOF必须同一receive回调到达；实现收到非terminal payload后保持`.ready`是正确的，测试现改为继续读取并验证后续peer close使channel进入`.closed`。不把本轮计为通过，源码修改后以全新目录重跑。
- 2026-08-27：全新focused修正轮通过`17/17/0/0`且build structured diagnostics为`succeeded/0/0/0`。确定性覆盖连续complete UDP datagram、真实UDP echo后ready、TCP payload+terminal同回调、empty terminal及payload/peer-close分离回调；开始扩大媒体、RTSP、AppModel、取消与恢复相关矩阵。
- 2026-08-27：fresh related warnings-as-errors矩阵通过`225/224/1/0`，唯一skip精确为`testLiveTanmyWhiteProductionAcceptanceWhenExplicitlyEnabled()`，build structured diagnostics为`succeeded/0/0/0`。覆盖network channel、Moonlight media receive、session media environment、AppModel workflow、RTSP bootstrap、session cancellation与recovery；准备串行macOS normal。
- 2026-08-27：fresh串行macOS normal通过`1311/1309/2/0`，两个skip精确为`testLiveTanmyWhiteProductionAcceptanceWhenExplicitlyEnabled()`和`testRealKeychainIdentityRoundTripWhenExplicitlyEnabled()`；build structured diagnostics为`succeeded/0/0/0`。共享networking源码变化要求继续执行五份generic产品兼容build，但不推进冻结平台产品状态。
- 2026-08-27：五份串行产品build通过：macOS Debug/Release、iOS/iPadOS generic Debug、tvOS generic Debug、visionOS generic Debug均为`succeeded/0/0/0`；macOS两可执行文件均为`x86_64 arm64`。未创建、启动、克隆、查询或操作Simulator；冻结平台仅记录compatibility。
- 2026-08-27：权威Ruby generator连续两次返回`0`，`project.pbxproj`生成前/第一次/第二次SHA-256均稳定为`783a749484d0dc173a1296f7a573dfc0a7e9f5a3292fccf08046c0fa54035944`且工程零diff；准备执行OpenSpec与完整repository gate。
- 2026-08-27：完整repository gate输出`FINAL_UDP_REPOSITORY_GATE_OK`：retained focused/related/normal、五build、双universal、OpenSpec `11/11`与`7/27 next 2.3`、stable project、精确六文件scope、零untracked/diff error/新增skip/secret material/普通remote-cancel/opt-in/process及三方`05aa877`基线全部通过。人工调用点/diff复核无新问题；Task 2.3保持pending，准备提交精确SHA。
- 2026-08-27：UDP修复已以`f7c059bda4765923c8b67ebbf71ffa9a249356f3`提交推送并三方一致。fresh live bundle build结构化为`succeeded/0/0/0`；唯一双opt-in gate正确返回xctest `1`，1.981秒后回执前进至`media_presentation_failed / video_pipeline_failed / stream.video`，`launch/resume/cancel=0/1/0`且control events到`video_color_metadata`、零control failure。四state文件`0600`且mode/size/SHA不变，零process残留；本精确尝试不重跑。
- 2026-08-27：对照Sunshine FEC生成与moonlight-common-c接收逻辑定位下一确定性缺陷：LuneX错误地对data和parity一并强制`multiFECFlags==0x10`，但Sunshine只在编码前设置data shard，生成parity后不会重写该字节。现将marker要求收窄到data shard并新增非`0x10` parity成功、同字节data失败的相反回归；其余FEC envelope边界保持fail closed，Task 2.3继续pending。
- 2026-08-27：fresh parity focused `/private/tmp/LuneX-M1-2_3-parity-focused.HsLPvH/Focused.xcresult`结构化通过`10/10/0/0`，warnings-as-errors build为`succeeded/0/0/0`。新增非`0x10` parity成功和非法data marker失败回归均通过；三真实opt-in unset，未访问Keychain/live host/Simulator。下一步fresh串行媒体相关矩阵。
- 2026-08-27：fresh串行parity related `/private/tmp/LuneX-M1-2_3-parity-related.5SDTlR/Related.xcresult`通过`260/259/1/0`且build structured diagnostics为`succeeded/0/0/0`；唯一skip精确为显式live Sunshine gate。覆盖packet receive/assembly、session media、VideoToolbox/Metal、clock、diagnostics与AppModel；下一步fresh串行macOS normal。
- 2026-08-27：fresh串行parity normal `/private/tmp/LuneX-M1-2_3-parity-normal.1OEYxw/Normal.xcresult`通过`1312/1310/2/0`且build structured diagnostics为`succeeded/0/0/0`；两个skip精确为live Sunshine和真实Keychain opt-in。并行读取同一xcresult时辅助tests tree触发一次`database.sqlite3`移动竞争，随后串行读回成功，不重复测试；下一步五产品兼容build。
- 2026-08-27：fresh `/private/tmp/LuneX-M1-2_3-parity-builds.lVi5xX`串行五产品build `5/5`通过，macOS Debug/Release及iOS/iPadOS、tvOS、visionOS generic Debug的structured diagnostics均为`succeeded/0/0/0`；两macOS executable均为`x86_64 arm64`。未操作Simulator，冻结平台只计共享兼容；下一步generator/OpenSpec/repository gate。
- 2026-08-27：权威Ruby generator前与双跑后project hash均稳定为`783a7494...5944`且零drift。OpenSpec strict已成功生成JSON，但首个jq误把顶层object当array而exit `5`；只重解析保存JSON后确认`11/11`，apply为`7/27 next 2.3 pending`。现已补parity生成字节与data marker fail-closed场景，最终门将重跑post-spec strict，不重复测试/build/generator。
- 2026-08-27：首轮post-spec repository gate已通过三方remote和OpenSpec `11/11`、`7/27 next 2.3 pending`，但macOS awk不接受隐私正则中的空alternation并在source boundary退出；产品无失败、零runtime副作用，本轮不计完整门。改为显式PEM marker后从头重跑只读门，不重复test/build/generator/live。
- 2026-08-27：修正后的完整repository gate输出`FINAL_PARITY_REPOSITORY_GATE_OK`：三方`f7c059b`基线、OpenSpec `11/11`与`7/27 next 2.3 pending`、精确7文件、stable project、零privacy/普通remote-cancel、retained `10/10`、`260/259/1/0`、`1312/1310/2/0`、五build零structured diagnostics、双universal、三opt-in unset、fallback权限与零process全部通过。下一步人工diff审阅和独立提交推送，不勾选2.3。
- 2026-08-27：parity修复已以`3533b8078fe9c4eef634d3e840a2dc8dc3f1d28d`提交推送并三方一致；fresh exact-SHA live bundle build structured diagnostics为`succeeded/0/0/0`。首个wrapper因zsh local `path`覆盖命令搜索数组在状态快照阶段退出，无`LIVE_START`、无xctest、无host/session访问，不计尝试。
- 2026-08-27：同一精确SHA唯一actual双opt-in gate在6.571测试秒后失败为`stream_interrupted / transport_failed / stream.transport`，`launch/resume/cancel=0/1/0`、control events到`video_color_metadata`且零control failure；四state文件`0600`且mode/size/SHA不变，零process残留。本尝试不重跑；下一步先新增privacy-bounded typed media failure stage recorder和确定性映射测试，Task 2.3继续pending。
- 2026-08-27：恢复检查与当前Git/OpenSpec一致：`HEAD==origin/main==3533b807...`，仅四份既有证据文档未提交，change仍`7/27 next 2.3 pending`。已确认下一修复只需改macOS test harness：包装production video/audio receive providers并记录bounded stage/cause，不改产品源码、不操作Keychain/Simulator/host，也不重复旧exact-SHA live gate。
- 2026-08-27：新增test-only typed media recorder/wrappers与3项回归后，首轮focused在测试执行前因两个`XCTAssertEqual` autoclosure直接包含`await`而编译失败；无test/live/Keychain/host副作用。已改为先await局部值再断言，使用同一DerivedData增量重跑。
- 2026-08-27：media receipt focused第二轮通过`3/3/0/0`，xcresult structured build为`succeeded/0 errors/0 warnings/0 analyzer warnings`；证明video/audio stage归属、事件与stop转发、原始错误传播、bounded cause、unknown文本隔离和cancellation抑制。三个opt-in均unset、零build/test残留，开始相关AppModel/media回归。
- 2026-08-27：相关`AppModelWorkflowTests + MoonlightMediaReceiveTests + SessionMediaEnvironmentTests`通过`159 total / 158 passed / 1 exact live skip / 0 failed`，structured build为`succeeded/0/0/0`，零process残留。因仅test harness和文档变化，不重复冻结平台generic build；继续跑macOS normal全量门。
- 2026-08-27：pre-final macOS normal通过`1315 total / 1313 passed / 2 exact skips / 0 failed`，skips精确为live与真实Keychain，structured build `succeeded/0/0/0`。并行读取同一xcresult时一次只读summary因临时数据库争用失败，未重跑测试，串行重读成功。
- 2026-08-27：语义终审确认production timeout operation只有`connect/send/receive`，已细分为三类bounded cause，任意operation归`network_other_timed_out`；同步补OpenSpec privacy-bounded live media receipt场景。最终focused仍`3/3`，接下来对最终代码重跑normal与repository gates，不执行live。
- 2026-08-27：最终repository预门为OpenSpec strict `11/11`、apply `7/27 next 2.3 pending`、精确6文件、零产品源码/隐私命中/pbx drift/opt-in/process；终审发现尚缺wrapper consumer-termination直接回归，现补可控upstream测试并以兜底finish确保失败也不挂起，之后重跑focused与final normal。
- 2026-08-27：consumer-termination回归后focused通过`4/4/0/0`；final normal xcresult已串行确认`1316 total / 1314 passed / 2 skipped / 0 failed`，skips精确为live Sunshine与real Keychain opt-in，structured build为`succeeded/0 errors/0 warnings/0 analyzer warnings`。
- 2026-08-27：用户再次纠正free/busy不应影响客户端容量。审计定位production `initialSessionOperation`仍在different busy或missing state时本地拒绝；现改为matching running app用`/resume`，其余状态一律尝试`/launch`并以认证服务器响应为准，不调用`/cancel`制造free。同步更新RTSP回归、live harness计数、OpenSpec和matrix；开始新的focused/normal/build验收，尚未执行live、Keychain或Simulator。
- 2026-08-27：fresh host-admission focused xcresult通过`35/35/0/0`，structured build为`succeeded/0 errors/0 warnings/0 analyzer warnings`；直接覆盖different busy与missing state走`/launch`、matching running app走`/resume`以及4项media receipt语义。三个opt-in均unset，零live/Keychain/Simulator副作用，开始final normal。
- 2026-08-27：fresh final normal通过`1316 total / 1314 passed / 2 exact skips / 0 failed`，skips为live Sunshine和real Keychain，structured build `succeeded/0/0/0`。随后为确认generic destination误执行`xcodebuild -showdestinations`并只读列出Simulator destination；未创建、启动、安装、运行、关闭、删除或修改任何设备，但违反本轮不查询约束，已记录且后续只用generic placeholder。
- 2026-08-27：generic placeholder五build全部通过：macOS Debug/Release、iOS+iPadOS Debug、tvOS Debug、visionOS Debug均`succeeded/0 errors/0 warnings/0 analyzer warnings`，各有AIR/metallib；macOS两产物均`x86_64 arm64` universal。未再查询或操作Simulator，冻结平台只记compile compatibility。开始OpenSpec/repository/diff终审。
- 2026-08-27：首轮final repository wrapper的OpenSpec strict本身成功，但jq误读不存在的`.results/.summary.total`并在其余检查前退出；实际schema为`.items/.summary.totals`。零仓库/runtime副作用，修正汇总后从头重跑只读gate，不重复test/build/generator/live。
- 2026-08-27：修正后的完整只读门输出`FINAL_HOST_ADMISSION_REPOSITORY_GATE_OK`：strict `11/11`、apply `7/27 next 2.3 pending`、精确8文件、focused `35/35`、normal `1316/1314/2/0`、五build与Metal `5/5`、双macOS universal、stable project、零privacy/普通remote-cancel/opt-in/process均通过。Task 2.3不勾选，进入提交推送。
- 2026-08-27：host-state修复已以`5a4d1e650ef7cce87bb40b427037acf68c5f86fa`提交推送并三方一致；fresh live bundle build为`succeeded/0/0/0`。该SHA唯一双opt-in gate在6.472秒后失败为`stream_interrupted / transport_failed / stream.transport`，`launch/resume/cancel=0/1/0`、control events到`video_color_metadata`、无control failure，typed media receipt精确隔离为`audio_receive:network_receive_timed_out`。四state文件`0600`且mode/size/SHA不变，零process残留；该SHA不重跑。
- 2026-08-27：源码对照确认当前20-byte自定义`SS_PING`形状与上游一致，旧GFE 3.22的PLAY前ping注释不能直接解释当前Sunshine回执。真正确定性差异是上游将UDP receive timeout作为非致命poll，而LuneX每5秒无包就取消整个Network.framework channel并终止session。开始实施仅针对long-lived media UDP的无deadline receive；保留connect/send、真实transport错误、取消和全部TCP/RTSP timeout边界，Task 2.3继续pending。
- 2026-08-27：fresh warnings-as-errors focused `/private/tmp/LuneX-M1-media-idle-focused.kzeo3F/Focused.xcresult`结构化通过`25/25/0/0`，build为`succeeded / 0 error / 0 warning / 0 analyzer warning`。覆盖带deadline receive仍失败、无deadline UDP等待期间保持ready且显式取消有界清理、TCP/UDP真实loopback、媒体stop/replacement/consumer cancellation与ping/packet解析。源码调用点确认只有`MoonlightMediaReceive`使用无deadline入口，RTSP仍显式15秒receive deadline；已同步OpenSpec合同，下一步fresh related矩阵。
- 2026-08-27：人工API复核后将无deadline重载收紧为显式`receiveWithoutDeadline`，避免未来调用遗漏timeout却静默变成长等待；随后在最终源码上fresh重跑。focused `/private/tmp/LuneX-M1-media-idle-focused-final.NwjF3U/Focused.xcresult`仍为`25/25/0/0`，related `/private/tmp/LuneX-M1-media-idle-related-final.vMpQ2n/Related.xcresult`为`230 total / 229 passed / 1 skipped / 0 failed`，唯一skip精确为live Sunshine gate；两份structured build均`succeeded/0/0/0`且零process残留。下一步fresh串行macOS normal。
- 2026-08-27：fresh串行macOS normal `/private/tmp/LuneX-M1-media-idle-normal.f2AAdp/Normal.xcresult`结构化通过`1317 total / 1315 passed / 2 skipped / 0 failed`，两个skip精确为live Sunshine与real Keychain opt-in；build为`succeeded / 0 error / 0 warning / 0 analyzer warning`且零process残留。共享networking源码变化要求继续做五产品generic compatibility build，冻结平台不推进产品状态。
- 2026-08-27：fresh `/private/tmp/LuneX-M1-media-idle-builds.lbqLtc`串行五build全部通过：macOS Debug/Release、iOS+iPadOS generic Debug、tvOS generic Debug、visionOS generic Debug均为`succeeded/0 errors/0 warnings/0 analyzer warnings`，每项都有Metal AIR/metallib；两份macOS executable均为`x86_64 arm64` universal。未查询或操作Simulator，冻结平台仅计共享编译兼容；下一步generator/OpenSpec/repository gate。
- 2026-08-27：generator前与双跑后project hash稳定为`783a7494...5944`，OpenSpec strict `11/11`且apply仍`7/27 next 2.3 pending`。最终取消路径审阅发现`NWConnectionDriver`可返回typed `.cancelled`，而channel此前只识别Swift `CancellationError`，可能在无deadline stop后把状态重标`.failed`；现统一connect/send/receive取消归类并补driver typed-cancellation回归。由于production源码再次变化，之前成功证据降为pre-final辅助证据，需在最终源码重跑完整矩阵。
- 2026-08-27：取消归类后的fresh focused通过`26/26`，related通过`231 total / 230 passed / 1 live skip / 0 failed`。首个final normal为`1318 total / 1315 passed / 2 skipped / 1 failed`，唯一失败是无关的`testVisionResizePreservesCaptureUntilReplacementAndTeardown()`在测试helper固定1秒等待处超时，build structured diagnostics仍`succeeded/0/0/0`；同一用例从fresh DerivedData隔离运行以`0.020s`通过。该full normal不计验收，记录为并行负载夹具波动并执行一次fresh full normal复核；若再现则修夹具，不直接反复重跑。
- 2026-08-27：fresh full normal复核 `/private/tmp/LuneX-M1-media-idle-normal-final.wlhB4d/Normal.xcresult`通过`1318 total / 1316 passed / 2 skipped / 0 failed`，两个skip精确为live Sunshine与real Keychain；focused/related/normal三份structured build均`succeeded/0/0/0`，零process残留。下一步在最终源码重跑五build、generator/OpenSpec/repository gates。
- 2026-08-27：最终源码的fresh `/private/tmp/LuneX-M1-media-idle-builds-final.AcZDjx`五build全部通过；macOS Debug/Release、iOS+iPadOS、tvOS、visionOS的structured result均`succeeded/0/0/0`，各有1份AIR和1份metallib，两macOS executable均`x86_64 arm64` universal。未操作Simulator，进入generator/OpenSpec/repository final gate。
- 2026-08-27：完整最终repository gate输出`FINAL_MEDIA_IDLE_REPOSITORY_GATE_OK`：三方基线`5a4d1e65`、generator双跑project hash稳定`783a7494...5944`、OpenSpec strict `11/11`与apply `7/27 next 2.3 pending`、精确9文件scope、focused `26/26`、related `231/230/1/0`、normal `1318/1316/2/0`、五build/Metal/双universal、零privacy/普通remote-cancel/opt-in/process全部通过。Task 2.3保持未勾选，准备独立提交推送。
- 2026-08-27：media idle修复已提交推送为`edb75bf26f02b54da51945a2d44b5742ed31d31b`且HEAD/origin/main/GitHub remote三方一致、工作树clean。该SHA fresh live bundle `/private/tmp/LuneX-M1-media-idle-live-build.2IhDz9`为`succeeded/0/0/0`，普通Debug identity继续使用文件fallback，未启用real Keychain或操作Simulator。
- 2026-08-27：`edb75bf`唯一双opt-in live gate以xctest exit `1`在13.355测试秒后结束；`launch/resume/cancel=0/1/0`、control events为`launch_accepted,rtsp_ready,negotiated,channels_1,video_color_metadata,channels_0,reconnecting_1`、control bootstrap failure为none、`mediaFailures=none`。终点为`input_unavailable / input_stream_ended / stream.input`，证明上一轮audio receive timeout已消失，下一缺陷在已建立ENet control/input stream丢失后的生命周期/reconnect路径，不是free/busy或host admission。
- 2026-08-27：live wrapper用`pipefail`正确捕获xctest失败，但gate后的`pgrep`在`set -e`下因零匹配提前退出报告尾；没有重跑live。独立只读post-check确认四state文件mode `0600`、size、SHA-256逐项不变，活动xctest/xcodebuild均为0，日志SHA-256为`d47fe093507181a11fa174d87a320d0eaff004cb5888a2cf3ede753d3ef00876`。该SHA不得重跑，Task 2.3继续pending。
- 2026-08-27：对照Sunshine与moonlight-common-c锁定10秒control loss根因：Sunshine默认`ping_timeout=10s`且只在收到control数据时续期；上游START A/B后立即并每100ms发送可靠`0x0200` periodic ping，LuneX此前完全缺失。现将periodic ping接入generation-owned `MoonlightControlChannel`，与IDR/input共享加密序列及ENet队列，stop清除deadline；同步补精确payload、重复周期、失败回滚、stop和input序列回归，尚未执行live/Keychain/Simulator。
- 2026-08-27：续接审阅确认periodic ping的type、8-byte确定性payload、generic channel、reliable flag及共享AES-GCM sequence与上游合同一致；补齐`ControlChannelTests.assertAsyncError`测试helper，准备执行fresh warnings-as-errors focused gate。Task 2.3继续pending，未运行live、real Keychain或Simulator。
- 2026-08-27：首轮keepalive focused在测试执行前因两处`XCTAssertEqual(try await ...)`触发Swift 6同步autoclosure编译错误；产品源码无诊断、零测试/runtime副作用。已改为先await局部event再断言，将从fresh DerivedData/result重跑而不复用失败bundle。
- 2026-08-27：fresh focused r2 `/private/tmp/LuneX-M1-control-keepalive-focused-r2.edj5kQ/Focused.xcresult`通过`108/108/0/0`，structured build为`succeeded/0/0/0`；三个真实opt-in unset且零进程残留。终审补充生产100ms常量断言和ENet service阻塞期间stop的竞态回归，要求旧generation恢复后只能`.invalidState`且不得产生late keepalive；该新增测试尚待fresh复核。
- 2026-08-27：最终lifecycle focused `/private/tmp/LuneX-M1-control-keepalive-lifecycle.iKCxAp/Lifecycle.xcresult`通过`11/11/0/0`且structured build为`succeeded/0/0/0`；明确证明100ms合同、阻塞service期间stop、旧generation event拒绝、仅初始一份ping且零late send。当前实现无需额外常驻Task；production provider的replacement会等待teardown与旧bootstrap结束后再复用control channel。下一步session/media/AppModel related矩阵。
- 2026-08-27：fresh related `/private/tmp/LuneX-M1-control-keepalive-related.2Y2Ebj/Related.xcresult`通过`318 total / 317 passed / 1 skipped / 0 failed`，唯一skip精确为显式live Sunshine gate；structured build为`succeeded/0/0/0`。覆盖control/ENet/input/RTSP/recovery/cancellation/media environment/resource ownership/network/AppModel/state machine，确认replacement串行teardown、reconnect和普通零`/cancel`语义未回归。下一步fresh macOS normal。
- 2026-08-27：fresh normal `/private/tmp/LuneX-M1-control-keepalive-normal.Rwo9gd/Normal.xcresult`通过`1321 total / 1319 passed / 2 skipped / 0 failed`，skips精确为live Sunshine与real Keychain opt-in；structured build为`succeeded/0/0/0`。并行读取同一xcresult时tests tree再次触发Xcode 26.4内部`database.sqlite3`移动竞争，已串行只读成功且未重跑测试。下一步共享networking五产品generic build。
- 2026-08-27：fresh `/private/tmp/LuneX-M1-control-keepalive-builds.SF1yHZ`五项build全部通过；macOS Debug/Release、iOS+iPadOS generic Debug、tvOS generic Debug、visionOS generic Debug的五份structured result均为`succeeded/0 errors/0 warnings/0 analyzer warnings`，各有1份AIR和1份metallib，两份macOS executable均为`x86_64 arm64` universal。未查询或操作Simulator，冻结平台只计共享编译兼容；下一步generator/OpenSpec/repository final gate。
- 2026-08-27：权威`/usr/bin/ruby Tools/generate_xcodeproj.rb`连续两次成功，生成前、第一次及第二次`LuneX.xcodeproj/project.pbxproj` SHA-256均为`783a749484d0dc173a1296f7a573dfc0a7e9f5a3292fccf08046c0fa54035944`，工程零drift。未重跑test/build，下一步OpenSpec strict与最终repository gate。
- 2026-08-27：首轮只读repository gate确认OpenSpec strict `11/11`、retained `11/11`、`318/317/1/0`、`1321/1319/2/0`、五build structured diagnostics全零、精确8文件、fallback `0700/0600`与零普通remote cancel；但隐私扫描再次因macOS awk不接受复合正则而退出，skip提取也误用不存在的`testStatus`字段。产品/仓库/runtime零副作用，本轮不计完整门；改用显式PEM marker与`.result`字段从头重跑，只读复用证据。
- 2026-08-27：修正后的完整只读门输出`FINAL_CONTROL_KEEPALIVE_REPOSITORY_GATE_OK`：三方`edb75bf`基线、OpenSpec strict `11/11`与apply `7/27 next 2.3 pending`、精确8文件、project hash稳定、retained `11/11`、`318/317/1/0`、`1321/1319/2/0`、五build/Metal、双macOS universal、精确skip、fallback `0700/0600`以及零privacy/普通remote-cancel/opt-in/process全部通过。人工代码/测试/文档审阅未发现新问题；下一步独立提交推送，不勾选2.3。
- 2026-08-27：同步最终离线状态后只读复核输出`FINAL_CONTROL_KEEPALIVE_POST_RECORD_GATE_OK`，再次确认精确8文件、diff hygiene、OpenSpec `11/11`与`7/27 next 2.3 pending`、retained tests/builds/Metal/universal、fallback权限和零privacy/普通remote-cancel/opt-in/process；未重跑test/build/generator/live。准备提交推送。
- 2026-08-27：control keepalive修复已提交并推送为`9516a557de2ebe03ff32ecc5c74d42f49a3afeb9`，`HEAD == origin/main == GitHub remote`且提交后工作树clean。fresh live bundle `/private/tmp/LuneX-M1-control-keepalive-live-build.6klv5l`为`succeeded/0 errors/0 warnings/0 analyzer warnings`。
- 2026-08-27：`9516a557`唯一双opt-in live gate以xctest exit `1`在45.829秒后结束；`launch/resume/cancel=0/1/0`，events为`launch_accepted,rtsp_ready,negotiated,channels_1,video_color_metadata`，control/media failure均为none。与旧SHA相比，`channels_1`跨越Sunshine 10秒deadline持续到gate结束，未出现`channels_0/reconnecting/input_stream_ended`，直接证明应用层control keepalive修复有效。
- 2026-08-27：该live gate未进入AppModel `streaming`，video/audio readiness始终未加入；45秒后由harness执行本地stop，因此cleanup后的`phase=idle, issue=none`不能解释为会话自行完成。下一步审计媒体UDP ping、datagram receive、parser/processor acceptance和cleanup前readiness的privacy-bounded test-only telemetry；Task 2.3继续pending且不重跑该SHA。
- 2026-08-27：post-check确认四个state文件mode/size/SHA完全不变、state diff为0 bytes、活动xcodebuild/xctest均为0，live log SHA-256为`d237e828c4e582b27e79cca975e1e464eb82958af9ef21748ad95d07aa25aa73`。wrapper仅在已完成live后的零匹配`pgrep` + `set -e`报告尾退出，未据此重跑。
- 2026-08-27：media activity harness首轮focused通过`5/5`且structured build为`succeeded/0/0/0`；补provider event直接断言后的fresh复核为`4/5`，唯一失败是测试fixture使用`example.invalid`却错误期望endpoint kind为`ipv4`，实际正确为`name`。production源码、live/Keychain/Simulator均无副作用；改正断言并用保留IPv4地址独立覆盖地址分类/不泄露后，从fresh evidence复核一次，不复用失败bundle。
- 2026-08-27：修正后的fresh activity focused `/private/tmp/LuneX-M1-media-activity-focused-r3.p98rr0/Focused.xcresult`通过`5/5/0/0`；完整AppModel workflows `/private/tmp/LuneX-M1-media-activity-related.bqiijY/Related.xcresult`通过`102 total / 101 passed / 1 exact live skip / 0 failed`；两份structured build均`succeeded/0/0/0`。
- 2026-08-27：fresh macOS normal `/private/tmp/LuneX-M1-media-activity-normal.Bq1pqG/Normal.xcresult`通过`1322 total / 1320 passed / 2 exact opt-in skips / 0 failed`，skips精确为live Sunshine与real Keychain；structured build为`succeeded/0/0/0`且零process残留。本批仅test harness/OpenSpec/planning/docs变化，不修改产品源码或工程membership，不要求冻结平台generic build。
...
- 2026-08-27：继续审阅 normalized FEC assembler。确认 production parser 已丢弃 parity 后，原始 stream sequence 天然存在空洞，必须用 block/shard 完整性判定；补强 frame-level `dataShardCount`、`parityShardCount`、`fecPercentage` 一致性，2-bit block envelope、非负 shard 范围和同 shard 冲突重复的 fail-closed 处理。未运行 live、Keychain、Simulator 或 host 操作；下一步新增回归并从 fresh evidence 重新验收。
- 2026-08-27：FEC hardening fresh focused `/private/tmp/LuneX-M2-video-fec-focused-final.fpj07t/Focused.xcresult` 通过 `3/3`，覆盖 parity-gap assembly、metadata drift/conflicting shard 与 block envelope；structured build `succeeded/0/0/0`，无 xcodebuild/xctest 残留。
- 2026-08-27：视频媒体相关矩阵 `/private/tmp/LuneX-M2-video-fec-related-final.bQkVXL/Related.xcresult` 通过 `94/94`，完整 macOS normal `/private/tmp/LuneX-M2-video-fec-normal-final.wOFT9w/Normal.xcresult` 通过 `1330/1328/2 exact skips/0`，skip 精确为 live Sunshine 与 real Keychain；所有 structured diagnostics 为零，普通测试使用文件 fallback，未操作 Simulator。
- 2026-08-27：共享源码五产品 generic build `/private/tmp/LuneX-M2-video-fec-builds-final.q1a74B` 全部成功且 `0/0/0`，macOS Debug/Release 均为 `x86_64 arm64` universal，Metal AIR/metallib 共 `10` 个；generator 双跑 project hash 稳定为 `783a7494...5944`，OpenSpec strict `11/11`。准备最终只读 repository gate，Task 2.3 仍 pending。
- 2026-08-27：新 exact SHA `212958f` 已推送后完成唯一一次双 opt-in live gate。`tanmy-white/Desktop` 按匹配运行应用选择 `/resume`，回执为 `launch=0/resume=1/cancel=0`，control/RTSP/ENet readiness 到达 `channels_1` 与 video color metadata。46 秒内视频收到 `26578` 个 datagram、产生 `20144` 个 parser event；音频连接并发送 `87` 个 ping 但收到 `0` 个 datagram，未产生 decoded audio readiness，模型停留 `waitingForTransport`，由 harness 有界本地 stop 并以 `stream.video` 失败退出。四个 LuneX 状态文件均保持 `0600` 与原 SHA，未产生 remote cancel，进程清理为零；该 SHA 不重跑，Task 2.3 继续 pending。
- 2026-08-28：恢复 planning-with-files 与 OpenSpec apply；change `prioritize-macos-product-completion` 为 `spec-driven`、`7/27 ready`，proposal/design/spec/tasks全部重读。对照上游确认moonlight-common-c在audio SETUP后、PLAY前启动audio UDP ping，而LuneX在完整PLAY/control后才由media environment同时启动audio/video；该时序差异进入下一离线回归和生产修复候选。`212958f`首次universal live bundle构建还暴露既有x86_64 HDR Float16测试编译错误，发生在host/test前且不计live；arm64 bundle成功后唯一live结果保持既有记录。
- 2026-08-28：用既有trusted SSH只读确认`tanmy-white`为Windows `10.0.26200.9168`、用户`tanmy-white\\tanmy`，Sunshine进程与自动服务均运行且安装在`C:\\Program Files\\Sunshine`。`config\\sunshine.log`当前为0字节，下一步读取Windows事件日志与限定音频/日志配置键。两条初始PowerShell分别因嵌套引用和空管道语法在读取前失败，已记录并改用UTF-16LE `-EncodedCommand`；远端未修改。
- 2026-08-28：主机只读事件取证发现Sunshine于`2026-08-27 04:20:44 +08:00`记录Event 1000/1001 `APPCRASH`，故障模块`KERNELBASE.dll`、异常码`0x80000003`，进程于`04:21:28`重建；但`212958f` gate实际运行于`04:43:20–04:44:06`，因此该崩溃早于零音频回执约22分钟，不是该gate的中断原因。限定配置查询仅见`output_name`，未见显式audio/virtual sink/Steam audio driver/capture/log override；下一步查询Windows音频服务、endpoint和可用Sunshine日志位置。
- 2026-08-28：Windows `Audiosrv`/`AudioEndpointBuilder`均自动运行，物理音频设备、`Steam Streaming Speakers`及playback endpoint均报告`OK`且无PnP error；唯一Sunshine诊断文件仍为0字节日志。静态取证无法回溯audio peer是否登记，所以进入上游一致的SETUP后/PLAY前audio ping修复，且必须由同一UDP socket被后续receive provider接管。
- 2026-08-28：已实现shared audio reservation、SETUP后首ping、PLAY前refresh ping、同socket claim及sequence续接，并注入production inventory和live receipt harness；增加handoff/replacement/missing-reservation/RTSP ordering回归，同时将HDR readback的`Float16`先转`Float`以恢复x86_64编译。首次focused build因不存在的`LuneX` scheme与zsh只读`status`变量编排错误在编译前退出，未触发测试或host。
- 2026-08-28：audio reservation focused最终在`/private/tmp/LuneX-audio-reservation-build.SpINHJ/Focused-r2.xcresult`通过`43/43`，覆盖RTSP顺序、同socket handoff、ping sequence续接、replacement、unclaimed cancel、missing reservation fail-closed及既有media parser/cancellation。xcresult结构化诊断为`0 error / 0 warning / 0 analyzer warning`；build log仅有AppIntents无framework的metadata skip工具提示，非源码诊断。
- 2026-08-28：session/reconnect/AppModel related在`Related-r1.xcresult`通过`282 total / 281 passed / 1 live opt-in skip / 0 failed`且structured diagnostics全零。fresh `arm64 x86_64` test build确认既有HDR readback在x86_64无法使用`Float16(bitPattern:)`或转`Float`；现改为架构无关的IEEE-754 binary16显式解码，待fresh universal重验。
- 2026-08-28：fresh universal test bundle在`/private/tmp/LuneX-audio-reservation-universal-r2.XxGqru`成功生成`x86_64 arm64`，关闭HDR test compile缺口。完整macOS normal在`/private/tmp/LuneX-audio-reservation-normal.Oxh61e/Normal.xcresult`通过`1333 total / 1331 passed / 2 explicit skips / 0 failed`，skip精确为live Sunshine和real Keychain，structured build diagnostics为`0/0/0`。
- 2026-08-28：macOS Debug/Release universal及iOS/iPadOS、tvOS、visionOS generic compatibility构建全部`BUILD SUCCEEDED`，macOS两个产品执行文件均为`x86_64 arm64`，五产品Metal AIR/metallib共10个；冻结平台结果仅作兼容性证据。
- 2026-08-28：提交前逐文件审阅确认audio reservation在actor reentrancy、replacement、bootstrap failure、reconnect、claim ownership及teardown路径保持单socket与有界取消，普通停止仍为零remote `/cancel`。同时发现test-only live control receipt仍保存任意错误描述；已改为封闭cause枚举、忽略取消并增加对抗隐私测试，待fresh focused/related回归。
- 2026-08-28：隐私修订后的fresh `AppModelWorkflowTests` `/private/tmp/LuneX-audio-privacy-related.mb8bXV/Related.xcresult`通过`103/102/1 live skip/0`，structured build为`succeeded/0 error/0 warning/0 analyzer warning`；`LUNEX_RUN_KEYCHAIN_TEST`、`LUNEX_RUN_LIVE_HOST_TEST`、`LUNEX_RUN_LIVE_DESKTOP_SESSION`均显式unset，未访问真实Keychain或host。
- 2026-08-28：最终test-only隐私修订的fresh universal bundle `/private/tmp/LuneX-audio-precommit-universal.psNLhP`成功且`x86_64 arm64`；generator双跑hash稳定为`783a7494...5944`并且project零diff，OpenSpec strict `11/11`、apply `7/27 ready`且2.3仍pending。
- 2026-08-28：pre-commit repository gate通过：diff hygiene clean、HEAD与`origin/main`同为`a1647b1`、精确10个修改文件；源码/测试修改无remote cancel true、任意error description、endpoint/identity/certificate/key/payload材料新增，live/Keychain opt-in默认关闭，零xcodebuild/xctest残留。下一步提交推送新exact SHA，再且仅再执行一次double-opt-in live gate。
- 2026-08-28：audio reservation修复已提交推送为exact SHA `0b803c3eb1c5a1a2cd2c3c8680da0c7f0fa35c8f`；从clean pushed SHA fresh构建arm64 live bundle `/private/tmp/LuneX-live-0b803c3e.GP9JrU`。
- 2026-08-28：`0b803c3`唯一double-opt-in live gate结束且不重跑。Desktop匹配运行状态走`/resume`，`launch/resume/cancel=0/1/0`；control到`channels_1`与video metadata且无control/media receive failure。live recorder只在provider启动时配置，所以`connections=0,pings=1`直接表示handoff后的runtime sequence 2 ping，SETUP/PLAY前sequence 0/1由确定性测试而非live计数证明；live收到87 datagram并产出58 parser event，关闭旧audio零包缺陷。随后本地`audio_pipeline_failed/audio_output_unavailable`在1.125秒内终止，audio runtime未发布、video仅1 datagram/0 event，完整2.3矩阵仍pending。
- 2026-08-28：live前后Sunshine PID/start/event列表一致，四个LuneX状态文件mode/size/hash一致，Git clean、零remote cancel、零xcodebuild/xctest残留。下一步定位有限的Opus decode/AVAudioEngine output失败原因，实施下一最小修复并为新SHA重走全部门禁。
- 2026-08-28：新增产品级audio processing有限cause，将Opus/jitter/pipeline/runtime关联值丢弃并增加secret-bearing graph对抗测试。首轮focused `/private/tmp/LuneX-audio-cause-focused.0IoMlX/Focused.xcresult`为`32/33`，唯一失败是测试误把合法枚举词`payload`当作泄露；产品分类与编译无失败，已拆分断言并要求fresh复核，不复用该bundle。
- 2026-08-28：修正测试断言后的fresh focused `/private/tmp/LuneX-audio-cause-focused-r2.IEu55O/Focused.xcresult`通过`33/33/0/0`；fresh相关音频/runtime/AppModel矩阵`/private/tmp/LuneX-audio-cause-related.Fib1FH/Related.xcresult`通过`268 total / 267 passed / 1 live opt-in skip / 0 failed`。两份structured build均为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`，未访问live host、真实Keychain或Simulator。
- 2026-08-28：fresh完整macOS normal `/private/tmp/LuneX-audio-cause-normal.iTatVA/Normal.xcresult`结构化通过`1335 total / 1333 passed / 2 skipped / 0 failed / 0 expected failures`，build为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`；两个skip从tests tree精确确认为`testLiveTanmyWhiteProductionAcceptanceWhenExplicitlyEnabled()`和`testRealKeychainIdentityRoundTripWhenExplicitlyEnabled()`，普通测试继续使用Debug文件identity fallback。下一步执行universal test build、五产品generic compatibility build与完整repository gate。
- 2026-08-28：用户明确恢复后再次调用`create_goal`，控制面仍将旧`blocked`阶段13-20 goal视为unfinished并拒绝新建；没有仓库、host、Keychain、Simulator或runtime副作用。继续以OpenSpec `prioritize-macos-product-completion`与三份planning files为执行权威，不虚假关闭未完成目标。
- 2026-08-28：最终源码fresh universal test build `/private/tmp/LuneX-audio-cause-universal.FdMwmr/Universal.xcresult`结构化为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`，`LuneXCoreTests`二进制精确为`x86_64 arm64`。随后五产品build `/private/tmp/LuneX-audio-cause-builds.tFbGS5`全部通过，macOS Debug/Release、iOS/iPadOS、tvOS、visionOS五份structured result均为`succeeded / 0 / 0 / 0`；未运行或操作Simulator，冻结平台只计共享编译兼容。
- 2026-08-28：五build结果读取器正确解析全部structured result后，产物架构步骤错误假设app名为`LuneX.app`并在只读`find`断言退出；构建与仓库/runtime无副作用且不重跑build。枚举现有Products确认实际产物为`LuneX-macOS.app/Contents/MacOS/LuneX-macOS`，后续只对该路径读取架构。
- 2026-08-28：首轮完整repository wrapper静默退出；分段复核确认baseline/OpenSpec、retained tests、五build、universal/双macOS架构、隐私、零remote cancel、fallback权限、opt-in与零process全部通过，唯一失败是过宽`find . -name '*.profraw'`命中`.gitignore`覆盖的既有`build/IdentityLifecycle-Tests/...profraw`。该文件mtime为`2026-07-10`且非本轮污染，不删除；最终门改为精确检查Git可见未跟踪`*.profraw`和仓库根`default.profraw`，不重复test/build/generator/live。
- 2026-08-28：修正coverage边界后的完整只读门输出`FINAL_AUDIO_CAUSE_REPOSITORY_GATE_OK`：三方baseline `0b803c3`、project hash稳定`783a7494...5944`、OpenSpec strict `11/11`与apply `7/27 next 2.3 pending`、精确5文件scope、focused `33/33`、related `268/267/1/0`、normal `1335/1333/2/0`、universal test与双macOS产品均`x86_64 arm64`、五build/Metal `5/5`、零privacy/remote-cancel/opt-in/process/Git-visible coverage污染全部通过。下一步人工diff终审后提交推送，不勾选2.3。
- 2026-08-28：finite audio cause候选以exact SHA `838e5b56543c2e4127e286b652a8b614e218179f`提交推送并三方一致；从clean pushed SHA构建arm64 live bundle `/private/tmp/LuneX-live-838e5b5.mQT1aK`，structured build为`succeeded/0/0/0`且绑定该SHA。
- 2026-08-28：`838e5b5`唯一double-opt-in live gate已消费且不得重跑。matching Desktop走`/resume`，`launch/resume/cancel=0/1/0`；control到`channels_1`/video metadata，media receive无typed failure，audio收到`93` datagram并产出`62` parser event；1.176秒后明确失败为`audio_pipeline_schedule_capacity / audio_output_unavailable`，因此根因收窄到本地AudioSessionPipeline调度容量/backpressure，而非decrypt/Opus/PCM/clock/host audio。video仅1 datagram/0 event是音频早停后的截面，不判定视频回归。
- 2026-08-28：live前后四个LuneX状态文件mode/size/hash一致，Sunshine PID/start/event快照一致，Git clean、零remote cancel、零build/test进程；live log SHA-256为`d49c7dd...3460`。下一步审计AudioSessionPipeline pending capacity、player-node completion与音频首批burst合同，实施最小产品修复后为新SHA重走离线门；Task 2.3继续pending。
- 2026-08-28：已实施低延迟有界backpressure：`AudioSessionPipeline.schedule()`在8-buffer容量满时FIFO等待`.dataConsumed`，stop/reconfigure/generation invalidation以`.notRunning`唤醒，Task cancellation删除对应waiter，late completion由generation隔离。首轮fresh focused `/private/tmp/LuneX-audio-backpressure-focused.YXYpo4`在测试执行前以exit `65`失败，structured result为`0 tests / 2 errors / 0 warnings`，唯一源码诊断为`withCheckedThrowingContinuation`无法推断`Void`。已显式标注`CheckedContinuation<Void, Error>`，将从fresh证据目录重跑；该编译失败不是产品行为回归，Task 2.3仍pending。
- 2026-08-28：第二fresh focused `/private/tmp/LuneX-audio-backpressure-focused-r2.FrkRDb`仍在测试执行前失败，产品源码已编译；`xcresult` summary为`0 total`，build为`5 errors / 0 warnings / 0 analyzer warnings`。四个精确诊断均指向新测试Task closure通过`makePCM`捕获XCTest `self`的Swift 6 sending-closure风险，已改为Task外预构建Sendable PCM value。该bundle不复用，将从第三个fresh目录验证。
- 2026-08-28：第三轮focused `/private/tmp/LuneX-audio-backpressure-focused-r3.cSHy9c/Focused.xcresult`通过`30 total / 30 passed / 0 skipped / 0 failed`，structured build为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。新回归实测了容量满时不立即失败、消费后恢复、取消不占用容量、stop/reconfigure唤醒及旧completion不污染新generation。zsh wrapper仅在Xcode成功后因赋值只读`status`退出，bundle/log完整，不重跑；后续使用`exit_code`。
- 2026-08-28：fresh related `/private/tmp/LuneX-audio-backpressure-related.L6W4Bf/Related.xcresult`通过`271 total / 270 passed / 1 skipped / 0 failed`，structured build为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。唯一skip精确为`testLiveTanmyWhiteProductionAcceptanceWhenExplicitlyEnabled()`；真实Keychain、live host和Simulator均未触发。覆盖AudioPipeline、AudioRuntimeRecovery、NativeSessionAudioProcessor、jitter、Opus、SessionMediaEnvironment、RuntimeDiagnostics和AppModelWorkflow。
- 2026-08-28：fresh macOS normal `/private/tmp/LuneX-audio-backpressure-normal.dK1TgP/Normal.xcresult`通过`1338 total / 1336 passed / 2 skipped / 0 failed / 0 expected failures`，structured build为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。两个skip精确是live Sunshine与真实Keychain显式opt-in，环境中无`LUNEX_RUN_*`，普通测试继续文件identity fallback；零xcodebuild/xctest残留。
- 2026-08-28：fresh universal test build `/private/tmp/LuneX-audio-backpressure-universal.3NrPfA/Universal.xcresult`为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`，`LuneXCoreTests` Mach-O精确为`x86_64 arm64`。五产品generic build根`/private/tmp/LuneX-audio-backpressure-builds.kKMKTC`中macOS Debug/Release、iOS/iPadOS Debug、tvOS Debug、visionOS Debug全部`succeeded / 0 / 0 / 0`，每份均生成1 AIR和1 metallib，两个macOS App均为`x86_64 arm64`。未操作Simulator，冻结平台仅记共享编译兼容。
- 2026-08-28：generator连续两次运行前/后hash均为`783a7494...5944`且project零drift。首轮repository wrapper的OpenSpec CLI本身已返回strict `11/11`，但汇总jq误用`.results`导致在后续断言前退出；已改用当前`.items`字段，将不重跑已完整的test/build，而是从头重跑只读repository gate。
- 2026-08-28：修正schema后的完整仓库门输出`FINAL_AUDIO_BACKPRESSURE_REPOSITORY_GATE_OK`：三方baseline `838e5b5`、精确5文件scope、OpenSpec strict `11/11`与apply `7/27 next 2.3 pending`、stable project hash、focused `30/30`、related `271/270/1/0`、normal `1338/1336/2/0`、universal test与双macOS产品`x86_64 arm64`、五build/Metal `5/5`、fallback `0700/0600`及零privacy/remote-cancel/opt-in/process/coverage drift全部通过。Task 2.3不勾选，下一步人工diff终审后提交推送。
- 2026-08-28：提交前人工差异终审确认production `AVAudioPlayerNode.scheduleBuffer`实际使用`.dataConsumed`，actor内容量检查在每次唤醒后重新执行，backend schedule失败释放槽位，取消与stop/reconfigure不会双重resume continuation，generation不匹配的late completion不能修改新队列。无容量上限放大、无新remote cancel或隐私材料，diff hygiene通过；准备提交推送。
- 2026-08-28：音频backpressure修复已以exact SHA `9d4cabe12ea5a55dd847916e0fc9bd735bb5e7cf`提交推送，三方一致且工作树clean。从clean pushed SHA构建fresh arm64 live bundle `/private/tmp/LuneX-live-9d4cabe.tOhqrR`，structured build为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`，codesign验证通过，Mach-O为arm64，可执行SHA-256为`f4bfd965...0d94`。
- 2026-08-28：`9d4cabe`唯一double-opt-in live gate从`18:24:31Z`到`18:25:18Z`执行并以exit `1`结束，不得重跑。matching Desktop走`/resume`，`launch/resume/cancel=0/1/0`，control保持`channels_1`，无control/media failure。音频为`running`且收到`13500 datagrams / 9000 events`，证明本批已解决容量fatal；新阻塞为video虽收到`304531 datagrams / 247604 events`，但published `frames=0`，pre-cleanup仍为`waitingForTransport`。live log SHA-256为`e72dd10d...c1da2`。
- 2026-08-28：live前后四个LuneX状态文件mode/size/hash完全一致，Sunshine PID `14640`、start time、service和Event 1000/1001快照完全一致，零remote `/cancel`且零build/test残留。direct xctest生成的唯一未跟踪`default.profraw`已按精确路径用`unlink`删除，Git恢复clean；不重跑live。下一步审计video parser到首帧发布的有限阶段证据和队列消费。
- 2026-08-28：对照本地Sunshine `stream.cpp:1552-1600,1623-1690`与moonlight-common-c `RtpVideoQueue.c:690-790`确认multi-FEC以多个独立block分割一帧，最后块按剩余payload延伸，因而每块`data_shards`合法地可以不同。production normalized assembler错把首块shard count/parity count/fec percentage保存为整帧全局元数据，使短尾块的每帧都被当作`inconsistentFrameMetadata`，精确解释海量parser event但零access unit/零published frame。
- 2026-08-28：已将normalized assembler改为per-block FEC metadata：整帧只要求FEC模式与last-block envelope一致，各block内要求data/parity/percentage一致，不再要求不同block shard count相同。fresh focused `/private/tmp/LuneX-video-multifec-focused.E1R2PX/Focused.xcresult`通过`6/6/0/0`，structured build为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`；新用例先输1-shard尾块再输2-shard首块，并验证完整access unit。
- 2026-08-28：fresh multi-FEC related `/private/tmp/LuneX-video-multifec-related.2aZC07/Related.xcresult`通过`229 total / 228 passed / 1 skipped / 0 failed`，structured build为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`；唯一skip为显式live Sunshine gate。覆盖MoonlightMediaReceive、VideoPacketAssembly、VideoDecodePipeline、VideoFormatDescription、SessionMediaEnvironment、RuntimeDiagnostics与AppModelWorkflow，未触发Keychain、host或Simulator。
- 2026-08-28：fresh multi-FEC macOS normal `/private/tmp/LuneX-video-multifec-normal.ymguAk/Normal.xcresult`通过`1339 total / 1337 passed / 2 skipped / 0 failed / 0 expected failures`，structured build为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。两个skip仍精确为live Sunshine和真实Keychain opt-in，普通测试继续文件identity fallback，零build/test残留。
- 2026-08-28：fresh multi-FEC universal `/private/tmp/LuneX-video-multifec-universal.IUZdji/Universal.xcresult`为`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`，test bundle为`x86_64 arm64`。五产品generic build根`/private/tmp/LuneX-video-multifec-builds.55urUg`中全部`succeeded / 0 / 0 / 0`，每份1 AIR/1 metallib，macOS Debug/Release均为`x86_64 arm64`；冻结平台仅记共享编译兼容，未操作Simulator。
- 2026-08-28：恢复后重新读取planning-with-files、OpenSpec apply skill及`prioritize-macos-product-completion`全部context；schema为`spec-driven`、apply为`7/27 ready`且next仍是2.3。`create_goal`再次因旧blocked goal仍被服务端视作unfinished而拒绝，未虚假完成旧goal；继续以OpenSpec和三份planning files推进。
- 2026-08-28：当前multi-FEC源码上权威generator连续两次运行，生成前/第一次/第二次`project.pbxproj` SHA-256均为`783a749484d0dc173a1296f7a573dfc0a7e9f5a3292fccf08046c0fa54035944`且工程零drift。OpenSpec strict通过`11/11`，apply仍为`7/27 next 2.3 pending`；retained focused/related/normal/universal/五build证据目录全部存在，下一步完整只读repository gate与人工diff终审。
- 2026-08-28：multi-FEC repository wrapper前两版分别在JavaScript模板插值与zsh整段预解析阶段失败，均未执行shell命令且零副作用。第三版改用`sed`/`awk`拆分tuple后在`/private/tmp/LuneX-video-multifec-repository.bOd8sm`输出`FINAL_VIDEO_MULTIFEC_REPOSITORY_GATE_OK`：三方baseline `9d4cabe`、精确5文件scope、stable generator、OpenSpec `11/11`与`7/27 next 2.3 pending`、focused `6/6`、related `229/228/1/0`、normal `1339/1337/2/0`、universal/双macOS `x86_64 arm64`、五build/Metal、fallback `0700/0600`及零privacy/remote-cancel/opt-in/process/coverage drift全部通过。下一步人工diff终审后提交推送，不勾选2.3。
- 2026-08-28：人工上游终审发现Sunshine短block还会为满足`minRequiredFecPackets`提高该block实际FEC percentage，因此最终回归扩展为block 0=`2 data/1 parity/50%`、先到的tail block=`1 data/2 parity/200%`，直接覆盖三个per-block字段同时变化。fresh final focused `/private/tmp/LuneX-video-multifec-final-focused.8Lh5cK`为`6/6/0/0`，related `/private/tmp/LuneX-video-multifec-final-related.Ya91rG`为`229/228/1/0`，normal `/private/tmp/LuneX-video-multifec-final-normal.5Gk9xz`为`1339/1337/2/0`，三份structured diagnostics全零；两个normal skip精确为live Sunshine与real Keychain。
- 2026-08-28：fresh final universal `/private/tmp/LuneX-video-multifec-final-universal.7KH3au`为`succeeded/0/0/0`且test bundle是`x86_64 arm64`。人工production终审确认每帧最多4个metadata entry、同block drift仍fail closed、FEC/non-FEC路径隔离、乱序首尾block按block/shard排序、packet/access-unit/age容量不变、duplicate与generation/teardown语义未扩大；五产品build因该终审后仅test/planning变化而继续绑定相同production源码。下一步最终repository gate、提交推送和新SHA唯一live。
- 2026-08-28：首个final repository gate因使用zsh特殊变量`path`而令后续`awk`不可见，在部分只读断言后退出；改名`result_path`并从头重跑后，`/private/tmp/LuneX-video-multifec-final-repository.PnUTPG`输出`FINAL_VIDEO_MULTIFEC_FINAL_REPOSITORY_GATE_OK`。最终门确认精确5文件、OpenSpec `11/11`与`7/27 next 2.3 pending`、fresh final `6/6`、`229/228/1/0`、`1339/1337/2/0`、fresh universal、retained五产品build/Metal、fallback `0700/0600`和零privacy/remote-cancel/opt-in/process/coverage drift。下一步post-record只读门后提交推送。
- 2026-08-28：恢复后重新核对clean pushed `8ca1d71`、OpenSpec `7/27 ready`及三份planning authority；2.3仍pending且旧SHA不重跑。已在live harness中接入test-only production video processor wrapper：同一`StreamVideoPresentationSource`、真实`NativeSessionVideoProcessorFactory`、VideoToolbox与音频processor保持不变，影子normalized assembler只输出饱和packet/first/last/access-unit/loss/discard/submission计数，原事件无修改转发。fresh隐私focused `/private/tmp/LuneX-video-processing-focused.fj1efm/Focused.xcresult`通过`1/1`，普通live/Keychain opt-in显式unset且未操作Simulator；下一步补wrapper转发回归并跑fresh相关门。
- 2026-08-28：首轮双focused `/private/tmp/LuneX-video-processing-focused-r2.YHs222/Focused.xcresult`在执行前以Swift 6编译错误停止，唯一诊断为新测试在`XCTAssertTrue` autoclosure内直接`try await processor.consume`；production与wrapper源码无诊断。已将异步结果先绑定局部变量，下一轮使用fresh目录，不复用失败bundle。
- 2026-08-28：修正测试异步断言后的fresh focused `/private/tmp/LuneX-video-processing-focused-r3.EXvh02/Focused.xcresult`通过`2/2/0/0`，structured build为`succeeded/0 errors/0 warnings/0 analyzer warnings`。fresh完整`AppModelWorkflowTests` `/private/tmp/LuneX-video-processing-related.dByINL/Related.xcresult`通过`105 total / 104 passed / 1 live opt-in skip / 0 failed`且structured diagnostics全零；证明显式production media environment与原AppModel工作流兼容，普通测试未访问live host、Keychain或Simulator。OpenSpec strict仍为`11/11`、apply `7/27 next 2.3 pending`；下一步fresh完整macOS normal与universal test build。
- 2026-08-28：fresh完整macOS normal `/private/tmp/LuneX-video-processing-normal.AEmdMH/Normal.xcresult`通过`1341 total / 1339 passed / 2 skipped / 0 failed / 0 expected failures`，structured build为`succeeded/0/0/0`；两个skip精确为live Sunshine与real Keychain显式gate，普通测试继续Debug文件identity fallback。fresh universal test build `/private/tmp/LuneX-video-processing-universal.iQVxId/Universal.xcresult`为`succeeded/0/0/0`且test bundle精确`x86_64 arm64`；零xcodebuild/xctest残留，未操作Simulator。因本批仅修改test/spec/planning且production源码与project membership未变，既有`8ca1d71`五产品build继续证明相同production tree的兼容性；下一步generator/OpenSpec/repository gate、提交推送后新SHA唯一live。
- 2026-08-28：generator双跑前/后`project.pbxproj` hash均为`783a7494...5944`且零drift。首轮repository gate已通过源码scope、OpenSpec、全部xcresult、universal和retained production build断言，但在fallback权限检查误用旧拼写`client-identity-debug.json`后退出；实现中的实际文件名为`client_identity.debug.json`，只读确认目录/文件仍为`0700/0600`。该错误无仓库、identity、Keychain、host或runtime副作用；修正精确路径后从头重跑完整gate。
- 2026-08-28：第二轮repository gate使用`ps ... | rg 'xcodebuild|xctest'`检查残留时命中了包含该正则文本的gate shell自身并静默退出；前置断言无失败、实际`pgrep -x xcodebuild/xctest`均为空。改用精确进程名并加入阶段回执后再次从头重跑，不重复test/build/generator/live。
- 2026-08-28：第三轮repository gate输出`scope_ok`与`openspec_ok`后，在大体积tests JSON的`printf | rg -q`精确skip检查处因`pipefail`把`rg`提前成功退出导致的上游SIGPIPE视为失败；xcresult count与内容本身无误。改为here-string完整输入检查后从头重跑只读gate；这是wrapper脚本错误，无产品或环境副作用。
- 2026-08-28：第四轮gate仍在`openspec_ok`后退出，定位为嵌套命令字符串把skip匹配模式错误转义成包含单引号的literal，导致`rg -c`无匹配；直接对落盘tests JSON使用`rg`确认精确2个skip和两个预期名称。最终gate改为证据目录内临时JSON文件，避免大变量、SIGPIPE和嵌套引号后重跑。
- 2026-08-28：第五轮虽已落盘tests JSON，但同一手工转义模式仍被带入`rg -c`而在`openspec_ok`后退出。停止使用文本引号匹配，改由`jq`递归按结构化`.result == "Skipped"`计数；两个测试名继续对文件直接匹配。所有失败均发生在只读汇总wrapper，不重复任何构建或live gate。
- 2026-08-28：最终repository gate `/private/tmp/LuneX-video-processing-repository-r6.*`输出`FINAL_VIDEO_PROCESSING_REPOSITORY_GATE_OK`：baseline/remote均为`8ca1d71`、精确5文件scope且production Sources/project零diff、generator hash稳定、OpenSpec strict `11/11`与apply `7/27 next 2.3 pending`、focused `2/2`、related `105/104/1/0`、normal `1341/1339/2/0`、universal `x86_64 arm64`、retained相同production tree五产品build、fallback `0700/0600`以及零privacy/remote-cancel/process/coverage drift全部通过。下一步post-record只读终审、提交推送并仅对新SHA执行一次double-opt-in live gate。
- 2026-08-28：恢复后执行 planning-with-files catch-up、读取 `prioritize-macos-product-completion` 全部 OpenSpec context 与三份 planning authority；确认 `HEAD == origin/main == 80055846be96b3f0ea7ddacf6a0cab5d6dbf52b7`、工作树 clean、OpenSpec `7/27 ready` 且 Task 2.3 pending。新 SHA 尚未构建或消费 live gate。
- 2026-08-28：按用户此前明确要求再次调用 `create_goal`，服务端仍因旧 `blocked` goal 被视作 unfinished 而拒绝；未产生仓库或外部状态副作用。本轮继续准备 `8005584` 的 fresh arm64 bundle、只读前后快照和唯一 double-opt-in live gate，不操作 Simulator/真实 Keychain，不修改或重启 Sunshine。
- 2026-08-28：从 detached clean exact SHA `80055846be96b3f0ea7ddacf6a0cab5d6dbf52b7` 在 `/private/tmp/LuneX-live-8005584.DARbz9` fresh 构建 arm64 `LuneXCoreTests.xctest`；structured result 为 `succeeded/0 errors/0 warnings/0 analyzer warnings`，bundle 为 arm64，`codesign --verify --deep --strict` 通过，可执行 SHA-256 为 `4e895979...d1a2b43`。所有 live/Keychain opt-in 在构建时显式 unset。
- 2026-08-28：首个 Sunshine 快照命令误用未配置的 `tanmy-white` alias，在本地 host-key verification 阶段以 255 退出且远端命令未执行；改用既有固定 Windows 用户和地址后只读快照成功。运行前四个 LuneX 文件为 `0600`，Sunshine PID `14640`、start `2026-08-27T04:21:28.2870310+08:00`、service `Running`，唯一 Event 1001 为该进程启动后 8 秒的既知 WER 条目。
- 2026-08-28：`8005584` 唯一 double-opt-in live gate 从 `19:19:10Z` 到 `19:19:57Z` 执行并 exit `1`，从此不得重跑。Desktop 走 `/resume`，`launch/resume/cancel=0/1/0`，control 到 `channels_1`/video metadata，audio `running` 且 `13494 datagrams / 8996 events`；video 为 `309380 datagrams / 251588 events`，shadow assembler 得到 `4628 accessUnits`、全部 loss/discard `0`，但 production `submitted=0`、published `frames=0`。
- 2026-08-28：live log SHA-256 为 `4c4ffdf192fff87279e3e98040a0d911f511ba6c4cb15b0d79134e7d57843266`。前后四文件 mode/size/hash与 Sunshine JSON 快照完全一致，零remote `/cancel`、零build/test残留，唯一coverage文件定向在本轮临时目录；Task 2.3保持pending。下一步增加有限 decode pipeline snapshot 并审计 awaiting-IDR/format/VideoToolbox submission，不重复本 SHA。
- 2026-08-28：续作恢复后重新读取 OpenSpec `prioritize-macos-product-completion` 全部 context、三份 planning authority 与未提交 diff；权威状态仍为 `7/27 ready`、next Task 2.3。旧 goal 控制面仍显示 blocked，但仓库推进未受阻；没有重跑 `8005584` live gate。
- 2026-08-28：审阅 exact binary coverage，确认 `NativeSessionVideoProcessor.consume()` 约 251k 次、`VideoDecodePipeline.consume()` 与 `MoonlightControlChannel.requestIDR()` 均为 0；根因是 UI 用 decoded `session.isStreaming` 驱动 lifecycle active，production processor 已因 `.inactive` 进入 transport drain。
- 2026-08-28：实现 active stream session lifecycle 投影、RootView 接线、test-only direct live foreground lifecycle 与有限 decode pipeline receipt；真实 occlusion/minimize/focus/drawable policy 未放宽，receipt 不记录敏感或高基数运行值。
- 2026-08-28：fresh focused `/private/tmp/LuneX-lifecycle-decode-focused.t0Vl6g/Focused.xcresult` 通过 `3/3`，0 skip/failure，structured build `succeeded/0/0/0`。下一步执行 fresh related 与完整 macOS normal、产品 compatibility、generator/OpenSpec/repository gate；通过后提交推送新 SHA，再执行一次且仅一次 double-opt-in live gate。
- 2026-08-28：fresh related `/private/tmp/LuneX-lifecycle-related.qhdiEV/Related.xcresult` 通过 `218/217/1 live skip/0`，0 expected failure，structured build `succeeded/0 errors/0 warnings/0 analyzer warnings`。Xcode 枚举锁定外接 iOS 设备时产生非结构化 notification-proxy 输出，但 destination 为本机 macOS，未解锁、安装或操作该设备。下一步 fresh serial macOS normal。
- 2026-08-28：fresh serial normal `/private/tmp/LuneX-lifecycle-normal.9dckfG/Normal.xcresult` 通过 `1341 total / 1339 passed / 2 exact skips / 0 failed / 0 expected failure`，structured build `succeeded/0/0/0`；skip 精确为 live Sunshine 与 real Keychain。三个真实 opt-in 均显式 unset，普通测试继续文件 fallback；下一步 universal test build 与五产品 generic compatibility build。
- 2026-08-28：fresh universal `/private/tmp/LuneX-lifecycle-universal.Eow0DM` build-for-testing 为 `succeeded/0/0/0`，test bundle `x86_64 arm64`。`/private/tmp/LuneX-lifecycle-builds.ansDKS` 五产品 generic build 全部 `succeeded/0/0/0`，macOS Debug/Release executable 均 `x86_64 arm64`；非 macOS 仅证明共享兼容，未启动 Simulator。下一步 generator/OpenSpec/repository gate。
- 2026-08-28：权威 generator 双跑前后 project hash 稳定为 `783a7494...5944`；OpenSpec strict `11/11`、apply `7/27 next 2.3 pending`。pre-commit gate 通过精确 10 文件 scope、diff/untracked/coverage、receipt privacy、普通 remote cancel、三个 opt-in、fallback `0700/0600`、零 process、三方基线与全部 retained evidence。Task 2.3 不勾选，下一步 post-record 复核后提交推送。
- 2026-08-28：续作恢复确认 `HEAD == origin/main == remote df76fa58b994f4f8ad3f67bd85bcfd7e5e012d22`、工作树clean、OpenSpec `7/27 next 2.3 pending`；候选 `/private/tmp/LuneX-live-df76fa5.ys7L6g` 的arm64 executable hash为`014bfbc3...e505a`且strict codesign通过。运行前三个真实opt-in均unset，四文件有限快照、Sunshine同PID/start/service/event只读快照完成。首次snapshot wrapper仅在最后展示时误用不存在的`/usr/bin/cat`而exit 1；快照与断言此前已完成，直接读取落盘证据，没有重复host查询。
- 2026-08-28：`df76fa5`唯一double-opt-in live gate已消费且不得重跑。direct XCTest原始exit `0`，1.334秒通过完整自动化路径：production session达到streaming，首个decoded frame与后续至少30帧增长、audio runtime `running`、相对指针`+1/-1`、input release、一次local stop、重复stop幂等及model session/audio/video teardown均通过。
- 2026-08-28：exact live coverage证明`NativeSessionVideoProcessor.consume()`为`2109`次、`VideoDecodePipeline.consume()`为`38`次、`MoonlightControlChannel.requestIDR()`为`1`次，直接反证旧SHA的decoder前inactive lifecycle早退。前后四文件mode/size/hash与Sunshine PID/start/service/event完全相同，Git clean、零xcodebuild/xctest残留、coverage仅在证据目录；live log SHA-256为`44068b9d...3a3c28`。解码生命周期死锁已闭环，但Task 2.3因物理可听同步、host可见输入、真实reconnect/remote termination、actual AppKit window/TCC及完整资源/重启矩阵仍保持pending。
- 2026-08-28：从真实 macOS SwiftUI 产品发现首窗口因 `.windowResizability(.contentSize)` 塌缩为 `220x104`。当前未提交修复在 `ProductWorkflowState.swift`、`LuneXApp.swift`、`ProductWorkflowSurfaceTests.swift` 中加入 `1280x800` 默认、`960x640` 最小 content policy、`.contentMinSize` 和聚焦回归；`/private/tmp/LuneX-window-sizing-focused/Focused.xcresult` 通过 `1/1` 且 structured diagnostics 全零。
- 2026-08-28：基于修改源码的单一 test-only ad-hoc App `/private/tmp/LuneX-window-sizing-product` 在 1 秒与 5 秒实际 AppKit frame 均为 `960x692`，UI 完整显示三台 saved host 和 cached apps。该 build 仅在命令行清空 entitlements，不能作为 head tracking、稳定签名/TCC、notarization 或 release 证据；source entitlement 未修改。
- 2026-08-28：恢复后完成 planning-with-files catch-up 并重读 OpenSpec 全部 context，确认 pushed baseline `ce4aa125`、工作树仅含三个窗口修复文件加三份 planning authority、OpenSpec `7/27 ready` 且 Task 2.3 pending。当前只有一个 App PID `67334` 和 log stream PID `67289`；继续从 AX 精确选择在线 `tanmy-white` 与 `Desktop`，选择复核和启动前快照通过前不触发产品 launch，也不启动 Simulator 或真实 Keychain gate。
- 2026-08-28：用户指出实例数异常后确认 Computer Use 以显示名解析并自动启动了 DerivedData 同名 App，新增 PID `70982`；原 test-only 产品仍为 PID `67334`。已立即 `SIGTERM 70982` 并确认只剩 PID `67334`。误实例上仅选择了 `tanmy-white`，未触发 Launch 或主机网络/session 动作。后续只允许 Computer Use 绑定 `/private/tmp/LuneX-window-sizing-product/DerivedData/Build/Products/Debug/LuneX-macOS.app` 绝对路径，且动作前后核对唯一 PID。
- 2026-08-28：检查用户提供的三张产品截图并追踪 `RootView.swift`、`AppModel.loadInitialState/loadHosts`、`MoonlightHost`、`HostLibraryManager` 和尚未接入的 `BonjourHostDiscoveryService`，确认问题属于信息架构、自动状态编排和状态语义缺陷，不是局部间距问题。
- 2026-08-28：恢复 planning-with-files session，重读 active OpenSpec change 全部 context，确认 `prioritize-macos-product-completion` 仍为 `spec-driven / 7 of 27 / ready` 且 Task 2.3 pending；未虚假勾选任务。把 macOS 单 Library 工作台、自动状态、临时全画布串流、原生 Settings 和按需 Diagnostics 记录为当前产品 gate 前置修复；尚未操作 Simulator、真实 Keychain 或重复 live launch。
- 2026-08-28：对运行中macOS产品完成完整UI/UX评审（逐页截图Library/Stream/Diagnostics/Settings），确认：屏幕类别侧栏与无会话纯黑Stream页、Library四面板仪表盘把一次性Pairing与Launch常驻化且主CTA低于默认窗口折叠线、Settings缺grouped form style导致列式布局溢出裁切并混入移动端状态行、host行"Unknown"常态文本与Paired双重表达、tile选中三重表达与容器focus ring、工具栏常驻禁用Disconnect。结论已契约化为新OpenSpec change `redesign-macos-library-shell`（新capability `macos-product-shell`：host侧栏、目录直启、上下文配对、会话独占窗口、原生Settings scene、按需Diagnostics、macOS控件习惯保真），`openspec validate --strict`通过；`task_plan.md` M4行已挂接该change。实现交由后续执行会话，本轮零产品源码修改。
- 2026-08-28：重新加载 Claude 新增的 `redesign-macos-library-shell` 全部 proposal/design/spec/tasks，并与用户关于自动刷新、导航频率、设置布局的反馈合并。该 change 现为 shell 实现权威：主机侧栏属于可折叠高频上下文而非屏幕菜单；自动 discovery/reachability/selected catalog 为正常路径；refresh 只作失败恢复；catalog 直启、上下文配对、会话独占窗口、native Settings scene 和独立 Diagnostics window 保持。按其明确 non-goal 撤回尚未使用的 artwork API，避免扩大本 change。
# 2026-08-28 macOS shell redesign resume

- Reloaded Claude's latest `redesign-macos-library-shell` proposal, design, capability spec, tasks, and apply instructions; authoritative progress remains `1/18` before implementation verification.
- Recovered the prior single macOS build session: it had already exited with code 65. Confirmed no `xcodebuild` process, no Simulator action, and exactly one existing `LuneX-macOS` PID (`83182`).
- The first fresh-build wrapper was rejected before process launch because it contained `rm -rf` for a dedicated DerivedData path. No build, source, App, host, Keychain, or Simulator side effect occurred; retry uses a new never-before-created DerivedData path without cleanup.
- The first actual fresh macOS build reached the new SwiftUI shell and failed on one compile error: `MacOSCatalogView.noticeTitle` called nonexistent `LocalizedStringResource.description`. Kept the value localized end-to-end by returning `LocalizedStringResource` directly instead of flattening it to `String`.
- Reused the same dedicated DerivedData path for a single incremental retry. Universal macOS Debug build succeeded (`arm64 + x86_64`); no Simulator, Keychain gate, live Sunshine gate, or additional LuneX process was started.
- First focused-test command selected `LuneX-macOS`, whose scheme has no Test action, so `xcodebuild` exited 66 before building or running tests. Project inventory confirms the correct independent unit-test scheme is `LuneXCoreTests`; retry switches schemes and does not launch the App.
- Correct focused scheme ran 28 `ProductWorkflowSurfaceTests`: 26 passed and only two old source-contract tests failed because they still required the retired macOS 2x2 dashboard/sidebar/ambient pairing/Launch panel. Rewrote those contracts against the new split macOS files while retaining the frozen-platform adaptive dashboard assertions; cancelled pairing now renders the ordinary pairing-ready state instead of an ambient cancelled panel.
- Added direct resolver coverage for first-use, contextual pairing, catalog, and stream ownership, including the session-owner precedence case where no host is currently selected.
- Added deterministic automatic reachability coverage: later-address success, all-address failure, trust/pairing/capability preservation, concurrent Bonjour merge rebase, repository persistence, and cancellation without false offline persistence.
- Focused shell/reachability gate passed `41/41` with warnings as errors. Audit then found native-display resolution was omitted when its pixel size equaled a named preset; replaced size-only picker identity with typed `preset/native/custom` choices, added arbitrary custom resolution/frame-rate round-trip tests, and asserted that only the owning workspace resolves to stream during a real two-window controlled session.
- Settings/multiwindow focused gate passed `30/30`; the macOS App then rebuilt successfully as universal Debug with warnings as errors. Updated `redesign-macos-library-shell` only for independently verified implementation/test tasks; authority is now `14/18`, leaving focus traversal, full matrix, Computer Use acceptance, and final strict/planning sync pending.
- 2026-08-28：再次逐行审阅 Claude 的 `redesign-macos-library-shell` proposal/design/spec/tasks，并与用户的高频任务反馈合并：新增“保留显式主机选择，否则优先 online+paired”的确定性选择契约，避免首开停在离线字母序主机而在线主机存在。
- 2026-08-28：实现 macOS-only provisional/explicit selection 协调和纯排序策略，冻结平台仍沿用原 reconcile 顺序；新增 AppModel 测试证明首次 reachability 会自动提升到在线已配对主机，而用户显式选择离线主机后后台刷新不会抢走选择。
- 2026-08-28：fresh warnings-as-errors focused shell+discovery gate `/tmp/LuneX-Redesign-Selection-C3AA8E44-4D28-470D-8141-4D3717E65CBA` 通过；真实 Keychain/live opt-in 均 unset，未启动或操作 Simulator。锁定外接设备的 notification-proxy 输出仍是非目标枚举噪声，实际 destination 为 macOS。
- 2026-08-28：第一次完整 macOS warnings-as-errors test 发现两项回归并判定本轮不通过：旧 diagnostics source-contract 仍寻找已为独立 window 提升为模块内可见的 `private struct DiagnosticsView`；恢复出的 secondary workspace host selection 未被 provisional/explicit coordinator 识别，trust reset 后被自动策略改选。保留测试覆盖，分别更新合法复用边界并把新发现的恢复选择登记为显式所有权后，先跑两项隔离复核再重跑完整测试。
- 2026-08-28：一次 planning 三文件同步 patch 因误把只存在于 `progress.md` 的英文尾行同时作为 `findings.md` 上下文而在写入前验证失败；没有文件被该次 patch 修改，随后按各文件真实尾部重新应用并成功，未重复产品或外部操作。
- 2026-08-28：恢复既有 macOS universal Release build session `82545`，未重复启动第二个构建；该构建最终 `BUILD SUCCEEDED`。`lipo -info` 与 `file` 均确认 Release executable 精确包含 `x86_64 arm64`。OpenSpec apply 仍为 `14/18 ready`，下一步运行 iOS、tvOS、visionOS generic compatibility builds；不启动 Simulator，不推进冻结平台产品状态。
- 2026-08-28：iOS/iPadOS、tvOS、visionOS 三个 warnings-as-errors generic Debug build 分别使用独立 DerivedData，并均在日志中以 `BUILD SUCCEEDED` 结束。汇总 wrapper 在构建完成后因误用 zsh 只读变量名 `status` 返回 1；直接核验日志结论后确认产品构建本身均成功，没有重复构建。最终 `pgrep` 无 `xcodebuild/xctest`，`simctl` 无 booted device。结合完整 macOS `1350 total / 1348 passed / 2 explicit opt-in skips / 0 failed`，勾选 `redesign-macos-library-shell` Task 7.2；冻结平台仍只获得共享代码兼容性证据。
- 2026-08-28：`script/build_and_run.sh --verify` 精确终止旧 PID `83182`、构建当前源码并仅启动 PID `3042`；三个真实 opt-in 均 unset。Computer Use 绑定绝对 `.app` 路径验收：首屏自动选中在线已配对主机并直接显示应用目录；离线灰、在线绿、待配对仅 lock；无 Unknown/Paired/Refresh/blank Stream/disabled Disconnect；Cmd+, 打开 grouped Settings 且 Stream 设置无裁切。Window 菜单发现两个重复 Diagnostics，侧栏同时发现已配对与未配对的同名 `tanmy-white`；两项判定为不通过并进入修复，未触发 Pair 或 live session。
- 2026-08-28：修复并实机复验 diagnostics 原生 scene 单一菜单注册、Bonjour 唯一名称合并/旧重复折叠/mDNS 来源、启动 checking 投影、可达地址首位提升和 catalog owner 驱动自动刷新。Computer Use 最终单实例 PID `8740`：三条无歧义 host 行、在线已配对 `tanmy-white` 自动选中、当前目录三张 tile 直接可启动、单一 Diagnostics、grouped Settings、可折叠 sidebar；完整键盘导航临时打开时 Tab 仅经过 Add Host/toolbar/app buttons，系统 `AppleKeyboardUIMode` 已从原值 0 临时设 2 后恢复 0。未 Pair、未启动 live session、未启动 Simulator。Task 2.2/4.2/7.3 勾选；因最终共享源码已变化，Task 7.2 重新置 pending 等待 fresh 全矩阵。
- 2026-08-28：重新读取 Claude 最新 `redesign-macos-library-shell` 全部 OpenSpec artifact 与 apply instructions，确认权威进度为 `16/18`，剩余 Task 7.2/7.4；用户关于高频主机/目录工作流、自动刷新、状态无歧义、低频 Settings/Diagnostics 不常驻的意见均已有明确 requirement/scenario，没有退回旧屏幕类别导航。
- 2026-08-28：最终完整测试的三项 destructive failure 来自旧断言仍把启动内存 reachability 期待为持久 `.online`。仅更新测试期望为完整 host 的 `.unknown` checking 投影，同时保留 repository 原值、ID/name/address/pairing/pinned trust 的严格比较；不撤销新启动语义。
- 2026-08-28：fresh focused `/tmp/LuneX-Redesign-Focused-CD0BA473-4745-46C2-94DE-9BF6EBAFC90A.xcresult` 通过 `4/4`：三项 destructive workspace 与 vision resize 全部通过，warnings-as-errors、三个真实 opt-in unset、无 Simulator 操作。vision 全量超时未在隔离复现，因此不删测、不放宽 wait；下一步运行最终源码完整 macOS tests，若再次失败再按共享时序回归处理。
- 2026-08-28：下一轮 full suite 暴露真实 stop race：teardown 已开始后，排队的 `.streaming` snapshot 可把公开 phase 从 `.stopping` 改回 `.streaming`。`applySessionSnapshot()` 现拒绝当前 stop owner 的迟到 snapshot；stop/replacement targeted matrix `5/5` 通过。
- 2026-08-28：再一轮 full suite 的唯一失败是 Vision resize 测试把 action history 整体 `last` 等同于 scene，未考虑合法后续 `.input`。断言改为查找最后一个 `.scene`；曾尝试的 2 秒 sleep 公共等待器会错过瞬时 action，已完整撤回。最终 targeted matrix `/tmp/LuneX-Redesign-TargetedFinal-E1D87665-7870-47A0-9563-C0A9A29AF691.xcresult` 通过 `5/5`。
- 2026-08-28：最终源码 fresh macOS full `/tmp/LuneX-Redesign-FinalTests-E78AD5A3-4C27-428C-BB68-984BC28FFA92/Full.xcresult` 结构化通过 `1355 total / 1353 passed / 2 exact opt-in skips / 0 failed / 0 expected failure`，build diagnostics `0/0/0`。两个 skip 精确为 live Sunshine 与 real Keychain；三个 opt-in 均 unset，普通 Debug identity 继续使用文件 fallback。xcresult 读取严格串行，避免已观察过的 Xcode 26.4 `database.sqlite3` 竞争。
- 2026-08-28：fresh macOS universal Release `/tmp/LuneX-Redesign-FinalRelease-74913118-7E91-425A-86AC-14E8A1A9DEF8` `BUILD SUCCEEDED`，`lipo`/`file` 均确认 `x86_64 arm64`；iOS/iPadOS、tvOS、visionOS 独立 generic Debug build `/tmp/LuneX-Redesign-FinalGeneric-82150B74-70A9-4C69-B8CC-B0507C5065DE` 全部 `BUILD SUCCEEDED / 0 structured diagnostics`。未启动或查询 Simulator，generic 证据不计其他平台产品进度。
- 2026-08-28：Task 7.2 与 7.4 均已勾选，`redesign-macos-library-shell` 最终为 `18/18 / all_done`。权威 Ruby generator 双跑前/后 project SHA-256 均为 `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`；最终 strict OpenSpec、`git diff --check`、零 `xcodebuild/xctest`、零 booted Simulator 与三个 opt-in unset 全部通过，只保留验收期间既有的单一 LuneX PID `8740`。一次只读历史检索包含不存在的 `README.md`，仅产生 `rg` 路径提示，零文件/runtime 副作用。
- 2026-08-28：用户人工双击 `Desktop` 已确认真实连接手势有效；Computer Use AX `click_count: 2` 未触发 SwiftUI 双击 gesture，先前“连接代码失效”的判断已撤回，未修改产品源码。随后执行标准 OpenSpec 归档：delta spec 已同步到 `openspec/specs/macos-product-shell/spec.md`，change 已移至 `openspec/changes/archive/2026-08-28-redesign-macos-library-shell/`，归档后全仓 strict `12/12` 通过。下一步立即完成最终 Git 审计、commit 和 push。
- 2026-08-28：恢复 `prioritize-macos-product-completion`（`7/27 ready`，当前仍为 Task 2.3）并确认 Git `main`/`origin/main` 同为 `3056a9c`、工作树 clean。最新 screenshot/Diagnostics 对应真实 `Waiting for Audio` 失败；当前只保留 PID `8740` 的一个 LuneX 实例，未操作 Sunshine、Keychain 或 Simulator。
- 2026-08-28：完成启动竞态源码审阅：media consumer 安装晚于 spatial/lifecycle startup，允许原始 `.streamEnded(.audio)` 被二次 `.inactiveSession` 的 `media_session_state_invalid` 遮蔽。本批将增加确定性 blocked-lifecycle terminal-ownership 回归，保留 `testLifecycleEffectFailureFailsSessionAndCleansInputGeneration` 的既有致命语义；通过 focused/related/full gates 后立即 commit/push，再精确替换唯一 App 实例做 live 复验。
- 2026-08-28：首轮 focused `/tmp/LuneX-AudioTerminalOwnership-4D941C18-90B8-42AF-A78C-25DD8227ED29/Focused.xcresult` 为 `2 total / 1 passed / 1 failed`：既有真实 lifecycle effect failure 回归通过；新竞态回归的最终 audio actionable code 已是 `audio_stream_ended`，但历史仍含一条 `media_session_state_invalid`，失败精确在 `AppModelWorkflowTests.swift:3118`。当前先增加有限 code 序列断言消息并复用已编译 DerivedData 单测定位，不重复首轮 bundle；外接锁定设备 notification-proxy 输出为非目标枚举噪声。
- 2026-08-28：diagnostic R2 精确输出 `media_session_state_invalid,audio_stream_ended`，定位前者为 audio terminal cleanup 的 macOS input release barrier 二次 `.inactiveSession`，不是新的媒体根因。产品 catch 现核对 environment snapshot ownership：generation 仍 active 的真实 input send/release failure 继续记录，已终止 generation 的二次 cleanup error 不再污染原始 terminal diagnostic。
- 2026-08-28：focused R3 在测试执行前因 Swift 不接受 `guard !await ...` 语法而编译失败；正确形式为 `guard !(await ...)`。失败 bundle 保留为编译证据，0 tests 执行，未访问 host/Keychain/Simulator/runtime；只修正括号后从新 result bundle 重跑同一门。
- 2026-08-28：focused R4 结构化通过 `2/2`，related 结构化通过 `91/91`；后者覆盖完整 SessionMediaEnvironment、NativeSessionAudioProcessor、AudioRuntimeRecovery 及 AppModel media/input/lifecycle failure 矩阵。fresh full `/tmp/LuneX-AudioTerminalOwnership-Full-425B6C7F-E4AC-40BF-982B-F96CA4629BE1/Full.xcresult` 通过 `1356 total / 1354 passed / 2 exact opt-in skips / 0 failed / 0 expected failures`，structured build `succeeded/0 errors/0 warnings/0 analyzer warnings`；skip 精确为 live Sunshine 与 real Keychain，三个 opt-in 均 unset。
- 2026-08-28：full 后只读 generator 探索命令包含 zsh 未匹配 glob `Makefile*`，在执行 generator 前以 `no matches found` 退出，零仓库/runtime副作用；随后用 `rg --files` 精确定位权威 `Tools/generate_xcodeproj.rb`，不重复 full tests。
- 2026-08-28：权威 Ruby generator 双跑前/后 `project.pbxproj` SHA-256 均为 `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`，零工程 drift；OpenSpec strict `12/12`，`prioritize-macos-product-completion` 仍为 `7/27 ready`、Task 2.3 pending。
- 2026-08-28：共享兼容性 generic Debug builds `/tmp/LuneX-AudioTerminalOwnership-Generic-7B0A0D48-AFAC-4956-8519-BD1418DAE1DD` 的 iOS/iPadOS、tvOS、visionOS 三项均 `BUILD SUCCEEDED`，未启动或创建 Simulator；这些仅证明 AppModel 共享改动编译兼容，不推进冻结平台产品状态。下一步最终只读 repository gate 后立即 commit/push。
- 2026-08-28：从 clean pushed `3af5de0` 的唯一 App PID `34308` 用 Computer Use 双击 `Desktop`，稳定复现 `Connecting: Waiting for Audio`；未启动第二实例、Simulator、Keychain gate 或修改 Sunshine。`tcpdump` 只读取现有流量：video `47998 -> local 63061` 持续 1408-byte datagram，control `47999 <-> local 51228` 正常，audio `local 51110 -> 48000` 每 500 ms 发送 20-byte custom ping但静音期零返回 RTP。
- 2026-08-28：产品错误地把第一帧 decoded audio 作为 readiness；已改为接受同 generation 的 `.running` audio runtime event 后发布 `[audio,input]` readiness，同时保留首包处理与所有 terminal failure。新增确定性无音频包 readiness 断言，准备 fresh focused gate。第一次跨五文件原子 patch 因 planning 锚点不匹配在写入前失败，随后 source/test 与三份 planning authority 均已按真实上下文应用。
- 2026-08-28：focused R1 `/tmp/LuneX-SilentAudioReady-Focused-20260828/Focused.xcresult` 在测试执行前因两处 `XCTAssertEqual` autoclosure 直接调用 async iterator 而编译失败；产品源码无诊断。已改为先 `await` 到局部变量再断言，将用 fresh R2 bundle 重跑，R1 不计通过。
- 2026-08-28：focused R2 `/tmp/LuneX-SilentAudioReady-Focused-R2-20260828/Focused.xcresult` warnings-as-errors 通过 `2/2`；两个 case 分别覆盖零 receive packet 的 `.running` runtime readiness 与倒退 sequence/graph generation 拒绝。三个真实 opt-in unset，未访问 host、Keychain 或 Simulator。
- 2026-08-28：related `/tmp/LuneX-SilentAudioReady-Focused-R2-20260828/Related.xcresult` 结构化通过 `192 total / 191 passed / 1 skipped / 0 failed`，覆盖完整 media environment、AppModel workflow、native audio processor 与 audio recovery；唯一 skip 精确为 `testLiveTanmyWhiteProductionAcceptanceWhenExplicitlyEnabled()`。三个真实 opt-in 均 unset，无 `xcodebuild/xctest` 残留，未操作 Keychain、host 或 Simulator。
- 2026-08-28：fresh full macOS `/tmp/LuneX-SilentAudioReady-Full-20260828-R1/Full.xcresult` 权威 summary 通过 `1356 total / 1354 passed / 2 skipped / 0 failed / 0 expected failures`；build result 为 `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`。skip 精确为 live Sunshine 与 real Keychain，三个 opt-in unset，完成后只剩唯一产品 PID `34308`。
- 2026-08-28：首轮 universal product build `/tmp/LuneX-SilentAudioReady-Universal-20260828-R1` 在源码编译前退出 65：generic destination 无 development team，带 entitlement 的 target 要求开发证书。未修改或清空 entitlement；后续 fresh R2 使用仓库 runtime script 同样的 `CODE_SIGNING_ALLOWED=NO` 仅作 compile/universal 兼容证据，不称为签名验收。
- 2026-08-28：权威 generator 连续两次运行，生成前/第一次/第二次 `project.pbxproj` SHA-256 均为 `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`，零 project drift。
- 2026-08-28：fresh unsigned compile-only universal R2 `/tmp/LuneX-SilentAudioReady-Universal-20260828-R2` `BUILD SUCCEEDED`，`lipo`/`file` 确认 macOS executable 精确含 `x86_64 arm64`。随后 `/tmp/LuneX-SilentAudioReady-Generic-20260828-R1` 的 iOS/iPadOS、tvOS、visionOS generic Debug 串行通过且三份 xcresult 均为 `succeeded/0/0/0`；未启动或查询 Simulator，冻结平台仅记共享编译兼容。所有 build 结束后仍只有旧产品 PID `34308`。
- 2026-08-28：pre-commit repository safety gate 输出 `REPOSITORY_SAFETY_AUDIT_OK`：OpenSpec strict `12/12`，apply `7/27 next 2.3 pending`，HEAD/origin baseline 一致，精确 5 文件且无 untracked，diff privacy/remote-cancel 清洁，Debug fallback 目录/identity 为 `0700/0600`，零 `xcodebuild/xctest` 且仅一个旧 App PID。进入立即 commit/push。
- 2026-08-28：silent-audio batch 已提交并推送为 `29a27d1`，本地/`origin/main` 同步且工作树 clean；`script/build_and_run.sh --verify` 精确替换 PID `34308`，只启动 PID `42117`。
- 2026-08-28：Computer Use 在唯一产品实例双击 Desktop，真实 UI 越过 `Waiting for Audio` 并保持连接，随后显示 `Waiting for Video`；Diagnostics 记录 transient `mac_lifecycle_drawable_unavailable` 后 `mac_lifecycle_active`。关闭 Diagnostics、恢复焦点并等待 8 秒仍无画面，已通过 UI 本地断开且未 remote cancel。
- 2026-08-28：首个 live xcodebuild bundle `/tmp/LuneX-SilentAudioReady-LiveVideo-29a27d1-R1/Live.xcresult` 因 Xcode runner 过滤两个 opt-in 而 `1 skipped`，0.003 秒内未触达 host。复用同一已签 bundle，以 direct `xcrun xctest` 和受限 `LLVM_PROFILE_FILE` 运行后 `1/1 passed in 2.226s`，真实 session 完成视频增长、audio running、input、本地 stop 与 clean teardown；产品窗口差异缩小到 transient drawable 后 IDR recovery。
- 2026-08-28：实现 lifecycle resume 后首个完整 predicted access unit 的单次 IDR 补请求；fresh focused `/tmp/LuneX-VideoResumeIDR-Focused-20260828-R1/Focused.xcresult` 通过 `1/1`，related `/tmp/LuneX-VideoResumeIDR-Related-20260828-R1/Related.xcresult` 通过 `173/172/1/0` 且 structured build `succeeded/0/0/0`，唯一 skip 为 live Sunshine opt-in。
- 2026-08-28：标准脚本把唯一产品实例替换为 PID `45402`；Computer Use 双击 Desktop，5 秒后截图已显示持续真实远程桌面，控制层明确 `Streaming`、`SDR`、`Fixed spatial`。不再出现 `Waiting for Audio`、自动断开或 `Waiting for Video`；控件已隐藏，实例保持运行供用户查看。
- 2026-08-29：读取完成后的 `/tmp/LuneX-VideoResumeIDR-Full-20260828-R1/Full.xcresult` 权威结果：`1356 total / 1354 passed / 2 skipped / 0 failed / 0 expected failures`，structured build `succeeded/0/0/0`；两个 skip 精确为 live Sunshine 与 real Keychain。当前唯一产品进程仍保留，不因离线门重复启动 App。
- 2026-08-29：审阅用户截图及 `StreamStatusOverlay`、AppModel runtime setting 边界；确定下一批 macOS 浮层改为应用/主机+stream profile，并把 pointer、Fit/Fill、HDR、spatial audio 变成真实会话控件。head-tracking entitlement 详情只在需要时呈现，未接入音频图的 mute 不做假开关。
- 2026-08-29：权威 Ruby generator 连续运行两次，生成前/第一次/第二次工程哈希一致且无 drift。`/tmp/LuneX-VideoResumeIDR-Compatibility-20260829-R1` 串行完成 macOS compile-only universal 与三个 generic shared-code builds；macOS executable 精确含 `x86_64 arm64`，四份 structured result 均为 `succeeded/0 errors/0 warnings/0 analyzer warnings`。未启动或查询 Simulator，结束后零 `xcodebuild/xctest` 且仍只有一个产品 PID `1564`。
- 2026-08-29：IDR batch repository gate 通过：OpenSpec strict `12/12`、主 change `7/27` 且 Task 2.3 pending；HEAD/origin baseline 同为 `29a27d1`，精确 5 个预期文件、diff hygiene clean、三个真实 opt-in unset、Debug fallback `0700/0600`、零测试进程和单一 LuneX PID `1564`。进入立即 commit/push。
- 2026-08-29：IDR batch 已提交并推送为 `cf7ebd4`，本地/`origin/main` 同步且工作树 clean；未替换现有 App。随后将用户截图意见写入主 OpenSpec design/spec/Task 5.1：macOS 活跃串流浮层是高频 action surface，不是只读 diagnostics banner；冻结平台 UI 不随本批推进。
- 2026-08-29：实现 macOS-only stream action overlay：app/host 标识、配置 profile、pointer segmented control、Fit/Fill segmented control、HDR/EDR toggle、spatial-audio toggle、icon-only hide 与 disconnect。正常 streaming 移除 standalone `Direct pointer / SDR / Fixed spatial` 和 general diagnostic/entitlement prose；连接态只保留 actionable message。新增 source-contract 测试保护真实 runtime/persistence 调用与禁止的只读 banner 内容。
- 2026-08-29：只读 formatter 检索因不存在的 `README*` zsh glob 在展开阶段退出，零文件/runtime 副作用；不重复该模式，直接进入 fresh focused Xcode gate。
- 2026-08-29：overlay focused `/tmp/LuneX-MacOverlay-Focused-20260829-R1/Focused.xcresult` 通过 `32/32` 且 structured build `succeeded/0/0/0`。related R1 执行 `228` 项，唯一失败是旧 HDR source contract 要求已退役的只读 overlay label；其余 `225` 通过、2 个 opt-in skip，所有 runtime 路径均通过。更新契约为可操作 HDR toggle 的明确 accessibility label/value，不撤回 UI 需求。
- 2026-08-29：更新后的 HDR source contract 隔离通过 `1/1`；related R2 `/tmp/LuneX-MacOverlay-Related-20260829-R2/Related.xcresult` 通过 `228/226/2/0` 且 structured build `succeeded/0/0/0`，两个 skip 精确为 live Sunshine 与 real Keychain opt-in。OpenSpec strict 再次通过 `12/12`，当前无 `xcodebuild/xctest`，进入 fresh full macOS gate。
- 2026-08-29：fresh overlay full `/tmp/LuneX-MacOverlay-Full-20260829-R1/Full.xcresult` 结构化通过 `1356 total / 1354 passed / 2 exact opt-in skips / 0 failed / 0 expected failures`，build `succeeded/0 errors/0 warnings/0 analyzer warnings`。两个 skip 不变，Task 5.1 尚未因离线结果勾选。
- 2026-08-29：overlay compatibility root `/tmp/LuneX-MacOverlay-Compatibility-20260829-R1` 的 macOS universal 与 iOS/iPadOS、tvOS、visionOS generic builds 全部成功且四份 structured diagnostics 全零；macOS executable 精确 `x86_64 arm64`。generator 双跑前/后 `project.pbxproj` hash 一致，零 drift；全程未启动或查询 Simulator。
- 2026-08-29：`script/build_and_run.sh --verify` 在无旧 App 的前提下只启动 PID `49417`。Computer Use 双击 Desktop、6 秒后持续真实视频，显示控件后可见 app/host/profile 与 Direct/Relative、Fit/Fill、HDR/EDR、Spatial audio；旧 pills 和 entitlement/general diagnostics 不再可见。临时 Fit->Fill 使画面立即填满且 AX 更新，恢复 Fit 后 AX 与 `settings.json` 均确认 `fit`，会话保持运行。
- 2026-08-29：一次只读截图 emit 复用未保留的 Node REPL `fs2` binding 而返回 `is not defined`；零 UI/设置副作用。重新 import 新 binding 后成功截图，未启动第二 App 或 session。
- 2026-08-29：用户判定第一版 panel 位置仍不合格。定位为 bounded-height frame 内默认 center alignment；已改为 wide panel 内外双重 `.top` 对齐、整体 top-center、最大宽度 `760pt`，隐藏入口同步 top-center，compact 保留 bottom。同步更新 OpenSpec 与 source contract，错误位置版本尚未 commit/push。
- 2026-08-29：position focused R1 执行 `32` 项，唯一失败为新增 source contract 过宽禁止整个 stream workspace 的所有 `.topLeading`，产品源码已编译。收窄为只禁止旧 compact/wide overlay alignment 表达式，未改变实现或放宽目标行为。
- 2026-08-29：position contract R2 隔离通过 `1/1` 且 structured diagnostics 全零。标准脚本精确替换唯一 App；Computer Use 双击 Desktop、显示控件后的真实 screenshot 证明 760pt panel 正确位于 titlebar 下方 top-center，远程头像/用户名/密码框可见，控件完整且无重叠。保持单一 live session，进入 exact-source full gate。
- 2026-08-29：最终定位源码 fresh full `/tmp/LuneX-MacOverlay-Position-Full-20260829-R1/Full.xcresult` 结构化通过 `1356/1354/2/0`、0 expected failures，build `succeeded/0/0/0`；精确 skip 仍只有 live Sunshine 与 real Keychain opt-in。唯一产品 live session 保持运行。
- 2026-08-29：最终定位源码 compatibility root `/tmp/LuneX-MacOverlay-Position-Compatibility-20260829-R1` 的 macOS universal、iOS/iPadOS generic、tvOS generic、visionOS generic 四项均 `BUILD SUCCEEDED`；macOS executable 精确含 `x86_64 arm64`。没有启动、创建、clone 或查询 Simulator。
- 2026-08-29：overlay 最终 repository gate 通过：`git diff --check`、精确 9 个预期 tracked 文件且零 untracked、OpenSpec strict `12/12`、Debug fallback `0700/0600/0600`、`LUNEX_RUN_KEYCHAIN_TEST`/`LUNEX_RUN_LIVE_HOST_TEST`/`LUNEX_RUN_LIVE_DESKTOP_SESSION` unset、零 `xcodebuild/xctest`，且只有当前 live LuneX PID `51200`。`HEAD` 与 `origin/main` 提交前基线同为 `cf7ebd4`，进入立即 commit/push，不重启会话。
- 2026-08-29：用户允许 Spatial audio 控件下方增加错误小字并询问 macOS 头部跟踪权限。Apple 官方 entitlement 文档与本机 Xcode 26.4 SDK 共同确认：`listenerHeadTrackingEnabled` 为 macOS 15+ API，启用时需要 Head Pose capability 对应的 `com.apple.developer.coremotion.head-pose`；这不是 TCC 弹窗，也不同于只访问个性化空间音频 profile 的 `com.apple.developer.spatial-audio.profile-access`。当前 PID `51200` 为无 entitlement 的 adhoc linker-signed Debug App，settings 请求 spatial/head tracking，因此 fixed-spatial fallback 是真实状态。
- 2026-08-29：实现 macOS Spatial audio subordinate issue：开关开启且 runtime 存在 fallback/failure 时才在其下方显示 `caption2`、orange、可换行且有独立 accessibility label 的有限原因；缺 Head Pose entitlement 明确说明本 build 仅为 fixed spatial，健康、用户关闭或无失败状态不占用布局。同步 OpenSpec design/spec 与 source contract，Task 5.1 仍保持 pending。
- 2026-08-29：fresh focused `/tmp/LuneX-SpatialIssue-Focused-20260829-R1/Focused.xcresult` warnings-as-errors 通过 `1/1`，精确覆盖 macOS Library/stream overlay source contract；产品 SwiftUI 编译成功，未启用三个真实 opt-in、未启动第二 App 或操作 Simulator，现有 PID `51200` 保持运行。
- 2026-08-29：related `/tmp/LuneX-SpatialIssue-Related-20260829-R1/Related.xcresult` 结构化通过 `281 total / 280 passed / 1 exact live-host skip / 0 failed / 0 expected failures`，build `succeeded/0 errors/0 warnings/0 analyzer warnings`。矩阵覆盖 ProductWorkflow、SpatialAudio presentation、Head Pose reader、spatial runtime、audio pipeline/integration/recovery、native processor、media environment 与 AppModel；三个 opt-in unset，唯一产品 PID 仍为 `51200`。
# 2026-08-29 Spatial-audio inline fallback full-gate failure triage

- Fresh full result `/tmp/LuneX-SpatialIssue-Full-20260829-R1/Full.xcresult`: `1356 total / 1353 passed / 2 skipped / 1 failed / 0 expected failures`.
- Structured build result: `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`.
- Sole failure: process crash at `MacStreamInputCaptureViewTests.testViewIsFlippedAndAcceptsFirstResponder()` with `libsystem_malloc.dylib: BUG IN CLIENT OF LIBMALLOC: memory corruption of free block`; this is not an assertion and does not exercise the changed `RootView` control.
- Next gate is a fresh, non-parallel focused run for this AppKit/Metal test class; do not repeat the full command unchanged.
- The first focused command was rejected before process creation because it included `rm -rf` for temporary evidence cleanup. No test ran and no state changed; the replacement uses a new unique directory without deletion.
- Fresh serial focused evidence `/tmp/LuneX-SpatialIssue-CrashFocused-20260829-R2/Focused.xcresult` passed the exact crashed test `1/1`; the test itself is stable when isolated. Proceed with one fresh full suite using `-parallel-testing-enabled NO` to remove cross-worker AppKit/Metal lifetime interference.
- Fresh serial full evidence `/tmp/LuneX-SpatialIssue-FullSerial-20260829-R2/Full.xcresult` passed `1356 total / 1354 passed / 2 skipped / 0 failed / 0 expected failures`; structured build diagnostics are `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`.
- The exact skips remain `testLiveTanmyWhiteProductionAcceptanceWhenExplicitlyEnabled()` and `testRealKeychainIdentityRoundTripWhenExplicitlyEnabled()`; live Sunshine, real Keychain, and live Desktop opt-ins were unset, so normal Debug identity coverage continued through the file fallback.
- Three concurrent `xcresulttool` readers caused only the skip-name reader to fail with a temporary `database.sqlite3` name collision inside the result bundle. Summary/build reads were already complete; a standalone serial read then confirmed both exact skip names, with no test rerun.
- Authoritative generator ran twice; `project.pbxproj` SHA-256 stayed `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf` before, after the first run, and after the second run.
- Compatibility root `/tmp/LuneX-SpatialIssue-Compatibility-20260829-R1`: macOS universal, iOS/iPadOS generic, tvOS generic, and visionOS generic builds all returned structured `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`; the macOS executable is exactly `x86_64 arm64`.
- Non-macOS builds are shared-source compatibility evidence only. No Simulator was started, created, cloned, queried, or otherwise operated.
- The first product replacement command used nonexistent `scripts/build_and_run.sh` and exited `127` before running any script, build, stop, or launch action. Correct path is `script/build_and_run.sh`.
- Computer Use sent Return to the selected `Desktop` item, then failed only while emitting the resulting screenshot because the persistent REPL no longer exposed `fs`. The launch action is not repeated; the next call re-reads state and imports screenshot dependencies locally.
- Computer Use with an actual coordinate double-click started `Desktop` in the one exact-path App instance. Showing controls proved the new orange caption is correctly placed below the spatial-audio switch, but also exposed the old generic connecting footer repeating the same missing-entitlement diagnostic.
- Updated the macOS footer to suppress a structured `spatial_audio_` actionable event only while `macOSSpatialAudioIssue` is already presenting it inline. Other stream actionable messages remain unchanged. Added source-contract coverage and tightened OpenSpec/design against duplicate spatial reasons.
# 2026-08-29 macOS runtime continuity batch resumed

- Recovered planning-with-files state and re-read all `prioritize-macos-product-completion` OpenSpec context. Current progress remains `7/27`; Tasks 2.3, 3.1, 4.1, and 5.1 remain pending.
- Accepted two new live observations as P0 evidence: audio latency accumulates well beyond Moonlight-Qt, and a background/return transition can show Library while audio continues.
- Next implementation gate: replace view-disappearance-driven workspace destruction with actual window-close semantics, add deterministic background/return ownership coverage, and implement bounded audio catch-up based on total pending age/duration.
- Focused R1 failed before test execution because four new XCTest assertions contained async calls inside autoclosures. The product sources were not behaviorally exercised; the test now awaits each result before assertion and R1 remains preserved as failed evidence.
- Focused R2 also stopped at test compilation: a new `Task` closure captured the non-Sendable XCTestCase through `makePCM`. The buffer is now created before spawning the task; no product/runtime test failed in R2.
- Focused R3 compiled the product implementation, then found the remaining two async XCTest autoclosures in the new provider test. A batch-wide search confirmed no other executable occurrences; both now assert previously awaited values.
- Focused R4 passed `88/88`. The first standalone app build then stopped before source compilation because the macOS Head Pose entitlement requires a development certificate; R2 will use the repository's ordinary unsigned Debug setting and does not claim signed/head-tracking acceptance.

# 2026-08-31 macOS realtime pointer input batch

- Recovered planning state and confirmed the prior unsigned `AppBuild-R2` succeeded with zero build errors, warnings, or analyzer warnings.
- Confirmed exactly one existing LuneX process (`29520`) and zero `xcodebuild`/`xctest`; did not launch another App or operate Simulator.
- Re-read OpenSpec `prioritize-macos-product-completion`: schema `spec-driven`, progress `7/27`; Tasks 2.3 and 3.2 remain pending and are the acceptance owners for this live input defect.
- Traced AppKit capture -> `AppModel` -> `MacSessionInputCoordinator` -> `MoonlightRemoteInputProvider` -> authenticated control sender. Identified a 256-sample strict FIFO before a downstream coalescer that cannot engage because the first queue awaits each physical send.
- Compared local Moonlight-Qt behavior: it drains all pending motion events together, sums relative deltas, and retains the newest absolute point before calling the common input send API.
- Next: implement bounded first-stage movement coalescing with state barriers, add deterministic backlog/click-order tests for Direct and Relative, then run focused and related gates before replacing the single product instance.
- Implemented first-stage realtime coalescing in `MacSessionInputCoordinator`: production outstanding bound is now 16; compatible queued Relative moves sum deltas, Direct moves retain the newest mapped point, and wire-invalid combinations remain separate. Added `coalescedSampleCount` so acceptance can distinguish merging from rejection/drop behavior.
- Added deterministic Direct/Relative backlog tests. With the physical sink blocked, 100 Relative samples occupy one queued event, preserve `(200,-100)`, and remain ahead of click down/up without rejection; Direct keeps the newest point on each side of a button barrier.
- Fresh serial warnings-as-errors focused result `/tmp/LuneX-MacInputRealtime-Focused-R1/Focused.xcresult` passed `19/19/0/0`; structured build result is `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`. No real opt-in, host, Keychain, Simulator, or second App operation occurred.
- Related R1 passed every executed AppModel/input/provider/wire test, then hung in the unrelated native-audio fixture because its engine stub never invokes a schedule completion after the production ceiling changed from eight to three buffers. After a bounded observation, interrupted and terminated the only `xcodebuild/xctest`; bundle `/tmp/LuneX-MacInputRealtime-Related-R1/Related.xcresult` is retained as interrupted evidence and is not counted as a pass or input failure.
- Audio fixture focused R1 also hung and was interrupted after a process sample confirmed an async schedule-capacity wait. The first test-only repair triggered a completion before jitter flush had scheduled any buffer; `.closed` later flushed four buffers into the three-buffer ceiling with no completion remaining. R1 is interrupted evidence and is not counted as passed.
- Updated the fixture to submit `.closed` concurrently, wait until the fake engine actually records the three-buffer ceiling, then complete the oldest scheduled buffer. Production audio behavior was unchanged. Fresh serial warnings-as-errors evidence `/tmp/LuneX-AudioFixtureCapacity-Focused-R2/Focused.xcresult` passed `1/1` in 0.015 seconds; structured build result is `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`.
- Fresh serial input-related evidence `/tmp/LuneX-MacInputRealtime-Related-R2/Related.xcresult` passed `242/243` with one expected live-host skip and zero failures. It covered the coordinator, AppKit capture, adapters, wire delivery, control/ENet, same-session generation replacement, and AppModel workflows; structured build result is `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`.
- Combined runtime/audio/input related evidence `/tmp/LuneX-RuntimeAudioInput-Related-R1/Related.xcresult` passed `426 total / 425 passed / 1 expected live skip / 0 failed`; production and fixture audio, media receive, workspace continuity, AppModel ownership, input capture/coordinator/provider/wire/control/ENet all passed together.
- Full serial R1 `/tmp/LuneX-RuntimeAudioInput-Full-R1/Full.xcresult` passed `1362 total / 1360 passed / 2 opt-in skips / 0 failed` before the final AppKit mouse-movement ownership addition.
- Added complete coordinate-snapshot barriers to first-stage motion coalescing and explicit AppKit `acceptsMouseMovedEvents` ownership. Focused R2 passed `56/56`, then review found a stale-view replacement hazard in per-view Boolean restoration; replaced it with a shared per-window owner set.
- Focused R3 stopped at strict Swift compilation because the nested ownership entry initializer read the main-actor-isolated `NSWindow.acceptsMouseMovedEvents` property from a nonisolated context. No tests ran. The main-actor acquire function now reads the value and passes a plain Bool into the entry; R3 remains failed evidence.
- Fresh focused R4 passed `56/56`, including two simultaneous surface owners, final-owner restoration, and coordinate revision barriers. Final serial full evidence `/tmp/LuneX-RuntimeAudioInput-Full-R2/Full.xcresult` passed `1364 total / 1362 passed / 2 opt-in skips / 0 failed`; structured diagnostics remain to be recorded before product replacement.
- Structured inspection of `/tmp/LuneX-RuntimeAudioInput-Full-R2/Full.xcresult` confirmed `1364 total / 1362 passed / 2 opt-in skips / 0 failed / 0 expected failures`; the build action is `succeeded / 0 errors / 0 warnings / 0 analyzer warnings` on the current macOS source.
- Compatibility root `/tmp/LuneX-RuntimeAudioInput-Compatibility-R1` is complete: macOS universal, iOS generic, tvOS generic, and visionOS generic builds all report `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`. The macOS executable is exactly `x86_64 arm64`; these non-macOS results are shared-source compatibility evidence only and do not advance frozen-platform product status.
- No Simulator was started, created, cloned, queried, or otherwise operated. `LUNEX_RUN_KEYCHAIN_TEST`, `LUNEX_RUN_LIVE_HOST_TEST`, and `LUNEX_RUN_LIVE_DESKTOP_SESSION` remained unset throughout the deterministic and compatibility gates.
- Repository pre-runtime gate passed: OpenSpec strict `12/12`, `git diff --check`, project hash `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`, and zero `xcodebuild`/`xctest` processes. Task 2.3 and 3.2 remain pending until actual host-visible Direct/Relative movement and click feedback are observed.
- `script/build_and_run.sh --verify` built the exact working source and launched one process only: PID `12579` at `/Users/tanmy/Projects/LuneX/build/DerivedData/Build/Products/Debug/LuneX-macOS.app`. No prior App existed, no second App was launched, and Computer Use targeted this exact bundle path.
- Computer Use double-clicked `Desktop`; sustained remote video appeared after the bounded startup interval. In Direct mode, a blank-area click immediately dismissed the remote Start menu, a continuous horizontal drag completed without a stale trail, and a second vertical-position click placed the host cursor at the requested location before the next screenshot.
- Switched through the real overlay to Relative. Host-visible cursor movement reached approximately `(900,500)`, then `(650,600)`, with each click completing before the next screenshot and no delayed catch-up. After raising Finder for ten seconds and returning, LuneX still showed the same live stream rather than Library; a post-return Relative click immediately moved the host cursor near `(800,600)`.
- Restored the pre-test Direct preference and hid controls. The exact stream remains running for user inspection; process count is one, `xcodebuild/xctest` count is zero, and the current unified-log error/fault query returned no entries. Audible latency/synchronization is deliberately not marked passed because Computer Use cannot hear the physical output.
- Tasks 2.3 and 3.2 remain pending: this spot-check does not cover their remaining keyboard, scroll, controller, cursor-release, resize/full-screen, multi-display, reconnect, remote-termination, repeated-stop, resource, or audible-sync rows.

# 2026-08-31 Moonlight-Qt latency comparison

- Recovered the active planning-with-files context and re-read the current macOS-first OpenSpec authority. `prioritize-macos-product-completion` remains artifact-complete while its implementation checklist remains `7/27`; no physical/live or performance acceptance item was advanced by this source audit.
- Recorded the confirmed input, video, and audio low-latency mechanisms from the local Moonlight-Qt/Common references. No LuneX product source, Sunshine state, Simulator, running App instance, or repository ref was changed.
- The remaining audit work is to finish the generic Qt pacer drop/VSYNC branches, inspect LuneX's VideoToolbox-to-Metal frame ownership and realtime audio policy values, and confirm AVFoundation completion/presentation-latency semantics from the installed SDK before proposing code changes.
- Completed those source/SDK reads. Confirmed adaptive oldest-frame dropping in Qt, latest-decoded-frame presentation but earlier queue risks in LuneX, fixed 60 Hz MTKView scheduling, shared ENet service/send serialization, per-packet ENet flush, and the non-playback meaning of AVAudioPlayerNode `.dataConsumed`.
- No code acceptance status changed. The audit now supports a prioritized remediation proposal: instrument real queue ages first; remove the 100 ms input send blocker; introduce display-linked one-frame-latency latest-frame presentation; and make audio catch-up depend on actual pending/render/device duration rather than `.dataConsumed` callback count.

# 2026-08-31 Relative direction and cursor-jump repair

- Recovered planning-with-files state, confirmed clean pushed baseline `1499c6e`, and re-read OpenSpec `prioritize-macos-product-completion` status/apply JSON plus every listed proposal/design/spec/tasks context file. Schema remains `spec-driven`, implementation progress remains `7/27`, and Tasks 2.3/3.1/3.2/9.1 remain pending.
- Located the vertical mirror at the shared macOS pointer-motion capture helper: raw `NSEvent.deltaY` crosses unchanged into the platform-neutral adapter and wire codec. The correction will stay at that AppKit boundary so Direct coordinates, scroll, shared wire behavior, and frozen platforms do not change.
- Defined the evidence boundary for the jump report: add distinct bounded input capture/coalesce/reject/send-latency and video receive/decode/replacement/present/frame-age observations before attributing the effect to input or video. No product source, host, Keychain, Simulator, running App, or repository ref has been changed yet.
- Implemented the narrow direction conversion in `MacStreamInputCaptureView.emitPointerMovement` and added semantic tests for mouse moved plus left/right/other drag entry points. Horizontal deltas are preserved, vertical deltas are inverted exactly once, and the existing scroll test remains unchanged.
- Fresh serial warnings-as-errors focused evidence `/tmp/LuneX-RelativeDirection-Focused-R1/Focused.xcresult` passed `37/37` with zero failures. It covers the new direction cases plus the full AppKit surface capture, scroll, coordinate, responder, cursor-lease, window-replacement, and lifecycle-notification class. No real opt-in, host, Keychain, Simulator, or second App operation occurred.
- Fresh serial warnings-as-errors related evidence `/tmp/LuneX-RelativeDirection-Related-R1/Related.xcresult` passed `127/127` with zero skips, failures, or expected failures. It covers AppKit capture, both input adapters, wire vectors, authenticated delivery/provider state, and the first-stage macOS coordinator. Structured test summaries confirm both focused and related results passed.
- OpenSpec now states the exact AppKit-to-Moonlight vertical conversion and its non-regression boundary. Full repository strict validation passed `12/12`; `git diff --check` passed, all three real opt-ins stayed unset, no `xcodebuild`/`xctest` remained, no Simulator or host operation occurred, and the existing single App process was not replaced. Tasks 2.3/3.1/3.2 remain pending because physical direction and the wider input matrix are not complete.
- Direction batch committed and pushed as `bc3716a`. Began the next ENet latency batch by splitting queue/send from flush in the C bridge and implementing a single serial pump with a 1ms maximum service slice, outbound mailbox, one flush per drained batch, bounded counters, and send-queue latency tracking.
- First warnings-as-errors ENet compile `/tmp/LuneX-ENetPump-Compile-R1/Compile.xcresult` stopped before tests because the local pump work closure was not explicitly `@Sendable` when passed to the immediate and delayed `DispatchQueue` APIs. This is two instances of one Swift 6 isolation issue, not a runtime or test failure; the closure is now explicitly `@Sendable` and R1 remains failed evidence.
- Fresh R2 compiled and passed the original production ENet loopback test `1/1`. Expanded R3 passed `4/4`: retransmitted handshake/echo/teardown, a send completing while a 100ms service request is pending, 32 concurrent packets using fewer flushes than packets, and disconnect resolving a five-second pending service after only bounded one-millisecond slices.
- Final review added hard bounds of 256 pending sends and four pending service requests plus saturated rejection counters, and resets every metric on a successful connection. Payloads, endpoints, identities, arbitrary error text, sequence numbers, and packet contents are not stored in the snapshot.
- Fresh final focused R4 `/tmp/LuneX-ENetPump-Focused-R4/Focused.xcresult` passed `4/4`; fresh related `/tmp/LuneX-ENetPump-Related-R1/Related.xcresult` passed `109/109`. The related matrix covered ENet/control, remote-input delivery/wire, macOS input coordination, cancellation, and reconnect.
- Fresh serial full R1 `/tmp/LuneX-ENetPump-Full-R1/Full.xcresult` executed 1368 tests with 1365 passed, two expected opt-in skips, and one unrelated visionOS input-state timeout. The failing test uses a bounded `Task.yield()` polling helper and does not call the changed ENet bridge or driver. Its fresh focused rerun `/tmp/LuneX-ENetPump-VisionFlake-Focused-R1/Focused.xcresult` passed `1/1` in 0.025 seconds; R1 remains failed evidence rather than being discarded.
- Fresh serial full R2 `/tmp/LuneX-ENetPump-Full-R2/Full.xcresult` passed `1368 total / 1366 passed / 2 expected opt-in skips / 0 failed`. Compatibility root `/tmp/LuneX-ENetPump-Compatibility-R1` built macOS universal plus iOS/iPadOS, tvOS, and visionOS generic Debug successfully with zero structured errors, warnings, or analyzer warnings; the macOS executable is exactly `x86_64 arm64`.
- No Simulator was created, started, cloned, or queried. Frozen-platform generic builds are shared-source compatibility evidence only and do not advance product status. Tasks 2.3, 3.1, 3.2, and 9.1 remain pending until the complete physical/live/performance acceptance rows pass.
- Final repository gate passed: the authoritative Ruby generator ran twice and `project.pbxproj` remained `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`; OpenSpec strict passed `12/12`; the active change remains `7/27 ready`; `git diff --check` passed; all three real opt-ins were unset; and no `xcodebuild` or `xctest` process remained. The ENet batch is ready for an independent commit/push without marking any physical/live/performance checkbox complete.
- Resumed from pushed `b4f14fa` with the AppKit visibility fallback and nested render-state invalidation batch still uncommitted. Re-read planning-with-files plus all `prioritize-macos-product-completion` apply context; schema remains `spec-driven`, progress remains `7/27`, and Tasks 2.3/3.1/3.2/9.1 remain pending.
- Added deterministic revision tests for semantic render-surface changes, unchanged-value stability, and platform coordinate snapshot replacement. No test, build, host, Keychain, Simulator, or App replacement has been performed for this uncommitted batch yet.
- Focused R1 used the product-only `LuneX-macOS` scheme and exited 66 before compilation because it has no Test action. The failed invocation is recorded in `task_plan.md`; R2 will use `LuneXCoreTests` in a new evidence directory.
- Fresh serial warnings-as-errors focused R2 `/tmp/LuneX-RenderRevision-Focused-R2/Focused.xcresult` passed `130/130` with zero failures. It covers lifecycle/render revision, AppKit capture and visibility ownership, and Metal presenter behavior; all real opt-ins remained unset and the existing App was not replaced during the test.
- `script/build_and_run.sh --verify` replaced the old App with exactly one new PID `89016`. Computer Use double-clicked `Desktop` twice in the same process. Both attempts reached the stream workspace, lifecycle logged `visible=true focused=true drawable=2676x1606`, then returned to Library near two seconds with generic `session_failed`; no second App, AppleScript, Simulator, Keychain test, or explicit live XCTest gate was used.
- The repeated failure is now assigned to a new ENet/control diagnostic-and-repair batch. The current lifecycle/render invalidation batch remains independently verified by focused tests, product build, and real AppKit attach state; it does not mark Tasks 2.3/3.1/3.2 complete.
- Began the independent ENet/control batch after pushed `3586ca5`. Added closed diagnostic mappings for every `ENetTransportError` case plus a matrix test; unknown raw numeric codes remain excluded. The purpose is to expose the exact finite live failure stage before changing pump behavior.
- Recovered the unsynchronized session context before continuing. The first live diagnostic replacement still reported generic `session_failed`, excluding a directly propagated `ENetTransportError`; the next closed diagnostic extension distinguishes control-stream completion and RTSP ANNOUNCE failures and logs only the Swift error type at the failure boundary.
- Fresh serial warnings-as-errors focused result `/tmp/LuneX-ControlDiagnostics-Focused-R1/Focused.xcresult` passed `35/35` with zero failures. It covers the complete runtime diagnostic store plus ENet, control-stream, and RTSP ANNOUNCE closed mappings; all real opt-ins remained unset and the existing single App was not replaced during the test.
- Committed and pushed the bounded control diagnostic batch as `fb6de36`. The standard script replaced the old App with exactly one PID `92623`; one Computer Use double-click of `Desktop` entered the stream workspace, then returned to Library after about 1.25 seconds. Unified logging identified the error type as `LuneX_macOS.MoonlightMediaReceiveError`; no second App, AppleScript, Simulator, live XCTest, or Keychain test was used.
- Added closed product diagnostic codes for all six `MoonlightMediaReceiveError` cases. Fresh serial warnings-as-errors result `/tmp/LuneX-MediaReceiveDiagnostics-Focused-R1/Focused.xcresult` passed `36/36`; the existing one App process was not replaced during this deterministic gate and all real opt-ins remained unset.
- The pushed diagnostic App reproduced the failure as `media_receive_buffer_overflow`, excluding the other five media receive causes. Implemented fixed-capacity newest-event buffering for production video, retained the existing audio policy, and added a saturated `discardedEventCount` snapshot for both receivers.
- Added deterministic backlog tests proving a capacity-two receiver offered events 1, 2, 3 returns 2 and 3, records exactly one discard, and remains active. Focused warnings-as-errors evidence `/tmp/LuneX-MediaLatest-Focused-R1/Focused.xcresult` passed `49/49` with zero skips or failures and zero structured diagnostics.
- Expanded related evidence `/tmp/LuneX-MediaLatest-Related-R1/Related.xcresult` passed `269 total / 268 passed / 1 expected live-host skip / 0 failed`. It covers media receive, packet assembly, VideoToolbox decode, Metal latest-frame presentation, session media ownership, and AppModel workflows; structured build diagnostics are all zero and no `xcodebuild/xctest` remains.
- Fresh serial full evidence `/tmp/LuneX-MediaLatest-Full-R1/Full.xcresult` passed `1377 total / 1375 passed / 2 exact opt-in skips / 0 failed`; the two skips remain live Sunshine and real Keychain. Structured build diagnostics are `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`.
- Committed and pushed the realtime media backlog fix as `25672d1`. The standard script built that SHA and launched exactly one product PID `96674`; one Computer Use double-click of `Desktop` entered the stream workspace and sustained visible remote video beyond the prior failure interval without returning to Library.
- The live overlay reported `2560x1440 @ 144 fps - 100 Mbps`. Relative mode toggled on, but Computer Use coordinate drag/click returned `noWindowsAvailable`, so no physical direction claim was made; Direct was restored and controls were hidden. The stable session remains running for inspection.
## 2026-08-31 macOS realtime discriminator and pacing continuation

- Recovered planning/OpenSpec state after context handoff. `prioritize-macos-product-completion` remains `spec-driven`, `7/27`, and ready; Tasks 2.3, 3.1, 3.2, and 9.1 remain pending.
- Confirmed the repository is clean at pushed `f9f8852` and the product still has exactly one running process (`96674`). No second App, AppleScript/JXA/CGEvent automation, Simulator, Keychain test, live XCTest gate, or Sunshine mutation was used.
- Re-read all OpenSpec apply context and selected the next two independently verifiable batches: bounded cross-layer realtime telemetry first, then source/display-aware Metal scheduling. No product code has been changed yet.
- Implemented the bounded realtime telemetry batch without event-history writes: macOS input queue/sink time, remote-input queue/coalesce/reject/delivery counts, existing ENet and media-receive snapshots, active decode state, and published/presented/superseded video plus frame age are available through one AppModel snapshot. Diagnostics polls it every 500 ms only while the window is present.
- Fresh warnings-as-errors R1 `/tmp/LuneX-RealtimeTelemetry-Focused-R1/Focused.xcresult` compiled the complete test target and passed `21/21` existing input/presentation tests. Fresh R2 `/tmp/LuneX-RealtimeTelemetry-Focused-R2/Focused.xcresult` passed `23/23`, including new deterministic queue-vs-sink timing, remote-input coalesce/rejection counters, and publish/supersede/present-delay coverage.
- Related warnings-as-errors evidence `/tmp/LuneX-RealtimeTelemetry-Related-R1/Related.xcresult` passed `331 total / 330 passed / 1 live-host opt-in skip / 0 failed`. Full macOS evidence `/tmp/LuneX-RealtimeTelemetry-Full-R1/Full.xcresult` passed `1379 total / 1377 passed / 2 explicit opt-in skips / 0 failed`; the skips remain live Sunshine and real Keychain.
- Final telemetry review added the already-aggregated audio receiver discard count to the on-demand Diagnostics surface and a deterministic AppModel aggregation test covering video/audio receive, remote input, ENet control, decode, presentation, and macOS input snapshots. No realtime transport, decode, render, audio, Keychain, host, Simulator, or running product state changed.
- Final focused R3 attempt 1 exited before compilation because the command selected product scheme `LuneX-macOS`, whose generated scheme has no Test action. This is command-selection error, not product/test evidence; the empty result is not reused. `xcodebuild -list` confirms the correct test scheme is `LuneXCoreTests`.
- Corrected final focused R3 `/tmp/LuneX-RealtimeTelemetry-Focused-R3.xcresult` passed `125/125/0/0` under Swift/Clang warnings-as-errors, including the new aggregation case and all macOS input, remote-input delivery, and session-media environment tests. The generator ran twice with stable project SHA `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`; OpenSpec strict passed `12/12`, diff hygiene passed, and exactly one existing product PID remained running.
- Committed and pushed the telemetry batch as `055d830`; `HEAD == origin/main` and the worktree was clean before starting source/display-aware pacing. The next batch will carry negotiated video FPS and current macOS display maximum FPS into `StreamRenderState`, with deterministic 30/60/120/144/240 resolver coverage and existing 15 fps throttling preserved.
- Implemented the source/display-aware pacing path and an on-demand Diagnostics cadence row. Fresh warnings-as-errors focused `/tmp/LuneX-DisplayPacing-Focused-R1.xcresult` passed `4/4`: 30/60/120/144/240 resolver cases, negotiated 144 fps plus 120 Hz display AppModel ownership, AppKit screen-parameter refresh from 60 to 120, and semantic render revision. No live host, Keychain, Simulator, or running product state was touched.
- Related warnings-as-errors `/tmp/LuneX-DisplayPacing-Related-R1.xcresult` passed `273 total / 272 passed / 1 live-host opt-in skip / 0 failed`. Full macOS `/tmp/LuneX-DisplayPacing-Full-R1.xcresult` passed `1381 total / 1379 passed / 2 explicit opt-in skips / 0 failed`; the skips remain live Sunshine and real Keychain.
- The first frozen-platform generic-build wrapper was rejected before any Xcode command because its temporary-directory setup included `rm -rf`. No build, Simulator, device, Keychain, host, or product-App action occurred; the retry uses a fresh `mktemp` directory without deletion.
- Frozen-platform compatibility builds in `/tmp/LuneX-DisplayPacing-Generic-R1.b9bch4` succeeded for iOS/iPadOS, tvOS, and visionOS with warnings-as-errors. Each log has zero source errors and only the expected AppIntents metadata skip; no Simulator was created, booted, or queried, and these builds do not advance frozen-platform product status.
- Final pacing gate passed: full xcresult is `1381 total / 1379 passed / 2 skips / 0 failed`, OpenSpec strict is `12/12`, generator output is stable at project SHA `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf` across two runs, diff hygiene passes, no xcodebuild/xctest remains, and exactly one old product PID `96674` remains until the post-push replacement.
- OpenSpec now requires the layer discriminator and removes the stale contradictory claim that recoverable receive-buffer saturation must terminate the session. Tasks 2.3, 3.1, 3.2, and 9.1 remain pending; the currently running App has not been replaced.
## 2026-08-31 macOS foreground input/presentation live check

- Recovered the planning/OpenSpec state after handoff and confirmed a clean pushed `aee855e` checkout with exactly one running product PID `9457`.
- Used Computer Use, not AppleScript or focus-stealing automation, to double-click `Desktop` once and open the on-demand Diagnostics window from the native Window menu.
- Recorded the first realtime snapshot and its proof boundary in `findings.md`: input and ENet are currently loss-free/low-delay, while presentation supersedes require foreground delta measurement because opening Diagnostics intentionally throttles the stream to 15 fps.
- Next: close Diagnostics, execute bounded harmless Direct and Relative pointer gestures against the live `Desktop`, reopen Diagnostics, and classify the counter deltas before selecting the next Moonlight-Qt-aligned render change.
- Completed the bounded Direct/Relative gesture check. Input stayed loss-free with single-digit millisecond queue/send maxima, while presentation replacement remained the dominant discriminator. No physical smoothness or vertical-direction acceptance is claimed from Computer Use alone.
- Selected the next isolated fix: inspect the current timer/MTKView path against Moonlight-Qt's `CAMetalDisplayLink` plus one-frame latency policy, then implement and verify display-linked latest-frame presentation without mixing audio changes into the batch.
- Implemented the macOS display-link owner, direct callback-drawable presenter entry, one-frame latency, exact active/throttled cadence, pause/teardown ownership, and MTKView fallback. Focused R1 compiled product sources but stopped while compiling the new owner tests because `MacStreamInputCaptureViewTests.swift` lacked an explicit `MetalKit` import; no test ran and no runtime/host/Keychain/Simulator state changed. Added the missing test dependency and retained R1 as failed evidence before R2.
- Focused R2 reached strict product compilation and exposed the expected Objective-C delegate isolation boundary plus the extracted render body's missing explicit MainActor annotation. Changed the main-run-loop display-link callback to a synchronous `MainActor.assumeIsolated` handoff and annotated the shared render entry; no asynchronous frame-delaying hop was introduced. R2 ran no tests and is retained before fresh R3.
- Focused R3 closed the render-body errors but Swift 6 rejected capturing QuartzCore's non-Sendable callback objects in the `assumeIsolated` closure. Replaced that bridge with an explicit `@preconcurrency CAMetalDisplayLinkDelegate` conformance, matching the callback's registration on `RunLoop.main` while preserving synchronous use of its drawable. R3 ran no tests and is retained before R4.
- Focused R4 compiled and ran; both new display-link owner tests passed. Its only failure was the existing display-move-during-draw stale-plan test because the refactor evaluated the drawable provider before taking the presenter snapshot. Restored the original snapshot-before-drawable ordering through a delayed provider closure, preserving the final revision check for both MTKView and display-link drawables. R4 is retained before R5.
- Fresh warnings-as-errors focused R5 `/tmp/LuneX-MetalDisplayLink-Focused-R5.xcresult` passed `116/116/0/0`. Structured build results are `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`; no test/live opt-in, Keychain, Simulator, Sunshine mutation, or product-App replacement occurred.
- Synchronized the active OpenSpec design/spec with the accepted runtime contract: one current-surface `CAMetalDisplayLink`, callback drawable, one-frame latency, latest-frame semantics, lifecycle pause/invalidation, and explicit MTKView fallback only when the display-link runtime cannot attach.
- Related warnings-as-errors evidence `/tmp/LuneX-MetalDisplayLink-Related-R1.xcresult` passed `344 total / 343 passed / 1 explicit live-host skip / 0 failed`; structured build diagnostics are `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`.
- Fresh serial full macOS evidence `/tmp/LuneX-MetalDisplayLink-Full-R1.xcresult` passed `1383 total / 1381 passed / 2 explicit opt-in skips / 0 failed`; the skips remain live Sunshine and real Keychain, and structured build diagnostics are `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`.
- Frozen-platform generic builds succeeded for iOS/iPadOS, tvOS, and visionOS in three isolated DerivedData/result bundles, each with `0 errors / 0 warnings / 0 analyzer warnings`. No Simulator was queried, created, booted, or otherwise operated; these results prove shared-source compilation only.
- Final repository gate passed: the authoritative Ruby generator ran twice with stable `project.pbxproj` SHA-256 `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf`; OpenSpec strict passed `12/12`; `git diff --check` passed; all real opt-ins remained unset; no test process remained; and exactly one old product process stayed running pending post-push replacement.
- Display-link batch committed and pushed as `6206165`; `HEAD == origin/main` and the worktree was clean. Standard product replacement launched exactly one initial PID `15431`.
- Catalog launch then produced two identical macOS 27 Observation crashes: initial PID `15431`, followed by Computer Use's automatic state-read relaunch PID `15740`. Both fail after the first launch suspension in `ObservationTracking._AccessList.addAccess` through `activeProductSessionOwner`; no stream reached media startup. Stopped UI actions, retained both incident reports, and selected a detached MainActor launch task as the narrow fix before any further product retry.
- Changed only the macOS catalog launch boundary from inherited `Task` to `Task.detached { @MainActor in ... }` and added a source-contract assertion while preserving double-click, Return launch, AppModel actor ownership, launch serialization, cancellation, and stale-owner revalidation. No shared platform source, host, Sunshine, Keychain, Simulator, or product process was touched by this edit.
- Focused R1 selected a nonexistent test method and executed `0` tests, so it is recorded as invalid evidence. Corrected related R2 passed `108/107/1/0`; fresh serial full R1 passed `1383/1381/2/0`. Both valid xcresults report `succeeded` with `0 errors / 0 warnings / 0 analyzer warnings`; no `xcodebuild` or `xctest` remains. The batch is ready for immediate commit/push before a one-instance live product retry.
- Committed and pushed that first form as `5f24f57`, then the standard product script stopped before launch: the `LuneX-macOS` scheme's Swift 6 region-based isolation checker rejected the detached MainActor closure's implicit captures with an internal unsupported-pattern diagnostic. No App process was created. Replaced implicit capture with explicit `[appModel, workspace]`; this corrected form requires fresh product, related, and full gates before the next push and product retry.
- Product compile R2 retained the explicit capture list but still failed on the closure-level MainActor pattern, proving the unsupported checker path is the actor-annotated detached closure itself. Replaced it with a plain detached closure that explicitly captures only `appModel` and `workspace`; the `@MainActor AppModel` call supplies the required actor hop while the detached task still excludes inherited Observation task-local state.
- Strict product compile R3 `/tmp/LuneX-MacCatalogObservation-ProductCompile-R3.xcresult` succeeded with `0 errors / 0 warnings / 0 analyzer warnings`. Broader related R3 `/tmp/LuneX-MacCatalogObservation-Related-R3.xcresult` passed `139/138/1/0`; fresh full R2 `/tmp/LuneX-MacCatalogObservation-Full-R2.xcresult` passed `1383/1381/2/0`. The two skips remain the explicit live-host and real-Keychain gates, and no product process was launched by these checks.
- Committed and pushed the compile correction as `ad4fb4c`. Standard product launch created only PID `19270`; one Computer Use double-click reached the stream workspace, then PID `19270` crashed with the same post-prepare Observation stack. Computer Use state inspection auto-relaunched replacement PID `19771`, confirmed by fresh startup diagnostics and `080537.ips`; no concurrent App existed.
- Replaced detached launch with next-main-dispatch-turn Task creation. This lets the SwiftUI action callback and its Observation tracking scope unwind before the MainActor async workflow starts, without suppressing observation on session ownership or derived UI state. Product/related/full and another exact single-instance live retry are pending for this candidate.
- Strict product compile R4 `/tmp/LuneX-MacCatalogObservation-ProductCompile-R4.xcresult` succeeded with zero structured diagnostics. Related R4 `/tmp/LuneX-MacCatalogObservation-Related-R4.xcresult` passed `139/138/1/0`; fresh full R3 `/tmp/LuneX-MacCatalogObservation-Full-R3.xcresult` passed `1383/1381/2/0`. No new product process was launched; the current PID remains the single Computer Use auto-relaunch from the failed pushed candidate until post-push replacement.
- Implemented the decoded-frame readiness gate: an empty macOS stream surface remains paused without a `CAMetalDisplayLink`, the first decoded presentation contract attaches it, and contract clear invalidates/detaches it. Strict product build `/tmp/LuneX-MetalDisplayReadiness-Product-R1.xcresult` succeeded. Focused R1 `/tmp/LuneX-MetalDisplayReadiness-Focused-R1.xcresult` failed during test compilation because the new fixture omitted the required no-op `sampleHandler`; production code was not implicated, the fixture is corrected, and R1 remains failed evidence pending fresh R2.
- Fresh focused R2 `/tmp/LuneX-MetalDisplayReadiness-Focused-R2.xcresult` passed `44/44` with zero skips, failures, expected failures, errors, warnings, or analyzer warnings. It includes the new first-frame readiness transition and the complete `MacStreamInputCaptureViewTests` lifecycle/input/display-link surface matrix.
- Fresh serial related R1 `/tmp/LuneX-MetalDisplayReadiness-Related-R1.xcresult` passed `345 total / 344 passed / 1 explicit live-host skip / 0 failed / 0 expected failures`; structured build diagnostics are `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`. The matrix covers AppModel, lifecycle/render policy, AppKit surface/input, Metal presentation, decoded-frame delivery, VideoToolbox recovery, HDR/EDR, and media-environment ownership.
- Fresh serial full R1 `/tmp/LuneX-MetalDisplayReadiness-Full-R1.xcresult` passed `1384 total / 1382 passed / 2 exact opt-in skips / 0 failed / 0 expected failures`; structured build diagnostics are `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`. The skips remain live Sunshine and real Keychain, so no physical/live completion checkbox changed.
- Final readiness repository gate passed: product strict build has zero diagnostics, OpenSpec all strict is `12/12`, the authoritative generator is stable at project SHA-256 `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf` before and after two runs, diff hygiene passes, all three real opt-ins are unset, and no test or product process remains. The exact seven-file batch is ready for immediate commit/push.
- Committed and pushed the readiness gate as `d0e36ce`. The standard script launched exactly PID `24495`; one Computer Use Desktop double-click crossed the old crash point, retained the original PID after eight seconds, produced no new crash report, and displayed real video. The first bounded snapshot was `3121 published / 3019 presented / 101 superseded`, 9.6 ms latest-frame age, and 18.4 ms maximum presentation delay.
- Computer Use switched to Finder for ten seconds and returned to the same active stream workspace and PID with updating video. The second mixed foreground/background snapshot was `12359 / 11376 / 982`, 18.3 ms latest-frame age, and 21.6 ms maximum presentation delay. Coordinate drag and coordinate click each returned tool-level `noWindowsAvailable`, so Relative physical direction remains pending and the failures are not counted as product input loss.
- Began the next isolated cleanup by restoring the catalog's original structured `Task` launch and adding a source contract that rejects both `DispatchQueue.main.async` and `Task.detached` workarounds. The readiness fix, not launch deferral, now owns the crash correction.
- Structured catalog cleanup passed every deterministic gate: strict product build `succeeded/0/0/0`; focused `/tmp/LuneX-CatalogStructuredTask-Focused-R1.xcresult` `32/32`; related R1 `200/199/1/0`; full R1 `1384/1382/2/0`; OpenSpec all strict `12/12`; and diff hygiene. The one running old pushed App PID `24495` remains untouched until the new batch is committed and pushed.
- Committed and pushed the structured catalog launch as `1e6dcdc`. Standard replacement produced only PID `26877`; one Computer Use Desktop double-click retained the original PID after eight seconds with no crash report and sustained real video. The bounded snapshot was control max 2.7 ms, zero media receive discards, `2592/2503/88` published/presented/superseded, 13.4 ms latest-frame age, and 6.6/12.3 ms current/maximum presentation delay. The App is left in the stream workspace for user testing.
- Live diagnostics simultaneously showed `Video decode: 0 access units, 0 frames, 0 failures`, which contradicts actual presentation. The next isolated batch will trace and repair decode telemetry publication so layer discrimination remains truthful.
- Began the host-verified input/decoder discriminator batch. Source tracing proved production decode snapshot ownership was already correct but its UI labels exposed only drop counters as if they were totals. Added submitted access-unit and current-generation decoded-frame totals, corrected the Diagnostics labels, restored host-verified raw AppKit vertical delta, and added replacement-safe ownership that disables AppKit mouse-event coalescing only during active capture. Focused verification is pending; no product process, host, Keychain, Sunshine, or Simulator state changed.
- Focused R1 failed before tests because Xcode 26's Swift overlay renames both the getter and setter to `NSEvent.isMouseCoalescingEnabled`; the implementation used the Objective-C property base name for writes. Corrected only that spelling and retained R1 as failed evidence before a fresh R2.
- Fresh focused R2 `/tmp/LuneX-InputDecode-Focused-R2.xcresult` passed `57/57` with zero failures or skips under warnings-as-errors. The next serial gate expands to macOS input coordination/delivery, VideoToolbox decode, generation-owned media environment, Metal presentation, AppModel aggregation, and runtime Diagnostics.
- The first related invocation was rejected before process creation because it tried to pre-clean temporary evidence paths with `rm -rf`. No build, test, deletion, App, host, Keychain, or Simulator action occurred; continue with a never-used R2 path and no cleanup command.
- Related R2 `/tmp/LuneX-InputDecode-Related-R2.xcresult` passed `406 total / 405 passed / 1 explicit live-host skip / 0 failed`; its build result is `succeeded/0 errors/0 warnings/0 analyzer warnings`. Proceed to the strict `LuneX-macOS` product scheme build before a fresh full suite.
- Strict unsigned Debug product build `/tmp/LuneX-InputDecode-Product-R1.xcresult` succeeded with zero structured errors, warnings, or analyzer warnings. This compiles the actual macOS App and corrected Diagnostics UI; continue to a fresh full macOS test result.
- Fresh full macOS `/tmp/LuneX-InputDecode-Full-R1.xcresult` passed `1385 total / 1383 passed / 2 exact opt-in skips / 0 failed` with zero structured diagnostics. Because the decode snapshot shape is shared, run only generic frozen-platform compatibility builds next; they do not advance those products and no Simulator may be touched.
- Generic unsigned warnings-as-errors compatibility builds for `LuneX-iOS`, `LuneX-tvOS`, and `LuneX-visionOS` all succeeded with zero structured errors, warnings, or analyzer warnings. No Simulator was started, created, or queried; move to generator/OpenSpec/diff/process/opt-in repository gates.
- Repository gate passed: the project generator produced the identical `e412926...7524bf` hash before, between, and after two runs; OpenSpec strict passed `12/12`; diff hygiene passed; all three real opt-ins are unset; no test process remains. The only product PID is the existing old pushed build `26877`, which remains untouched until commit and push.
- Investigated the user's pressed-drag window-exit crash using the new `15:40:54` system incident for original PID `34362`; the pasted report was the unrelated old PID `15431` launch crash. The new fault is again Swift Observation access-list corruption at the internal `activeProductSessionOwner` getter while the control-event loop resumes. Marked that MainActor reservation token `@ObservationIgnored`, added an Observation dependency regression, and added pressed/out-of-surface drag plus high-frequency pointer/control interleaving coverage. Validation is pending; current PID `39895` is the post-crash replacement and no second App was started.
- Focused R1 `/tmp/LuneX-DragExit-Focused-R1.xcresult` subsequently completed under Swift/Clang warnings-as-errors and passed `2/2/0/0`; structured build result is `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`. It exercised the Observation-ignore contract and a pressed left-button drag beyond the surface without Keychain, live-host, Simulator, or product-App access.
- Fresh serial related R1 `/tmp/LuneX-DragExit-Related-R1.xcresult` passed `309 total / 308 passed / 1 skipped / 0 failed`; the sole skip is the explicit live Sunshine acceptance gate. Structured build result is `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`. The matrix covered complete AppKit capture, macOS input coordination, AppModel workflow/session ownership, media environment, remote-input delivery, and product-workflow surfaces.
- Unsigned strict product build `/tmp/LuneX-DragExit-Product-R1.xcresult` completed the real `LuneX-macOS` scheme with `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`. This is compile evidence only and is not a signed/head-pose or physical drag-out acceptance claim.
- Fresh serial full macOS `/tmp/LuneX-DragExit-Full-R1.xcresult` passed `1387 total / 1385 passed / 2 skipped / 0 failed`; the two exact skips remain live Sunshine and real Keychain. Structured build result is `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`.
- Minimum shared-source compatibility builds succeeded for `LuneX-iOS` (iPhone/iPad compile surface), `LuneX-tvOS`, and `LuneX-visionOS` using generic Simulator placeholder destinations. All three structured results are `succeeded / 0 errors / 0 warnings / 0 analyzer warnings`; no Simulator was created, booted, cloned, or queried, and these builds do not advance frozen-platform product status.
- Final review confirmed every reservation-token mutation is paired with observable launch/session/workspace/render/input projections and no View directly binds `activeProductSessionOwner`. The generator remained stable at `e412926e1036f7ebda429dfffb4b5a2705cf01dccba65c08cdccbb74da7524bf` before, between, and after two runs; OpenSpec strict passed `12/12`; diff hygiene passed; all real opt-ins are unset; and no `xcodebuild`, `xctest`, or LuneX product process remains. Tasks 2.3/3.1/3.2 remain pending until the exact pushed product passes the user's physical drag-out reproduction.
- Committed and pushed next-turn launch as `78f0596`; standard replacement created only PID `21091`. A fresh Computer Use double-click was followed by an eight-second shell-side process check before any UI read: PID `21091` had exited, no LuneX process remained, and `081000.ips` showed a SwiftUI scroll-environment Observation access-list fault rather than the prior AppModel getter stack.
- Began the display-link readiness batch. The coordinator now keeps both CAMetalDisplayLink and MTKView scheduling paused until a decoded presentation contract exists, attaches at the negotiated/display cadence on the first valid contract, and invalidates/pauses again when the decoder contract clears. Added deterministic coverage for no pre-frame attachment, 144 Hz activation, and clear-time invalidation; validation is pending.
