...
| Host state | Server reported busy with `currentgame=881448767`, matching `Desktop` | Selects `/resume` for this request; busy is neither an exclusive client-capacity signal nor a session-admission gate |
...
| RTSP negotiation | Launch URL, DESCRIBE, audio/video/control SETUP, negotiated endpoints, and control readiness succeed | Passed on exact SHA `05aa877`: ANNOUNCE, PLAY, ENet, negotiated configuration, control readiness, and video color metadata completed. Current candidate additionally supports the server-requested control-v2 + audio-encryption path deterministically; no new live receipt has been run yet. |
...
1. `tanmy-white` is reachable. Matching running `Desktop` state selects
   `/resume`; every other, missing, unknown, or inconsistent state attempts
   the user-selected `Desktop` through `/launch`. The authenticated Sunshine
   response, not a client-side free/busy assumption, determines whether the
   request is supported. LuneX never calls `/cancel` to manufacture a free
   state.
...
Matching running-application state selects `/resume`; otherwise LuneX attempts
`/launch`. Server state is not used as a global connection-availability or
client-capacity gate:
...
- exactly one initial `/resume` when `Desktop` is already identified as
  running, otherwise one `/launch` attempt whose server response is
  authoritative;
...
The UDP complete-message repair was committed and pushed as exact SHA
`f7c059bda4765923c8b67ebbf71ffa9a249356f3`. Its fresh warnings-as-errors live
bundle build at `/private/tmp/LuneX-M1-2_3-udp-live-build.glscZj` succeeded with
zero structured errors, warnings, or analyzer warnings. The only double-opt-in
session gate ran from `2026-08-26T16:42:23Z` through `16:42:26Z`, correctly
propagated XCTest exit `1`, and failed after 1.981 seconds with
`issue=media_presentation_failed`, `diagnostic=video_pipeline_failed`, and
`subsystem=stream.video`. The recorder reported `launch=0`, `resume=1`,
`cancel=0`, `controlEvents=launch_accepted,rtsp_ready,negotiated,channels_1,video_color_metadata`,
and no control failure.

This proves the live path crossed the former UDP channel closure and reached
video packet processing; it does not prove decoded presentation or any later
Task 2.3 row. All four local state files retained mode `0600`, size, and SHA-256,
and no build or test process remained. This exact attempt must not be repeated.
The next deterministic repair targets Sunshine-generated video parity shards:
Sunshine writes `multiFecFlags=0x10` into data shards before Reed-Solomon
encoding, then rewrites parity RTP/FEC indexing fields but not the parity
`multiFecFlags` byte. LuneX must therefore require the marker for data shards
without rejecting an otherwise bounded parity envelope whose generated byte is
not `0x10`.

## Exact-SHA Parity Envelope Receipt

The parity-envelope repair was committed and pushed as exact SHA
`3533b8078fe9c4eef634d3e840a2dc8dc3f1d28d`. Its fresh warnings-as-errors live
bundle at `/private/tmp/LuneX-M1-2_3-parity-live-build.VB3UP5` built with zero
structured errors, warnings, or analyzer warnings. A first wrapper invocation
failed during the pre-run state snapshot because a zsh local named `path`
overrode the shell command-search array. It produced no `LIVE_START`, launched
no xctest process, and accessed neither host nor session, so it is not a live
attempt.

The only actual double-opt-in gate ran from `2026-08-26T17:01:21Z` through
`17:01:29Z`, propagated XCTest exit `1`, and failed after 6.571 test seconds
with `issue=stream_interrupted`, `diagnostic=transport_failed`, and
`subsystem=stream.transport`. The recorder reported `launch=0`, `resume=1`,
`cancel=0`,
`controlEvents=launch_accepted,rtsp_ready,negotiated,channels_1,video_color_metadata`,
and no control failure. All four local state files retained mode `0600`, size,
and SHA-256, and no build or test process remained.

This receipt does not invalidate the deterministic parity correction, but it
does not prove live decoded video either. The aggregate transport diagnostic
does not identify whether video receive, audio receive, ping/send, or another
session-media resource failed. This exact attempt must not be repeated. The
next repair must first add a privacy-bounded typed media failure stage recorder
and deterministic mapping tests so a future exact-SHA receipt can isolate the
failing production resource without exposing arbitrary error text.

## Typed Media Failure Receipt Harness

The macOS Debug live XCTest now wraps the production
`MoonlightVideoReceiveProvider` and `MoonlightAudioReceiveProvider` in test-only
forwarding providers. They preserve yielded events, original thrown errors,
stop calls, cancellation, and consumer-termination behavior while recording
only `video_receive` or `audio_receive` plus a finite cause code. Network
timeouts distinguish the production `connect`, `send`, and `receive`
operations; unknown operation strings map to `network_other_timed_out` rather
than entering the receipt. Numeric transport details, packet payloads,
endpoints, certificates, credentials, and arbitrary error descriptions are not
stored or formatted. `CancellationError` and
`NetworkChannelError.cancelled` produce no failure row.

Deterministic coverage proves video and audio attribution, event and stop
forwarding, original error propagation, parser and transport mapping, arbitrary
text exclusion, cancellation suppression, and upstream cancellation after
consumer termination. The final focused gate passed `4/4`. The related
AppModel/media gate passed `159 total / 158 passed / 1 live skip / 0 failed`,
and the final normal macOS gate passed `1316 total / 1314 passed / 2 explicit
skips / 0 failed`; every successful xcresult had zero structured errors,
warnings, and analyzer warnings. The two final skips were the explicit live
Sunshine and real-Keychain opt-ins. No new live attempt has been made, and Task
2.3 remains pending.

## Host-State Admission Correction

The client no longer interprets Sunshine's advertised free/busy state as an
exclusive client-capacity contract. If the server identifies the selected
application as already running, LuneX uses `/resume` to attach another stream
session. Every other free, busy-with-another-application, missing, unknown, or
inconsistent state results in one authenticated `/launch` attempt for the
user-selected application. The server response is authoritative; LuneX does
not reject the selection locally or call `/cancel` to manufacture a free
state.

Fresh deterministic evidence for this policy passed `35/35` focused tests and
the full macOS suite at `1316 total / 1314 passed / 2 explicit opt-in skips / 0
failed`. The focused and normal xcresults both report zero structured errors,
warnings, and analyzer warnings. macOS Debug and Release plus generic
iOS/iPadOS, tvOS, and visionOS builds all report
`succeeded / 0 errors / 0 warnings / 0 analyzer warnings`; both macOS products
are universal `x86_64 arm64`, and each build produced its Metal AIR and
