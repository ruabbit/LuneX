...
| 14 | `in_progress`，OpenSpec `24/29` | 完成AppKit合同、共享坐标、闭合directive、generation-scoped lifecycle、AppModel/media application、active input coordinator、actual direct/relative capture、balanced cursor ownership、responder/dismantle、stream-view backing/display/live-resize检测、privacy-bounded diagnostics、单session application gate，以及无live/Keychain副作用的normal suite | Release/深度质量门、simulator最终门与live hardware证明尚未完成 |
