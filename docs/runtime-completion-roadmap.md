...
| 17 | `in_progress`，OpenSpec `26/36` | actual UIKit scene/window/geometry/input、actual-window mobile EDR、sample-buffer PiP runtime、actual-state continuity policy、serialized mobile media owner，以及iPhone/iPad单值`audio` source/project/unsigned built-plist配置均完成 | serialized owner尚未接入media environment/AppModel；尚无signed配置接受、actual-state UI、固定simulator及物理PiP/background/Stage Manager/EDR/live Sunshine证明 |
...
- OpenSpec `integrate-mobile-scene-pip-continuity`当前`26/36 in_progress`；1.x合同、2.x actual UIKit scene/window/geometry/input、3.x actual-window mobile EDR、4.x sample-buffer PiP runtime和5.1至5.3均已完成。5.2以FIFO gate统一foreground、PiP/audio-only、pause/stop、replacement、失败回滚和exactly-once foreground resample的纯值动作。5.3修复了未进入built plist的旧单值设置：iPhoneOS Debug/Release实际plist精确为`UIBackgroundModes == [audio]`和`UIDeviceFamily == [1,2]`，其他平台未被阶段17声明；这些unsigned产物不证明签名接受或runtime background行为。下一项5.4把scene/geometry/PiP/continuity/mobile EDR和bounded diagnostics接入`NativeSessionMediaEnvironment`与`AppModel`并在stop/failure/replacement清理actual state。
