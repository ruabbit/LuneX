...
- macOS Escape、tvOS Menu/Back与visionOS Escape只打开本地controls，不进入remote serialization；所有可见Disconnect/Stop入口先产生workspace-local confirmation，确认后加入3.4共享stop operation。remote termination、failure、prepared-session invalidation与local stop都会清除旧overlay/dialog。3.6仍负责compact/wide重组，3.7负责完整session应用矩阵。
