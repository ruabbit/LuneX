...
| 18 | `in_progress`，OpenSpec `22/50` | 1.1–4.3已勾选；actual tvOS stream screen/layer observation、surface-owned semantic display revision、current-source coordinator/AppModel/render application、direct EDR或typed HDR-to-SDR diagnostic、replacement/exhaustion/failure/stop清理均已实现；fresh `190/190` focused、`361/361` related、`1047/1046/1/0` normal、direct tvOS及unsigned五平台Debug均通过 | 4.4–8.x audio、shared reconnect、visionOS、产品、release及signed/物理/live HDR证据未完成；same-view geometry replay明确保留给4.5 |
...
- OpenSpec `integrate-tvos-visionos-runtime`为`21/50 ready`、next 4.3；4.2在repository pre-gate `/tmp/LuneX-18-4_2-repository-pre.IRGscf`通过后勾选，post-mark final-state `/tmp/LuneX-18-4_2-final-state-r2.l4fp0g`与post-record `/tmp/LuneX-18-4_2-post-record.YjRjuV`只读确认权威状态、稳定project hash、精确scope与五份记录。3.7的remote/focus/controller和4.1 coordinator-admitted actual Metal presentation保持完成。4.2新增独立`preferredDynamicRangeAndHeadroom`能力、public tvOS screen/layer/color probe、稳定typed fallback、headroom-tagged EDR contract以及新旧layer field完整rollback；fresh focused `/tmp/LuneX-18-4_2-focused-r3.uUoogT`为`54/54`、related `/tmp/LuneX-18-4_2-related-r2.X5RkJV`为`209/209`、normal `/tmp/LuneX-18-4_2-normal-r2.q9nMOC`为`1040/1039/1 exact Keychain skip/0`，direct fixed tvOS与unsigned五平台Debug均零结构化/source diagnostics成功。下一步进入4.3。
- tvOS 26.4旧`wantsExtendedDynamicRangeContent`/`edrMetadata`不可用；4.2使用`CALayer.preferredDynamicRange`、`contentsHeadroom`、`toneMapMode`、extended-linear Display P3/ITU-R 2020和`UIScreen` current/potential headroom。direct contract只允许有限`1 < current <= potential <= 64`，否则保留typed HDR-to-SDR。4.3仍负责actual scene observation、semantic display revision、coordinator/AppModel应用与fallback diagnostics；visionOS没有公开有限current headroom来源时继续typed fallback。
