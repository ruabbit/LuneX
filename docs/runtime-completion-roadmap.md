...
| 15 | `in_progress`，OpenSpec `26/33` | color/luminance、decoded/Metal frame、shader/readback、surface/display/resolver/presenter、macOS transition矩阵、四平台typed capability/fallback、5.1 production graph、5.2 active-session eligibility、5.3 scoped diagnostics及5.4 application gate完成 | 5.5 status/settings UI及live/compositor/物理显示器验证仍待完成 |
...
- OpenSpec `implement-native-hdr-edr-pipeline`当前`26/33`；5.4已用单一application gate串联session/media/decoder、actual presentation source、presenter surface/runtime、diagnostics与stop。下一项5.5为accessibility-safe native HDR status/settings UI。
