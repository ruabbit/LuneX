...
| 17 | `in_progress`，OpenSpec `31/36` | actual UIKit scene/window/geometry/input、actual-window mobile EDR、sample-buffer PiP runtime、actual-state continuity policy、serialized mobile media owner、media environment/AppModel application与bounded diagnostics、stop/failure/replacement清理、iPhone/iPad单值`audio` source/project/unsigned built-plist配置、5.5 actual-state UI/PiP命令/migration、5.6跨层/UI/persistence完整确定性回归、6.1 normal/唯一Keychain skip，以及6.2五平台Debug/Release十配置零诊断/Metal/plist/simulator不变门均已完成 | 6.3–6.7 repository、sanitizer/resource、fixed-simulator与signed/physical PiP/background/Stage Manager/EDR/live Sunshine证明尚未完成 |
...
- OpenSpec `integrate-mobile-scene-pip-continuity`当前`31/36 in_progress`；1.x至5.6及6.1–6.2均已完成。完整normal为`909/908/1/0`且唯一skip是显式真实Keychain用例；6.2在`/tmp/LuneX-17-6_2-builds.ORyQlN`保留macOS、固定iPhone、固定iPad、tvOS与visionOS的Debug/Release十个独立build，全部零structured diagnostics并各有AIR/metallib。iOS simulator产物保持`UIBackgroundModes [audio]`与`UIDeviceFamily [1,2]`，其余平台无后台模式键；pre/post规范化simulator清单SHA-256同为`0470edc00aea815358b4bed51fa43b73b79a5cbc61f80856f9630c6128568d41`，固定iPhone/iPad仍唯一、available、`Shutdown`且全局`Booted=0`。下一项6.3为repository/static analyzer gates；unsigned产物仍不证明签名接受、system PiP、background duration、Stage Manager、visible EDR或live Sunshine。
