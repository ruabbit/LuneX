...
| 16 | `in_progress`，OpenSpec `16/35` | canonical布局、session-owned spatial graph/fallback、平台策略/测试、embedded reader、entitlement配置、移动audio-session、macOS actual-output capability及有界语义通知monitor已完成 | 尚未完成runtime recovery/application接线、signed provisioning及音频硬件验证 |
...
- OpenSpec `integrate-spatial-audio-runtime`当前`16/35 in_progress`。1.1至1.5完成实际边界、canonical布局、resolver及有界隐私history；2.1至2.6完成immutable graph intent、显式Core Audio interleaved格式、session-owned ambience-bed graph、typed direct fallback、macOS/iOS/tvOS listener与visionOS intended-experience策略，以及schedule/readback、partial failure、stop与ARC测试矩阵。3.1新增可注入embedded entitlement reader；3.2新增generator-owned macOS/iOS/tvOS entitlement配置；3.3把移动家族playback、多声道声明、preferred channel、spatial port readback、停用和capability notification名称收拢到production adapter；3.4从macOS实际engine output format与graph connection/algorithm/fallback readback形成不含产品名推断的三态capability；3.5把移动家族route/interruption/media-services/spatial-capability通知转成有界、单调、语义去重的revision，并处理reset-without-lost、stop/deinit和晚到callback。下一项3.6补齐平台adapter/observer矩阵。当前离线测试和unsigned build仍不证明signed provisioning、真实硬件route行为、AirPods head tracking、声道分离或硬件可听定位。
