...
| 17 | `in_progress`，OpenSpec `24/36` | actual UIKit scene/window/geometry/input、actual-window mobile EDR、sample-buffer PiP runtime，以及只接受current-generation native-confirmed PiP或active/permitted audio的continuity policy均完成离线实现与回归 | 尚无统一application media owner、生成配置验收、actual-state UI、固定simulator及物理PiP/background/Stage Manager/EDR/live Sunshine证明 |
...
- OpenSpec `integrate-mobile-scene-pip-continuity`当前`24/36 in_progress`；1.x合同、2.x actual UIKit scene/window/geometry/input、3.x actual-window mobile EDR、4.x sample-buffer PiP runtime和5.1 actual-state continuity policy均已完成。PiP通过最多8个generation-filtered订阅和单槽main-actor mailbox复用唯一presentation source及同一`CVPixelBuffer`，不创建第二decoder；continuity action现在只接受generation-matched native-confirmed PiP+operational sink或active/permitted audio，capability/configuration presence不能伪造active。下一项5.2通过一个serialized mobile media generation owner统一scene、audio、PiP、foreground renderer、decoder/control、pause/stop和foreground restoration。
