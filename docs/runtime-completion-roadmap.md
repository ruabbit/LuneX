...
| 19 | `in_progress`，OpenSpec `complete-native-product-workflows` | host/pairing/catalog/session恢复、多窗口ownership、typed accessibility semantics及actual-width/Dynamic Type adaptive layout已有确定性实现与任务级离线验收 | 5.3以后键盘/触控/reduced-motion/tvOS/visionOS focus、diagnostics export、跨产品回归及signed/physical/live验收仍未完成 |
