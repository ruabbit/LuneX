...
| 14 | `in_progress`，OpenSpec `23/29` | 完成AppKit合同、共享坐标、闭合directive、generation-scoped lifecycle、AppModel/media application、active input coordinator、actual direct/relative capture、balanced cursor ownership、responder/dismantle、stream-view backing/display/live-resize检测、privacy-bounded diagnostics，以及单session application integration gate | normal/Release/深度质量门与live hardware证明尚未完成 |
