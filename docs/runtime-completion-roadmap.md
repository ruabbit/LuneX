...
| 15 | `in_progress`，OpenSpec `6/33` | color/luminance foundation完成：immutable/decoded合同、color/PQ/gamut math、source-peak/current-headroom mapping及deterministic grids | 尚无generation/Metal binding、shader、production presenter、float surface、动态跨屏或物理显示器验证 |
...
- OpenSpec `implement-native-hdr-edr-pipeline`当前`6/33`；第1组color/luminance foundation及deterministic gates完成，下一项2.1绑定generation-owned decoded/Metal frame contract。
