...
- checked action dispatcher在invocation时重新核对workspace generation、当前展示token、active或terminal session identity及3.2 command disposition；旧session replay和replaced workspace返回typed `staleAction`且不操作replacement。并发共享结果、overlay与owning-window close分别由已完成的3.4、3.5与4.4合同约束。
...
- macOS Escape、tvOS Menu/Back与visionOS Escape只打开本地controls，不进入remote serialization；所有可见Disconnect/Stop入口先产生workspace-local confirmation，确认后加入3.4共享stop operation。remote termination、failure、prepared-session invalidation与local stop都会清除旧overlay/dialog。3.6现已完成compact/wide重组，3.7现已完成完整session应用矩阵。
...
- Task 3.6最终离线证据为focused `4/4`、related `176/176`、serial normal `1225/1224/1/0`，唯一skip仍为显式关闭的真实Keychain测试；macOS universal、iOS/iPadOS、tvOS、visionOS unsigned generic Debug `4/4`结构化零错误、零warning、零analyzer warning。普通测试继续JSON文件fallback，未操作Simulator，也不构成signed、physical resize/touch/focus、assistive-technology或live Sunshine证明。
- Task 3.7审计九类session application合同后只发现一个缺口：第一代clean stop、第二代已streaming后，第一代迟到termination必须被完整owner/generation guard拒绝。新增测试只为默认关闭的provider double保留旧continuation，production零修改；focused为`1/1`、九类expanded matrix为`10/10`、serial normal为`1226/1225/1/0`，三份build diagnostics全零，唯一skip仍是显式关闭的真实Keychain round-trip。该项自身不证明live Sunshine、signed/physical平台或真实窗口/输入/媒体行为；后续4.4已补离线close合同、5.1已补typed semantic descriptor、5.2已补adaptive dashboard合同，5.3至5.6与7.x仍独立。
- Task 4.1以typed `WindowGroup`序列化`ProductWorkspaceID`，由MainActor scene coordinator连接native scene与既有registry。supported配置的首个scene使用或采用primary，后续nil scene创建distinct workspace，断开后同ID重连推进generation并保留navigation/host/app、清空transient presentation；active duplicate ID在replacement前fail closed。
- scene root读取`supportsMultipleWindows`；unsupported配置忽略外部scene ID并继续单一checked primary。tvOS保留普通single-workspace `WindowGroup`，visionOS现使用显式ID的单实例`Window`；两者都不暴露application window commands。4.1的底层coordinator detach只撤销ephemeral attachment且不close registry或transfer session；4.4现由AppModel在调用detach前决定retain/stop。iOS manifest启用multiple scenes且保留background audio。
- Task 4.1最终离线证据为focused `8/8`、related `160/160`、serial normal `1233/1232/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic；唯一skip仍为显式关闭的真实Keychain round-trip，普通测试继续JSON文件fallback。没有操作Simulator，该证据不证明真实macOS window restoration、iPad Stage Manager、signed/physical交互、窗口关闭策略或live Sunshine。
- Task 4.2已把scene workspace贯穿RootView navigation、LibraryDashboard和host/pairing/catalog/launch/stream surfaces；navigation、selected host/app、Add Host sheet、host destructive dialog、validation与retry均只读写传入的checked workspace。RootView不再回退`primaryWorkspaceReference`，legacy AppModel属性仅保留primary compatibility projection；stale generation mutation fail closed。
- Add Host presentation现由`ProductWorkspacePresentationState.sheet`驱动，present/dismiss只重置所属workspace draft/result且提交期间拒绝dismiss；host confirmation同时核对destructive state与workspace dialog。stream overlay沿用3.5既有workspace owner，不创建第二presentation/runtime owner。4.4 close policy、4.5 unsupported平台scene/adapter owner与4.6完整双workspace application matrix现均已完成实现和行为验收。
- Task 4.2最终离线证据为focused `5/5`、final related `161/161`、serial normal `1234/1233/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic；唯一skip仍为真实Keychain opt-in，普通测试继续文件fallback。首轮related的陈旧source断言已修正，既有visionOS timeout经定向`2/2`和fresh final related确认无回归。未操作Simulator，该证据不证明真实双窗口/Stage Manager、signed/physical交互、共享mutation reconcile、窗口关闭或live Sunshine。
- Task 4.3新增`reconcileSharedHostRepositoryState`：host load/add/remove/reset与authenticated pairing success先运行checked selection reconcile，再向所有live workspace发布host-count-derived first-use/available。只在明确trust host变化时清理选择该host的陈旧pairing，authenticated initiator owner保留到本地paired发布；navigation、sheet/dialog、draft/submission、local retry与unrelated destructive state不被覆盖。
- catalog继续使用既有owner-keyed all-workspace publish，initiator为current、其他workspace从同一apps projection得到cached；settings保持唯一process-level observable/repository值，不进入workspace state。shared reconciliation对scene/session/media/input owner零写入；inactive workspace删除非session host后，另一workspace完整`ProductSessionOwner`与active host不变并仍由原owner clean stop。
- Task 4.3最终离线证据为focused `6/6`、related `167/167`、serial normal `1240/1239/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic；唯一skip仍为真实Keychain opt-in，普通测试继续文件fallback且未操作Simulator。该项自身不证明真实双窗口/Stage Manager、signed/physical交互、live Sunshine mutation或后续4.4 close行为；4.4的独立证据列于下方。
- Task 4.4以纯`ProductWorkspaceSceneClosePolicy`和typed outcome统一scene close：stale/replaced attachment先fail closed，inactive/non-owner只detach；同workspace另一attachment或当前实际PiP/audio-only保留原owner，其他launching/streaming/reconnecting owner在detach前同步建立既有stop reservation，already-stopping加入同一operation。所有路径均不隐式transfer session owner。
- close唯一性由`ProductSessionStopOperation`与control-provider stop保证；reconnect generation replacement、pending media late-start cleanup和tvOS/visionOS presentation/input teardown仍允许既有幂等media environment cleanup多次。不得重新用active-media early return跳过完整session cleanup。
- Task 4.4最终离线证据为focused `7/7`、related `150/150`、serial normal `1246/1245/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic；唯一skip仍为真实Keychain opt-in，文件fallback继续且未操作Simulator。该证据不证明真实macOS窗口关闭/最小化、iPad Stage Manager/system PiP、signed physical background continuity或live Sunshine teardown。
- Task 4.5让tvOS继续使用单一primary `WindowGroup`、visionOS使用显式ID的单实例`Window`，且应用源码不包含`openWindow`、`dismissWindow`、`CommandGroup`或`CommandMenu`。`activeSingleWorkspacePlatformOwner(in:)`要求current primary checked reference与active session owner同时匹配后，才允许tvOS focus/remote presentation或visionOS input/presentation；active non-primary session只能沿普通checked owner clean stop，不能创建第二platform adapter owner。
- Task 4.5最终离线证据为focused `2/2`、related `310/310`、serial normal `1248/1247/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic；唯一skip仍为真实Keychain opt-in，文件fallback继续且未操作Simulator。该证据不证明真实Apple TV/Vision Pro窗口、focus/input、signed physical交互、assistive technology或live Sunshine。
- Task 4.6在同一AppModel中增加两条真实scene-attachment双workspace组合时间线：第一条覆盖local navigation/selection/sheet/draft隔离、shared catalog/settings/host更新、disconnect+serialized reconnect replacement、旧generation binding/attachment拒绝；第二条覆盖唯一session owner、non-owner overlay/stop fail closed、non-owner detach及owner close clean teardown。production与工程图零修改，不创建第二runtime owner。
- Task 4.6最终离线证据为focused `2/2`、11类related `259/259`、serial normal `1250/1249/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic，macOS为`x86_64 arm64`；唯一skip仍为真实Keychain opt-in，文件fallback继续且未操作Simulator。该证据不证明真实macOS/iPadOS同时多窗口、Stage Manager、signed/physical交互、system continuity或live Sunshine。
- Task 5.1新增`ProductSemanticDescriptor`及host、pairing、catalog、stream、settings、diagnostics六类typed surface；所有label/value/hint使用`LocalizedStringResource`，稳定ID携带closed role、enabled/in-progress/disabled reason与destructive属性，不从可见字符串或icon反推语义。
- Remove Host、Reset Trust与Stop Stream动作标记destructive，但对应status保持非destructive；stream Show/Hide Controls对无session、non-owner与stale workspace fail closed，spatial audio关闭时head tracking disabled，diagnostics区分empty/export-supported/export-unsupported和四种severity。pairing descriptor不含PIN实值，host descriptor不含address/endpoint，类型也不接收key、certificate、device identity或任意provider text。
- Task 5.1最终离线证据为focused `5/5`、13类related `238/238`、serial normal `1255/1254/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic，macOS为`x86_64 arm64`；唯一skip仍为真实Keychain opt-in，文件fallback继续且未操作Simulator。该项自身只证明typed/localized语义构造、隐私shape和平台编译；5.2至5.6现已分别完成确定性验收，但仍不证明物理VoiceOver/Voice Control、hardware keyboard/touch/remote/focus、signed安装或live Sunshine。
- Task 5.2新增pure `ProductLibraryDashboardLayout`，用actual finite container width、horizontal size class与accessibility Dynamic Type统一选择dashboard compact/wide；小于900pt、compact size class、accessibility字号或非有限宽度均fail closed为单列，macOS窄detail与iPadOS窗口不再依赖iPhone条件编译。Host四动作、pairing PIN/progress与Apps header均以`ViewThatFits`提供horizontal-first/vertical-fallback，避免压缩、隐藏或primary horizontal scroll。
- 5.2首轮并行13类related为`240/238/0/2`，两个无production对应diff的pairing/stop时序失败在fresh独立串行均`1/1`通过，随后相同13类fresh完整串行矩阵`240/240`通过；失败轮不计最终证据。最终focused为`2/2`、serial normal为`1257/1256/1/0`、四平台unsigned generic Debug `4/4`结构化零diagnostic且macOS为`x86_64 arm64`，唯一skip仍为真实Keychain opt-in，文件fallback继续且未操作Simulator。
- Task 5.2本身只证明pure layout/source合同、离线回归与平台编译；5.6现补齐longest-localization/application矩阵，但真实窗口resize、iPad Stage Manager、物理VoiceOver/Voice Control、hardware input、signed安装与live Sunshine仍需后续physical gate。
- Task 5.3新增typed `ProductKeyboardFocusPolicy`：Add Host初始聚焦Address，pairing按ready/PIN/progress/retry/result切换，owning stream overlay初始聚焦Hide Stream Controls；macOS/iPadOS的Add/Start/Submit/Retry使用default action，取消与Hide Controls使用cancel action，Settings保留本地Command-S。
- host field、Add、pairing、stream、confirmation与save主动作都获得显式accessibility label，形成稳定Voice Control名称。legacy `captureSystemShortcuts`字段继续可解码，但新默认为false，AppModel runtime policy恒false，lower adapter即使收到legacy true也拒绝Command-Q/Tab/H，Settings显示只读`Always local`状态。
- Task 5.3最终离线证据为focused `7/7`、修正后8类serial related `218/217/1/0`、独立serial normal `1259/1258/1/0`及macOS universal、iOS/iPadOS、tvOS、visionOS unsigned generic Debug `4/4`结构化零diagnostic；唯一skip仍为真实Keychain opt-in，文件fallback继续且未操作Simulator。首轮tvOS build暴露unguarded shortcut modifier后已通过平台条件编译和完整fresh 4/4重跑闭合。
- Task 5.3不证明物理hardware keyboard焦点、VoiceOver/Voice Control识别、iPad Stage Manager、signed安装或live Sunshine input；5.4提供离线touch/reduced-motion合同，5.5提供tvOS/visionOS deterministic focus/reachability合同，5.6综合矩阵现已完成，7.7物理验收继续pending。
- Task 5.4新增44pt iOS custom-action target和纵向文本扩展，PiP命令不再固定`36x32`；app name不再限制两行，navigation/app selection使用checkmark与Selected/Not selected semantic value，diagnostic severity增加固定文本。stream overlay正常使用0.18秒opacity，Reduce Motion时使用identity和nil animation；其他平台modifier为no-op，不提前改变5.5 focus geometry。
- Task 5.4最终离线证据为focused `2/2`、13类serial related `244/244`、serial normal `1261/1260/1/0`及macOS universal、iOS/iPadOS、tvOS、visionOS unsigned generic Debug `4/4`结构化零diagnostic；唯一skip仍为真实Keychain opt-in，两个真实opt-in unset且未操作Simulator。该证据不证明物理touch geometry、contrast/assistive technology、signed安装、Stage Manager或live Sunshine；5.5和5.6已有独立确定性证据，7.7继续pending。
- Task 5.5新增typed `ProductTVStreamFocusTarget`与pure policy，tvOS stream surface、overlay和controls共享一个`FocusState`；overlay在actual local-controls状态打开时先聚焦Hide Controls，关闭时只从handoff/surface状态恢复stream surface，unavailable或不匹配状态返回nil。Hide Controls仍位于Disconnect之前且保留更高accessibility sort priority，不创建第二focus/session/input owner。
- visionOS `VisionStreamControlReachability`只在visible window、matching presentation revision/input generation、actual `.local(.overlayVisible)`与nonempty current capability set同时成立时启用Hide Controls；releasing、captured、其他ineligibility、unavailable、stale/missing或空capability均disabled并公开bounded accessibility value。默认initializer保持fail-closed unavailable。
- Task 5.5最终源码候选的离线证据为focused `6/6`、11类serial related `316/316`、serial normal `1263/1262/1/0`与macOS universal、iOS/iPadOS、tvOS、visionOS unsigned generic Debug `4/4`，全部structured diagnostics为0；唯一skip仍为真实Keychain opt-in，普通测试继续文件fallback，两个真实opt-in unset且未操作Simulator。该证据不证明物理Apple TV remote focus、Vision Pro gaze/hand、VoiceOver、signed安装或live Sunshine；5.6已提供独立应用矩阵，7.7继续pending。
- Task 5.6以两条真实`AppModel`应用矩阵连接六类semantic surface、最长动态本地化文本、keyboard focus、Dynamic Type、compact/wide、44pt target、Reduce Motion、tvOS overlay focus restoration和visionOS actual overlay eligibility。visionOS只在current owner/surface/scene geometry/input generation/capability set全部匹配后，以coordinator semantic revision重建当前runtime focus eligibility投影；geometry source revision与coordinator统一revision属于不同推进域，不直接比较，stale或partial state继续fail closed。
- Task 5.6最终离线证据为focused `2/2`、14类serial related `335/335`、serial normal `1265/1264/1/0`与macOS universal、iOS/iPadOS、tvOS、visionOS unsigned generic Debug `4/4`，全部structured diagnostics为0；唯一skip仍为显式真实Keychain opt-in，普通测试继续JSON文件identity fallback，两个真实opt-in unset且未操作Simulator。该证据不证明物理VoiceOver/Voice Control、hardware keyboard/touch、Apple TV remote、Vision Pro gaze/hand、signed安装或live Sunshine；6.1与7.7继续独立推进。
- Task 6.1将closed issue扩展到28项，新增launch pairing、media presentation与platform presentation三类，并以`ProductIssueFailureMapper`只消费typed diagnostic category/action；code/summary/severity、endpoint、identity和provider payload均不进入mapper。AppModel terminal failure使用该pure mapping，launch host-not-paired产生checked session-scoped library recovery action。
- Task 6.1最终离线证据为focused `14/14`、serial related `237/237`、serial normal `1268/1267/1/0`与四平台unsigned generic Debug `4/4`，全部structured diagnostics为0且macOS universal；唯一skip仍为真实Keychain opt-in，两个真实opt-in unset且未操作Simulator。6.2继续清理observable strings，6.3-6.5 retention/export与physical/live证明保持pending。
