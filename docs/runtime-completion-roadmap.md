...
| 18 | `in_progress`，OpenSpec `15/50` | 1.1–3.3已勾选；3.3 actual tvOS Menu/unsupported native responder分流、typed local/unavailable state与no-delivery路径已通过focused、相关44项、normal、五平台Debug和repository pre-gate | 下一项3.4；controller handlers/slots/release、HDR/audio、signed与物理证据仍未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`为`15/50 ready`、next 3.4。3.3已实现actual tvOS Menu/unsupported完整UIKit begin/change/end/cancel分流、Back/Menu overlay handoff、finite typed system-owned unavailable state和Moonlight no-delivery边界；修订focused `3/3`、相关矩阵`44/44`、normal `1007/1006/1 exact Keychain skip/0`、direct tvOS/visionOS、五平台Debug与repository pre-gate均通过。3.4–3.7 controller/release以及actual display/HDR/audio、signed/physical/live证据仍未实现。
