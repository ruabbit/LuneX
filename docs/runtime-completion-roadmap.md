...
| 15 | `in_progress`，OpenSpec `24/33` | color/luminance、decoded/Metal frame、shader/readback、surface/display/resolver/presenter、macOS transition矩阵、四平台typed capability/fallback、5.1 production graph及5.2 active-session eligibility完成 | 5.3 diagnostics、5.4 application gate、5.5 status/settings UI及live/compositor/物理显示器验证仍待完成 |
...
- OpenSpec `implement-native-hdr-edr-pipeline`当前`24/33`；5.2已删除settings-derived synthetic headroom，并仅在streaming、current media/video readiness、active decoder generation及negotiated/decoded metadata一致时把source交给结合preference、platform/display capability与current headroom的resolver。下一项5.3为deduplicated privacy-bounded HDR diagnostics。
...
- settings不再合成`renderState.headroom`，reconnect先进入权威fail-closed snapshot再等待media teardown；5.2的active-session eligibility已完成。5.3 diagnostics、5.4 application integration gate、5.5 HDR status/settings UI、production compositor EDR signaling、live Sunshine HDR和物理亮度/颜色/跨显示器证据仍未完成，当前离线证据不得描述为端到端HDR显示证明。
