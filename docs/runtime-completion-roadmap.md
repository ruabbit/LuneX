...
| 16 | `in_progress`，OpenSpec `14/35` | canonical布局、session-owned spatial graph/fallback、平台策略/测试、embedded reader、entitlement配置及移动/TV/vision audio-session adapter已完成 | 尚未完成macOS route与通知/recovery/application接线、signed provisioning及音频硬件验证 |
...
- OpenSpec `integrate-spatial-audio-runtime`当前`14/35 in_progress`。1.1至1.5完成实际边界、canonical布局、resolver及有界隐私history；2.1至2.6完成immutable graph intent、显式Core Audio interleaved格式、session-owned ambience-bed graph、typed direct fallback、macOS/iOS/tvOS listener与visionOS intended-experience策略，以及schedule/readback、partial failure、stop与ARC测试矩阵。3.1新增可注入embedded entitlement reader：macOS用公开Security SecTask并只接受literal CFBoolean true；iOS/tvOS/visionOS因public SDK不暴露SecTask查询而fail closed。3.2新增generator-owned macOS/iOS/tvOS独立entitlement plist及Debug/Release build settings，visionOS/tests保持分离；四平台两配置unsigned build通过。3.3把iOS/iPadOS/tvOS/visionOS playback、多声道声明、按maximum裁剪的preferred channel、spatial port readback、停用和capability notification名称收拢到可注入adapter并接入production engine owner。下一项3.4实现macOS route/output capability。当前离线测试和unsigned build仍不证明signed provisioning、真实route notification、AirPods head tracking、声道分离或硬件可听定位。
