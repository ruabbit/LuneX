...
- Task 3.7审计九类session application合同后只发现一个缺口：第一代clean stop、第二代已streaming后，第一代迟到termination必须被完整owner/generation guard拒绝。新增测试只为默认关闭的provider double保留旧continuation，production零修改；focused为`1/1`、九类expanded matrix为`10/10`、serial normal为`1226/1225/1/0`，三份build diagnostics全零，唯一skip仍是显式关闭的真实Keychain round-trip。该离线确定性证据不证明live Sunshine、signed/physical平台或真实窗口/输入/媒体行为；4.4、5.x与7.x边界继续pending。
...
- scene root读取`supportsMultipleWindows`；unsupported配置忽略外部scene ID并继续单一checked primary，tvOS/visionOS保留普通single-workspace `WindowGroup`。detach只撤销ephemeral attachment，不close workspace、不stop或转移session；4.4 owning-window close policy继续pending。iOS manifest启用multiple scenes且保留background audio。
...
- Task 4.3最终离线证据为focused `6/6`、related `167/167`、serial normal `1240/1239/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic；唯一skip仍为真实Keychain opt-in，普通测试继续文件fallback且未操作Simulator。该证据不证明真实双窗口/Stage Manager、signed/physical交互、live Sunshine mutation或4.4 owning-window close policy。
