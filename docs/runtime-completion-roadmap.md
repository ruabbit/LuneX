...
| 18 | `in_progress`，OpenSpec `12/50` | 1.1–2.6完成；replacement/application/shared-teardown实现与focused、相关88项、normal、五平台Debug及repository gate均通过 | 下一项3.1 actual tvOS press capture；actual input/HDR/audio adapter、signed artifact与物理设备证据仍未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`当前为`12/50 ready`、next 3.1，1.1–2.6已勾选。environment在platform await前登记唯一generation terminal reservation，使双stop与provider-failure/stop竞态共享first terminal和resource report且避免provider self-join；AppModel以最高admitted surface generation/geometry revision隔离queued replacement及前后late-old state。fresh focused `3/3`、相关矩阵`88/88`、normal `999/998/1 exact Keychain skip/0`、五平台Debug和corrected repository gate均通过。actual input/display/HDR/audio adapters及signed/physical/live证据仍未实现。
