...
| 15 | `in_progress`，OpenSpec `22/33` | color/luminance、decoded/Metal frame、shader/readback、surface/display/resolver/presenter、macOS transition矩阵及四平台typed capability/fallback完成 | 5.x生产接线/诊断及物理显示器验证仍待完成 |
...
- OpenSpec `implement-native-hdr-edr-pipeline`当前`22/33`；4.6的iOS/iPadOS、tvOS和visionOS compile-safe capability adapters已完成，下一项5.1连接production application/media/presentation/render graph。
