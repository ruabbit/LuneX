...
- scene root读取`supportsMultipleWindows`；unsupported配置忽略外部scene ID并继续单一checked primary，tvOS/visionOS保留普通single-workspace `WindowGroup`。4.1的底层coordinator detach只撤销ephemeral attachment且不close registry或transfer session；4.4现由AppModel在调用detach前决定retain/stop。iOS manifest启用multiple scenes且保留background audio。
