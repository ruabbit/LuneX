...
- OpenSpec `integrate-tvos-visionos-runtime`为`14/50 ready`、next 3.3。3.2已实现SwiftUI navigation/workspace/overlay与current press owner的同步local handoff、fresh `(surface generation, semantic revision)` focus和terminal clear恢复controls；修订fresh focused `5/5`、相关矩阵`43/43`、normal `1006/1005/1 exact Keychain skip/0`、五平台Debug与repository pre-gate均通过。3.3–3.7 reserved/controller/release以及actual display/HDR/audio、signed/physical/live证据仍未实现。
