...
| 17 | `in_progress`，OpenSpec `29/36` | actual UIKit scene/window/geometry/input、actual-window mobile EDR、sample-buffer PiP runtime、actual-state continuity policy、serialized mobile media owner、media environment/AppModel application与bounded diagnostics、stop/failure/replacement清理、iPhone/iPad单值`audio` source/project/unsigned built-plist配置、5.5 actual-state UI/PiP命令/migration，以及5.6跨层/UI/persistence完整确定性回归均已完成 | 6.x normal、Debug/Release、repository、sanitizer/resource、fixed-simulator与signed/physical PiP/background/Stage Manager/EDR/live Sunshine证明尚未完成 |
