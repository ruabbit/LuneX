...
| 14 | `in_progress`，OpenSpec `15/29` | 完成AppKit合同、共享坐标、闭合directive、generation-scoped lifecycle/application input sink、bounded coordinator、balanced cursor owner和keyboard/modifier capture | 尚未实现pointer/button/scroll capture，也未把capture/cursor/lifecycle接入active stream surface |
