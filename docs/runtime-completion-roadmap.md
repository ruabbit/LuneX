...
| 18 | `in_progress`，OpenSpec `16/50` | 1.1–3.4已勾选；3.4 generation-owned controller handlers、bounded slots、fresh leases、complete roster与AppModel application已通过focused、相关85项、normal、五平台Debug及repository pre-gate | 下一项3.5 routing/feedback；3.6 release、HDR/audio、signed与物理证据仍未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`为`16/50 ready`、next 3.5。3.4已实现actual tvOS main-actor extended/micro handlers、最低空闲16-slot分配、同input generation单调fresh controller lease、完整有限roster、disconnect replacement、handler cleanup，以及AppModel current-generation lease application；same-source-revision只允许snapshot相同的lease-only变化。focused `5/5`、相关矩阵`85/85`、normal `1011/1010/1 exact Keychain skip/0`、direct tvOS/visionOS、五平台Debug和repository pre-gate均通过。3.5 routing/feedback、3.6 ordered release以及actual display/HDR/audio、signed/physical/live证据仍未实现。
