...
| 18 | `in_progress`，OpenSpec `3/50` | 1.1 baseline、1.2 immutable checked platform snapshot，以及1.3 tvOS local-focus/stream-capture、reserved command、balanced press、16-slot controller roster/feedback与ordered release纯值合同完成；1.3 focused `16/16`、normal `938/937/1/0`、五平台Debug和repository门通过 | 下一项1.4 visionOS windowed/system-reserved/focus-release/immersive-unavailable合同；仍无actual handler、generation-owned platform owner、输入/scene/media接线或物理设备证据 |
...
- OpenSpec `integrate-tvos-visionos-runtime`当前`3/50 ready`，下一项1.4。1.2建立checked generation/revision、finite geometry/drawable、scene/surface/focus/input、16-slot controller lease、display headroom source、audio route与aggregate consistency；1.3建立local-focus/stream-capture分权、system-reserved command、balanced press token、完整controller roster/feedback和严格ordered release effect。两者都不持有框架对象或连接actual platform runtime。
