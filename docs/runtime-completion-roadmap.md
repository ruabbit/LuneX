...
| 18 | `in_progress`，OpenSpec `7/50` | 1.1–2.1完成；actual tvOS/visionOS surface raw callback bridge、focused/normal、五平台Debug与repository gate已通过 | 下一项2.2 generation owner；仍无normalized geometry、输入/scene/media接线、signed artifact或物理设备证据 |
...
- OpenSpec `integrate-tvos-visionos-runtime`当前`7/50 ready`，1.1–2.1完成；独立于iOS mobile pipeline的actual tvOS/visionOS UIKit Metal surface raw callback bridge、focused `2/2`、normal `963/962/1 exact Keychain skip/0`、五平台Debug及repository pre-gate均通过。下一项2.2实现checked generation owner；2.3 normalized geometry及后续application/media/input接线仍未实现。
