...
| 19 | `in_progress`，OpenSpec `complete-native-product-workflows` | host/pairing/catalog/session恢复、多窗口ownership、typed accessibility semantics、adaptive layout及macOS/iPadOS keyboard/Voice Control/system-shortcut合同已有确定性实现与任务级离线验收 | 5.4以后touch target/text/state/reduced-motion、tvOS/visionOS focus、diagnostics export、跨产品回归及signed/physical/live验收仍未完成 |
...
- Task 5.3不证明物理hardware keyboard焦点、VoiceOver/Voice Control识别、iPad Stage Manager、signed安装或live Sunshine input；5.4 touch/reduced motion、5.5 tvOS/visionOS focus、5.6综合矩阵和7.7物理验收继续pending。
