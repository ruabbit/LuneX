...
| 18 | `in_progress`，OpenSpec `10/50` | 1.1–2.4完成；serialized presentation coordinator已通过修订focused、normal、五平台Debug与final repository gate | 下一项2.5 media/AppModel接线；actual input/HDR/audio adapter、signed artifact与物理设备证据仍未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`当前`11/50 ready`，1.1–2.5完成。2.5让每个current media generation在`NativeSessionMediaEnvironment`中拥有唯一presentation coordinator和现有frame-source subscription，并通过typed event把current/terminal state送入`AppModel`；actual geometry串行应用，reconnect、remote termination、local stop与failure均清除actual accessor并隔离旧ownership。fresh focused `8/8`、normal `996/995/1 exact Keychain skip/0`、五平台Debug和pre-mark repository gate通过；下一项2.6扩充application ordering、late callback及幂等teardown测试。actual input/display/HDR/audio adapters及signed/physical/live证据仍未实现。
