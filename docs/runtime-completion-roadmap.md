...
| 18 | `in_progress`，OpenSpec `19/50` | 1.1–3.7已勾选；完成focus/Menu本地边界、supported Siri Remote到既有keyboard wire的canonical mapping、resolved held release、stale callback与完整236项相关回归；focused/normal/direct tvOS/五平台Debug及repository/final-state均已通过 | 4.x–8.x tvOS/visionOS media、产品、release gates及signed/物理/live证据仍未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`当前为`19/50 ready`、next 4.1。3.7已修复legacy adapter的Menu/focus本地边界，并在existing provider resolver中把方向/Select/PlayPause归约为Win32 arrows/Return/media PlayPause canonical keyboard events；映射事件复用既有wire codec与held release barrier，不新增tvOS协议。focused `7/7`、相关`236/236`、normal `1027/1026/1 exact Keychain skip/0`、direct fixed tvOS和五平台Debug均结构化零诊断通过，repository pre-gate与post-mark final-state通过。actual tvOS display/render/HDR/audio及signed/physical/live证据仍未完成。
