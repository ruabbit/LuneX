...
| 17 | `in_progress`，OpenSpec `8/36` | scene/window、PiP/continuity、mobile EDR值合同与确定性矩阵完成；iOS `MTKView`已有callback边界、current-generation actual `UIWindow`/`UIWindowScene`/`UIScreen` owner和attached-scene lifecycle identity filtering/cancellation | 尚无实际geometry/Stage Manager、drawable/input绑定、PiP content source、合法后台保活或移动EDR运行接线 |
