...
| 17 | `in_progress`，OpenSpec `25/36` | actual UIKit scene/window/geometry/input、actual-window mobile EDR、sample-buffer PiP runtime、actual-state continuity policy，以及generation/revision-scoped FIFO serialized mobile media owner均完成离线实现与回归 | serialized owner尚未接入media environment/AppModel；尚无生成配置验收、actual-state UI、固定simulator及物理PiP/background/Stage Manager/EDR/live Sunshine证明 |
...
- OpenSpec `integrate-mobile-scene-pip-continuity`当前`25/36 in_progress`；1.x合同、2.x actual UIKit scene/window/geometry/input、3.x actual-window mobile EDR、4.x sample-buffer PiP runtime、5.1 actual-state continuity policy和5.2 serialized mobile media generation owner均已完成。PiP通过最多8个generation-filtered订阅和单槽main-actor mailbox复用唯一presentation source及同一`CVPixelBuffer`，不创建第二decoder；continuity action只接受generation-matched native-confirmed PiP+operational sink或active/permitted audio。5.2以FIFO gate统一foreground、PiP/audio-only、pause/stop、replacement、失败回滚和exactly-once foreground resample的纯值动作，但尚未接入`NativeSessionMediaEnvironment`或`AppModel`。下一项5.3生成并验证窄化的iOS/iPadOS playback background-mode配置，同时保持配置、签名接受和runtime行为三类证据分离。
