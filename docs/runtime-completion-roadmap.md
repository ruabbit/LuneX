...
| 17 | `in_progress`，OpenSpec `32/36` | actual UIKit scene/window/geometry/input、actual-window mobile EDR、sample-buffer PiP runtime、actual-state continuity policy、serialized mobile media owner、media environment/AppModel application与bounded diagnostics、stop/failure/replacement清理、iPhone/iPad单值`audio` source/project/unsigned built-plist配置、5.5 actual-state UI/PiP命令/migration、5.6跨层/UI/persistence完整确定性回归、6.1 normal、6.2十配置build，以及6.3 strict/generator/membership/clean-room/license/plist/privacy/API/four-SDK/analyzer门均已完成 | 6.4–6.7 sanitizer/resource、fixed-simulator与signed/physical PiP/background/Stage Manager/EDR/live Sunshine证明尚未完成；固定ENet保留4项已归属analyzer finding |
...
- OpenSpec `integrate-mobile-scene-pip-continuity`当前`32/36 in_progress`；1.x至5.6及6.1–6.3均已完成。完整normal为`909/908/1/0`且唯一skip是显式真实Keychain用例；6.2十配置build、Metal/plist/simulator不变门通过。6.3 repository/API证据`/tmp/LuneX-17-6_3-repository.NHQpyc`通过strict `8/8`、generator/membership、clean-room/reference/package、ENet pin/license/逐文件一致、plist/privacy、iOS public API probe及四SDK strict compile；Debug/Release analyzer `/tmp/LuneX-17-6_3-analyzer.ZbHqMU`均成功且first-party finding为0，固定ENet各有相同4项finding并保留为依赖残余风险。下一项6.4为sanitizer/resource gates；unsigned/static证据仍不证明签名接受、system PiP、background duration、Stage Manager、visible EDR或live Sunshine。
