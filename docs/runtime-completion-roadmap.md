...
| 15 | `in_progress`，OpenSpec `1/33` | 已完成production/API/platform/hardware证明边界inventory；已保留bit depth/colorspace/MDCV/CLL、请求8/10-bit VideoToolbox格式、映射8/10-bit Metal plane并读取display headroom | actual presenter仍固定Core Image/sRGB；尚无显式color contract、PQ/EDR mapping、float surface、跨屏revision或物理显示器验证 |
...
- OpenSpec `implement-native-hdr-edr-pipeline`已创建并通过strict validation，含`hdr-video-output-contract`、`edr-tone-mapping`和`hdr-display-adaptation`三项capability及33个任务；1.1 inventory已固化在`docs/runtime/hdr-edr-contract.md`，下一项1.2定义immutable render/display/surface value contract。
