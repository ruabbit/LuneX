...
| 15 | `in_progress`，OpenSpec `23/33` | color/luminance、decoded/Metal frame、shader/readback、surface/display/resolver/presenter、macOS transition矩阵、四平台typed capability/fallback及5.1 production graph完成 | 5.2 eligibility、5.3 diagnostics、后续application gate/UI及物理显示器验证仍待完成 |
...
- OpenSpec `implement-native-hdr-edr-pipeline`当前`23/33`；5.1已连接production application/media/presentation/render graph，下一项5.2从active session、preference、valid source、platform/display capability和current headroom完整派生eligibility并移除synthetic settings fallback。
...
- legacy settings-derived `renderState.headroom`仍存在但5.1 resolver不使用它冒充display snapshot；5.2负责移除该synthetic eligibility fallback并收束完整active-session eligibility。5.3 diagnostics、5.4 application integration、production compositor EDR signaling、live Sunshine HDR和物理亮度/颜色仍未完成，当前离线证据不得描述为端到端HDR显示证明。
