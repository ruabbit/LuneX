...
| 18 | `in_progress`，OpenSpec `17/50` | 1.1–3.5已勾选；3.5 atomic existing-registry routing、exact 16-slot complete state、opaque current-lease feedback/motion与actual tvOS haptics/light cleanup已通过focused、相关170项、normal、direct tvOS、五平台Debug及repository pre-gate | 下一项3.6 ordered release；HDR/audio、signed与物理证据仍未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`为`17/50 ready`、next 3.6。3.5已把complete controller roster原子路由进existing `RemoteControllerRegistry`，保留exact `0...15` slot、完整held state与invalid replacement rollback；AppModel串行reconcile并仅把feedback和bounded latest motion应用到current opaque lease。actual tvOS owner按公开能力应用Core Haptics rumble/trigger、`GCDeviceLight`与finite motion rate，stop/disconnect恢复原profile handler并清理部分启动失败。最终focused `5/5`、相关矩阵`170/170`、normal `1015/1014/1 exact Keychain skip/0`、direct tvOS、五平台Debug与repository pre-gate均通过。3.6 ordered release以及actual display/HDR/audio、signed/physical/live证据仍未完成。
