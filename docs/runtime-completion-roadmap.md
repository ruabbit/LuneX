...
| 16 | `in_progress`，OpenSpec `7/35` | canonical布局、resolver/history、纯值graph intent/actual snapshot及显式layout interleaved Int16格式贯穿engine、PCM buffer和rebuild；单一buffer ownership fail closed | 尚无session-owned environment graph、实际`isListenerHeadTrackingEnabled`接线、entitlement/route硬件验证 |
...
- OpenSpec `integrate-spatial-audio-runtime`当前`7/35 in_progress`。1.1至1.5完成实际边界、canonical布局、resolver及有界隐私history；2.1以不可变`SpatialAudioGraphIntent`把revision、平台、route、entitlement和偏好传入injectable engine client并返回actual `SpatialAudioRuntimeSnapshot`。2.2由共享factory为mono/stereo/WAVE 5.1/WAVE 7.1建立显式Core Audio layout的interleaved Int16 `AVAudioFormat`，production connection与PCM buffer复用同一格式，并在copy前要求一个buffer、exact channel count与exact byte ownership。production现仍只诚实报告nonspatial mixer；下一项2.3建立session-owned environment graph。这些离线合同不证明environment graph、route、entitlement或头部跟踪。
