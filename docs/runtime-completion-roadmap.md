...
| 17 | `in_progress`，OpenSpec `28/36` | actual UIKit scene/window/geometry/input、actual-window mobile EDR、sample-buffer PiP runtime、actual-state continuity policy、serialized mobile media owner、media environment/AppModel application与bounded diagnostics、stop/failure/replacement清理、iPhone/iPad单值`audio` source/project/unsigned built-plist配置，以及5.5 actual-state UI、PiP命令与migration均已完成 | 5.6完整回归与6.x signed/simulator/physical PiP/background/Stage Manager/EDR/live Sunshine证明尚未完成 |
