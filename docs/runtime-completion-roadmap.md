...
| 18 | `in_progress`，OpenSpec `42/50 ready` | 1.1–7.5已完成task级离线验收；7.5本地化资源、compact/wide与辅助功能字号布局、tvOS focus/command顺序、visionOS current synchronized projection、partial/replacement/stale fail-closed、migration和clean-stop应用回归及repository pre-gate已通过 | next 8.1；8.x、Simulator runtime、signed/物理/live HDR/audio/input及性能功耗证据未完成 |
