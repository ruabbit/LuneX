...
| 15 | `in_progress`，OpenSpec `29/33` | color/luminance、decoded/Metal frame、shader/readback、surface/display/resolver/presenter、macOS transition矩阵、四平台typed capability/fallback、5.1–5.5 integration、6.1 normal tests及6.2五平台Debug/Release构建完成 | 6.3–6.6质量/模拟器/硬件/跟踪、live/compositor与物理显示器证据仍待完成 |
...
- OpenSpec `implement-native-hdr-edr-pipeline`当前`29/33`；6.2已完成macOS、固定iPhone/iPad/tvOS/visionOS的Debug/Release共10个隔离warnings-as-errors构建，全部零structured diagnostics并各生成一个AIR/metallib。下一项6.3为strict/generator/clean-room/dependency/Metal/analyzer/sanitizer/malloc/resource-release综合门禁。
