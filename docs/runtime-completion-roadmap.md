...
| 19 | `in_progress`，OpenSpec `complete-native-product-workflows` | host/pairing/catalog/session恢复、多窗口ownership、typed semantics、adaptive layout、keyboard/Voice Control、iOS touch/text/non-color state及Reduce Motion、tvOS focus restoration与visionOS actual-input reachability合同已有确定性实现和任务级离线验收 | 5.6完整accessibility matrix、diagnostics export、跨产品回归及signed/physical/live验收仍未完成 |
...
- Task 5.1最终离线证据为focused `5/5`、13类related `238/238`、serial normal `1255/1254/1/0`与四平台unsigned generic Debug `4/4`结构化零diagnostic，macOS为`x86_64 arm64`；唯一skip仍为真实Keychain opt-in，文件fallback继续且未操作Simulator。该项只证明typed/localized语义构造、隐私shape和平台编译，不证明SwiftUI accessibility modifier、物理VoiceOver/Voice Control、hardware keyboard/touch/remote/focus、signed安装或live Sunshine；5.2已完成adaptive layout，5.3至5.6仍独立推进。
...
- Task 5.2只证明pure layout/source合同、离线回归与平台编译；不证明真实窗口resize、iPad Stage Manager、完整longest-localization/snapshot矩阵、物理VoiceOver/Voice Control、hardware input、signed安装或live Sunshine。5.3至5.6及后续physical gate继续pending。
...
- Task 5.3不证明物理hardware keyboard焦点、VoiceOver/Voice Control识别、iPad Stage Manager、signed安装或live Sunshine input；5.4现提供离线touch/reduced-motion合同，5.5现提供tvOS/visionOS deterministic focus/reachability合同，5.6综合矩阵和7.7物理验收继续pending。
