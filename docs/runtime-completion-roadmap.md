...
| 14 | `in_progress`，OpenSpec `4/29` | 已有AppKit window通知和平台input adapter类型；完成AppKit合同、immutable共享snapshot及fit/fill/backing/resize/stale完整确定性矩阵 | 尚未把闭合生命周期directive、真实`NSEvent`/cursor capture/focus release接入active remote input与decoder生命周期 |
