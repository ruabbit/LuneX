...
| 18 | `in_progress`，OpenSpec目标 `33/50 ready` | 1.1–6.2已实现并完成行为/build审计；6.2把visionOS current window frame/revision/replacement/clear-resume/stale rejection接入共享owner，并用64-pending单consumer FIFO与typed overflow failure修复delivery Task乱序和无界增长 | next 6.3；6.3–8.x及Simulator runtime/signed/物理/live HDR/audio证据未完成 |
