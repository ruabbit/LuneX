...
- scene root读取`supportsMultipleWindows`；unsupported配置忽略外部scene ID并继续单一checked primary，tvOS/visionOS保留普通single-workspace `WindowGroup`。detach只撤销ephemeral attachment，不close workspace、不stop或转移session；4.2完整per-scene UI binding与4.4 owning-window close policy继续pending。iOS manifest启用multiple scenes且保留background audio。
