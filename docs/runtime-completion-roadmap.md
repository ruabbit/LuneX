...
| 18 | `in_progress`，OpenSpec pre-mark `40/50 ready` | 1.1–7.3已完成；7.4已在repository gate前接入单一有限DiagnosticsStore、opaque replacement lease、semantic revision dedup、owned recovery、二次脱敏export、AppModel current-owner clear路径与tvOS typed unavailable UI，并通过focused/related/normal、fixed tvOS/visionOS及五平台unsigned Debug | gate通过后推进到41/50 next 7.5；7.5、8.x、Simulator runtime、signed/物理/live HDR/audio/input及性能功耗证据未完成 |
