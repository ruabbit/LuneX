...
| 18 | `in_progress`，OpenSpec `32/50 ready` | 1.1–6.1已勾选；6.1已实现current attached visionOS scene派生的`.windowed` actual state、全量typed immersive/passthrough/stereoscopic/volumetric unavailable、跨组件revision重标及replacement/terminal清理，并通过fresh test/build/repository/final-state验收 | next 6.2；6.2–8.x及Simulator runtime/signed/物理/live HDR/audio证据未完成 |
