...
- OpenSpec `integrate-mobile-scene-pip-continuity`当前`27/36 in_progress`；1.x合同、2.x actual UIKit scene/window/geometry/input、3.x actual-window mobile EDR、4.x sample-buffer PiP runtime和5.1至5.4均已完成。5.4以generation/revision-scoped application把scene/geometry/PiP/continuity/mobile EDR与actual audio state接入`NativeSessionMediaEnvironment`/`AppModel`，production action client实际下发video/audio/control与input release，stop/failure/replacement共享完整teardown并清空actual state，diagnostics保持有界私密。最终离线门为focused `18/18`、expanded `301/301`、full `898/897/1/0`和四generic Debug零结构化诊断；普通测试未启用真实Keychain且没有操作simulator。下一项5.5实现accessible PiP commands、actual status、continuity settings和compact/wide UI；unsigned产物仍不证明签名接受、system PiP、background duration、Stage Manager、visible EDR或live Sunshine。
