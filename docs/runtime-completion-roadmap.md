...
| 14 | `in_progress`，OpenSpec `5/29` | 完成AppKit合同、immutable共享snapshot、完整坐标矩阵与闭合render/video/presentation/input lifecycle directive | 尚未按session generation应用directive，也未把真实`NSEvent`/cursor capture/focus release接入active remote input与decoder生命周期 |
