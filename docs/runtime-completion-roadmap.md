...
| 14 | `in_progress`，OpenSpec `20/29` | 完成AppKit合同、共享坐标、闭合directive、generation-scoped lifecycle、AppModel/media application、active input coordinator、balanced cursor owner、actual capture surface、responder/dismantle，以及stream-view backing/display/live-resize检测 | persisted capture/cursor eligibility、diagnostics、完整application gate与live hardware证明尚未完成 |
...
- `MacStreamInputCaptureView`现直接继承`MTKView`并成为SwiftUI实际Metal stream surface；surface attachment owner跟随真实window attach/detach，清理callback、transient input、Metal delegate与presentation pause。共享attachment lease阻止旧coordinator迟到dismantle清除replacement lifecycle。actual stream-view backing geometry和active-session input handler wiring已经完成；input admission与cursor eligibility仍默认关闭，留给5.3按当前session、focus、visibility和持久化设置派生。
