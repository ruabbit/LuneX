...
| 18 | `in_progress`，OpenSpec `13/50` | 1.1–3.1已勾选；3.1 actual tvOS begin/end/cancel、current-surface admission、balanced/FIFO/fail-closed input path已通过focused、相关41项、normal、五平台Debug与repository pre-gate | 下一项3.2；3.2–3.7 focus/overlay/reserved/controller/release、HDR/audio、signed与物理证据仍未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`为`13/50 ready`、next 3.2。3.1已把actual tvOS surface `pressesBegan/Ended/Cancelled`接到current surface/input generation owner与既有session input application；Menu/unsupported保持UIKit local，replacement/late callback、FIFO balanced delivery和failure release/queued-down suppression均有确定性覆盖。fresh focused `5/5`、相关矩阵`41/41`、normal `1004/1003/1 exact Keychain skip/0`、五平台Debug零诊断和repository pre-gate通过。3.2–3.7 focus/overlay/reserved/controller/release及actual display/HDR/audio、signed/physical/live证据仍未实现。
