...
| 16 | `in_progress`，OpenSpec `10/35` | canonical布局、显式interleaved格式、session-owned ambience-bed graph、typed direct-mixer fallback及分平台listener/visionOS intended-experience策略已接线 | 尚未完成2.6完整graph/resource测试、3.x entitlement/route adapters、签名与音频硬件验证 |
