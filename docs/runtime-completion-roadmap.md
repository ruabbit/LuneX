...
| 18 | `in_progress`，OpenSpec `20/50` | 1.1–4.1已勾选；production coordinator准入的current decoded frame已接到actual Metal presenter，关闭shared-latest bypass并覆盖scene/surface/revision/clear/rebind/replacement；fresh `8/8` focused、`241/241` related、`1035/1034/1/0` normal、direct tvOS、unsigned五平台Debug、repository pre-gate及只读final-state均通过 | 4.2–8.x display/HDR、audio、visionOS、产品、release及signed/物理/live证据未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`当前为`20/50 ready`、next 4.2。3.7的remote/focus/controller边界保持完成；4.1新增同一个`TVVisionMetalPresentationOwner`作为production coordinator与actual SwiftUI Metal surface的rendezvous，presenter进入platform admission后不再回退shared latest frame，并以session/media/presentation/input/surface/decoder/frame/revision fence完成clear/rebind/replacement。fresh focused `8/8`、related `241/241`、normal `1035/1034/1 exact Keychain skip/0`、direct fixed tvOS、unsigned五平台Debug、repository pre-gate `/tmp/LuneX-18-4_1-repository-pre.cIhA16` 与只读final-state `/tmp/LuneX-18-4_1-final-state-r2.BFSidM` 均通过。actual tvOS display/HDR与audio仍分别属于4.2–4.4，signed/physical/live证据未完成。
- tvOS 26.4旧`wantsExtendedDynamicRangeContent`/`edrMetadata`不可用，新`CALayer.preferredDynamicRange`、`contentsHeadroom`和`toneMapMode`可编译，actual scene screen headroom可读；4.2仍须以实际有限headroom和物理电视证明决定direct HDR。visionOS旧Metal EDR属性与新CALayer路径可编译，但`UIScreen`/`UIWindowScene.screen`不可用，不能虚构current headroom，6.3仍保持typed fallback直到公开有限来源成立。
