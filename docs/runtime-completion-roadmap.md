...
| 18 | `in_progress`，OpenSpec pre-mark `33/50 ready` | 1.1–6.3实现与行为/build审计已完成到repository gate前；6.3以actual visionOS Metal layer-only observer、平台互斥resolution和共享resolver稳定发布`.headroomUnavailable` HDR-to-SDR，不伪造screen/current headroom | gate通过后推进到34/50 next 6.4；6.4–8.x及AppModel组合接线、Simulator runtime/signed/物理/live HDR/audio证据未完成 |
