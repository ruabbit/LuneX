...
| 18 | `in_progress`，OpenSpec `18/50` | 1.1–3.6已勾选；3.6完整effect FIFO、per-operation release accounting、admission intent fence、controller quiesce、existing provider barrier、local restoration与terminal join已通过focused 37项、相关233项、normal、direct tvOS、五平台Debug及repository pre-gate | 下一项3.7完整回归；HDR/audio、signed与物理证据仍未完成 |
...
- OpenSpec `integrate-tvos-visionos-runtime`为`18/50 ready`、next 3.7。3.6把surface owner的完整effect FIFO接到AppModel：按operation UUID跟踪并发release，UUID admission intent阻止A→B→C stale open和revision回绕碰撞，controller handlers/roster/routing/motion先quiesce，再加入existing provider held-state barrier，最后恢复local overlay/focus；replacement admission与provider failure均按current effect generation fail closed，stop/reconnect/remote termination/media failure重复join同一FIFO。最终focused `37/37`、相关矩阵`233/233`、normal `1024/1023/1 exact Keychain skip/0`、direct fixed tvOS、五平台Debug和repository pre-gate均通过。下一项3.7完整回归以及actual display/HDR/audio、signed/physical/live证据仍未完成。
