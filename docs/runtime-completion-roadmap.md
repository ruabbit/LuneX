...
| 17 | `in_progress`，OpenSpec `30/36` | actual UIKit scene/window/geometry/input、actual-window mobile EDR、sample-buffer PiP runtime、actual-state continuity policy、serialized mobile media owner、media environment/AppModel application与bounded diagnostics、stop/failure/replacement清理、iPhone/iPad单值`audio` source/project/unsigned built-plist配置、5.5 actual-state UI/PiP命令/migration、5.6跨层/UI/persistence完整确定性回归，以及6.1 normal/唯一Keychain skip门均已完成 | 6.2–6.7 Debug/Release、repository、sanitizer/resource、fixed-simulator与signed/physical PiP/background/Stage Manager/EDR/live Sunshine证明尚未完成 |
...
- OpenSpec `integrate-mobile-scene-pip-continuity`当前`28/36 in_progress`；1.x至5.5均已完成。5.4把generation/revision-scoped scene/geometry/PiP/continuity/mobile EDR与actual audio state接入`NativeSessionMediaEnvironment`/`AppModel`并闭合action/teardown；5.5新增typed actual-state projection、current-generation PiP start/stop bridge、actual scene/PiP/background/mobile-EDR stream/Settings UI、compact/wide与accessibility/localization-safe布局，以及缺失/部分continuity preference迁移。5.5最终门为focused `9/9`、expanded `220/219/1/0`、fresh full `906/905/1/0`、四generic Debug零结构化诊断和repository pre-gate通过；普通测试未启用真实Keychain且没有查询或操作simulator。下一项5.6扩展policy-loss/audio-only/active-PiP/foreground/replacement/diagnostic/UI/accessibility/localization/migration/clean-stop回归；unsigned产物仍不证明签名接受、system PiP、background duration、Stage Manager、visible EDR或live Sunshine。
