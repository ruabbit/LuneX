...
| C. Video network receive | Provider contract, Network.framework channel, packet parser/assembly, decoder and presenter exist | Deterministic components only | No concrete `VideoReceiveProvider` is installed by `ProductionRuntimeProviderFactory.makeDefault()` | M1 |
| D. Audio network receive | Provider contract, network channel, jitter/Opus/PCM/audio graph exist | Deterministic components only | No concrete `AudioReceiveProvider` is installed by the default production runtime | M1 |
