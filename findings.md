...
## 2026-08-31 macOS source/display-aware Metal pacing

- The existing AppKit lifecycle monitor already observes window screen changes, backing-property changes, `NSApplication.didChangeScreenParametersNotification`, and actual surface geometry. Extending that state with the attached `NSScreen.maximumFramesPerSecond` keeps pacing generation-owned by the same current-surface attachment and prevents stale-window notifications from overwriting a replacement surface.
- The authoritative source cadence is `NegotiatedSessionConfiguration.video.frameRate` after the media environment has started successfully, not the mutable settings value. It must be cleared with active video presentation ownership so reconnect/stop cannot leak a prior session cadence.
- Active macOS scheduling should resolve to `min(valid negotiated source fps, valid attached-display maximum fps)`, with the existing 60 fps fallback while either value is unavailable. Throttled remains 15 fps; idle/paused remains paused with one immediate clear. This removes the 60 Hz ceiling for the observed 144 fps session while avoiding a request above the current display capability.
- Moonlight-Qt's native Metal renderer gives `CAMetalDisplayLink` the negotiated source frame rate and requests one-frame latency. This batch implements the cadence half through the existing MTKView lifecycle; `CAMetalDisplayLink.preferredFrameLatency = 1` remains a separate follow-up so its effect can be measured independently.
## 2026-08-31 macOS realtime live discriminator after source/display pacing

- The pushed `aee855e` Debug product is running as exactly one process (`9457`) from `build/DerivedData/Build/Products/Debug/LuneX-macOS.app`; `HEAD == origin/main` and no `xcodebuild`/`xctest` process is active.
- One Computer Use double-click of the existing `Desktop` catalog item entered a sustained live session at `2560x1440 @ 144 fps - 100 Mbps`; no second product process, AppleScript/JXA/CGEvent automation, Simulator action, Keychain test, or Sunshine mutation was used.
- The first on-demand Diagnostics snapshot was collected after the Diagnostics window became key, so the lifecycle correctly reported `Throttled` and `144 source / 144 display / 15 requested`. Its cumulative realtime discriminator read: macOS input `0 queued / 0 rejected / 0 dropped`, remote input `0 pending / 0 rejected`, ENet `2.1 ms max queue`, video receive `1256 obsolete events discarded`, video presentation `8021 published / 5661 presented / 2354 superseded`, and latest-frame age/presentation `11.0 ms old / 3.1 ms current / 13.3 ms max`.
- The low input/control delay and zero input loss do not support an input-backlog explanation for the current idle snapshot. The presentation supersede count is material, but it includes intentional 15 fps unfocused throttling while the 144 fps receive/decode path continues, so it cannot yet be attributed to foreground display pacing. The next live check must compare counter deltas around foreground Direct/Relative gestures rather than interpreting cumulative totals.
- A bounded foreground check then sent two harmless Direct clicks and two vertical Relative drags through Computer Use. Relative capture remained available, the mode switched successfully, and the post-gesture snapshot reported exactly `1 coalesced`, `0 rejected`, `0 dropped`, `4.8 ms` maximum macOS capture-queue age, `5.2 ms` maximum sink delivery, and `5.4 ms` maximum ENet queue delay. These figures reject the earlier input-FIFO/backlog failure mode for this run, though Computer Use screenshots cannot prove the remote cursor's physical vertical direction or subjective pointer smoothness.
- Over the same live interval, cumulative video changed from `8021 published / 5661 presented / 2354 superseded` to `15721 / 6740 / 8948`; latest-frame age remained `4.6 ms` and current/maximum publish-to-present remained `4.2/14.5 ms`. Diagnostics focus still adds intentional 15 fps throttling at the end of the interval, but the large replacement-to-presentation disparity together with low frame age points to scheduling/pacing mismatch rather than stale decode output. The next implementation batch should replace the timer-driven Metal draw request with display-linked scheduling and a one-frame latency budget, following Moonlight-Qt's native Metal policy.
- The resulting macOS owner binds exactly one `CAMetalDisplayLink` to the current `CAMetalLayer`, uses the callback-provided drawable synchronously on the registered main run loop, applies exact active/throttled cadence plus `preferredFrameLatency = 1`, pauses before idle/teardown, and invalidates before releasing the presenter/view. If a display-link runtime cannot attach, only then does scheduling fall back to MTKView.
- Presenter state is snapshotted before either the legacy provider or callback drawable is read, and the presentation revision is checked again before submission. This preserves the existing display-move-during-draw stale-plan rejection while eliminating an extra `currentDrawable` acquisition from the display-link path.
- Fresh focused R5 `/tmp/LuneX-MetalDisplayLink-Focused-R5.xcresult` passed `116/116` with zero skips/failures and zero build errors, warnings, or analyzer warnings. It covers the source/display cadence matrix, one-frame latency, display-link pause/fallback/invalidation, AppKit lifecycle and Relative Y conversion, stale display transitions, HDR/EDR surface replacement, and presenter teardown.
- The final deterministic matrix now covers the complete macOS suite (`1383/1381/2/0`) plus the shared iOS/iPadOS, tvOS, and visionOS generic compilation surface with zero structured diagnostics. This closes implementation and regression risk for the display-link batch but does not close the physical-performance rows.
- The remaining proof boundary is live and comparative: a fresh session on the pushed binary must measure foreground `published/presented/superseded` deltas and actual Direct/Relative/click behavior. The user's host-visible Relative direction, subjective cursor smoothness, complete physical input matrix, and measurable performance thresholds remain pending.
- The first pushed-product attempt exposed a separate macOS 27 Observation crash before media startup. Both incident reports (`075152` PID `15431` and auto-relaunched `075203` PID `15740`) have the same main-thread stack: `ObservationTracking._AccessList.addAccess` while `launchSelectedApp` revalidates `activeProductSessionOwner` after `streamSessionCoordinator.prepare` suspends. No display-link frame callback appears on the faulting stack; the second report only shows CoreVideo's normal waiting thread.
- The macOS catalog starts launch from SwiftUI gestures using an unstructured `Task`. A detached task with an explicitly MainActor-isolated body preserves all AppModel actor ownership and launch serialization while preventing inherited task-local Observation tracking from surviving the suspension boundary. This is narrower than hiding model state from Observation or weakening the post-prepare ownership revalidation.
- The first catalog-observation focused invocation used a nonexistent method selector, so `/tmp/LuneX-MacCatalogObservation-Focused-R1.xcresult` contains a strict build but `0` tests and remains invalid evidence. The corrected related matrix `/tmp/LuneX-MacCatalogObservation-Related-R2.xcresult` passed `108 total / 107 passed / 1 expected live-host skip / 0 failed`; the fresh full matrix `/tmp/LuneX-MacCatalogObservation-Full-R1.xcresult` passed `1383 total / 1381 passed / 2 expected opt-in skips / 0 failed`. Both valid runs have zero structured errors, warnings, and analyzer warnings.
- These offline gates establish source-contract and regression confidence only. They do not prove that macOS 27 no longer crashes at the exact product suspension boundary, nor do they prove Relative direction, subjective cursor smoothness, audio/video synchronization, reconnect, remote termination, or performance thresholds; the pushed single-instance product must supply that evidence.
- The first pushed form `5f24f57` proved that a complete test-target build is not sufficient product compile evidence under the current generated settings: the `LuneX-macOS` scheme enables a region-based isolation path that rejects the detached MainActor closure with an unsupported-pattern compiler diagnostic. Explicit captures were the first bounded hypothesis, but product compile R2 disproved it before any App launch.
- R2 showed that explicit captures do not fix this compiler path while the detached closure itself carries `@MainActor`. Because `AppModel` is a `@MainActor` type, a plain detached closure can safely capture the model reference and await its actor-isolated launch method; the method call performs the actor hop, while task detachment remains the mechanism that excludes the SwiftUI Observation task-local context.
- The final plain-detached form passed strict product compile R3 with zero structured diagnostics. Its broader related R3 matrix passed `139 total / 138 passed / 1 expected live-host skip / 0 failed`, covering all ProductWorkflowSurface and AppModelWorkflow tests; fresh full R2 passed `1383 total / 1381 passed / 2 expected opt-in skips / 0 failed`. Both test xcresults also have zero structured errors, warnings, and analyzer warnings.
- Live product evidence disproved the detached-task hypothesis: pushed `ad4fb4c` PID `19270` still crashed after entering the stream workspace, and the `080537` incident retains the exact Observation registrar stack at the first post-prepare ownership check. Computer Use then transparently relaunched replacement PID `19771`; it was not a concurrent second instance.
- `activeProductSessionOwner` is private session orchestration state, but many UI projections depend on it together with other session identifiers and workspace state. Marking one synthesized getter ignored would weaken observation semantics and could merely move the fault to the next observed getter. Deferring Task creation through the next main-dispatch turn is the narrower experiment: the SwiftUI gesture/keypress callback fully unwinds before a new MainActor Task is created, while all AppModel fields remain observable.
- The next-main-dispatch-turn candidate passed strict product compile R4 with zero structured diagnostics. Related R4 passed `139/138/1/0`, and fresh full R3 passed `1383/1381/2/0`; the only skips remain the explicit live-host and real-Keychain gates. This still does not prove the macOS 27 Observation incident is fixed until the pushed product survives the exact launch boundary.
- Pushed `78f0596` PID `21091` still crashed, but `081000.ips` now faults during SwiftUI outline-list constraint layout in `ScrollEnvironmentStorage.properties -> ObservationTracking._AccessList.addAccess`, with no AppModel launch frame. Process-first checking prevented Computer Use relaunch, so this evidence is unambiguous. The changing first accessor with the same corrupt Observation access-list mechanism points away from a specific AppModel getter and toward reentrant/concurrent UI activity during the catalog-to-stream transition.
- The display-link owner currently attaches and unpauses as soon as the stream surface is created, while `renderState.policy` is active but `decodedVideoPresentationContract` is still nil. That drives an empty Metal surface at the negotiated 144 Hz during SwiftUI teardown/layout. Deferring display-link attachment until the first decoded presentation contract, and invalidating it again when the contract clears, preserves callback-drawable/one-frame-latency behavior for actual frames while removing useless high-frequency callbacks from the transition boundary.
- The readiness implementation passes the strict product build. The first focused result did not exercise the product behavior because the new test fixture omitted `MacStreamInputCaptureView.sampleHandler`; this is a test-construction compile error, not evidence for or against the display-link hypothesis. The corrected fixture supplies a no-op handler and must be rerun into a fresh result bundle.
- Fresh focused R2 executed all 44 AppKit surface tests successfully with zero diagnostics. The new case proves the owner is unattached while no decoded contract exists, attaches and runs at the negotiated 144 Hz when readiness appears, and invalidates, pauses, and detaches when readiness clears.
- Fresh serial related coverage passed 345 total tests with only the explicit live-host opt-in skipped. AppModel lifecycle, current-generation media ownership, decoder recovery, HDR/EDR transitions, latest-frame delivery, presenter fences, and surface teardown all remained intact with zero build diagnostics.
- Fresh full macOS coverage passed 1,384 total tests, with 1,382 passes and only the two exact live Sunshine and real Keychain opt-in skips. This closes the deterministic regression gate for the readiness batch but does not establish the live crash hypothesis, physical Relative direction, subjective smoothness, audible synchronization, or performance thresholds.
- Pushed `d0e36ce` crossed the exact prior product crash boundary: the single original PID remained alive eight seconds after one Desktop double-click, no new crash report appeared, and real video continued. Before Diagnostics took focus, presentation was `3121 published / 3019 presented / 101 superseded` with 9.6 ms latest-frame age and 18.4 ms maximum publish-to-present delay, materially different from the old foreground replacement disparity.
- A ten-second Finder background interval and Computer Use return preserved the same stream workspace, PID, audio session, and updating video rather than returning to Library. The mixed foreground/background cumulative snapshot reached `12359 / 11376 / 982`, with 18.3 ms latest-frame age and 21.6 ms maximum presentation delay; background throttling means this mixed ratio is not a pure foreground frame-pacing measure.
- Computer Use coordinate drag and coordinate click each failed with its own `noWindowsAvailable` error while accessibility-element operations continued to work. Those failures provide no host input or Relative-direction evidence. The existing one-time AppKit Y conversion remains deterministic-only until the user or a reliable physical tool confirms host-visible direction.
- Since decoded-frame readiness resolves the crash without depending on the catalog launch workaround, the main-queue deferral and detached-task experiments should be retired. The original structured `Task` preserves SwiftUI cancellation/actor context and removes an unnecessary event-loop turn from launch admission.
- The restored structured catalog task passes the strict product compiler and the complete deterministic suite. Focused workflow coverage is 32/32, the launch/workspace/lifecycle/readiness related matrix is 200 total with only its explicit live-host skip, and the full suite is 1,384 total with only live Sunshine and real Keychain skipped; all structured build diagnostics are zero.
- Pushed `1e6dcdc` proves the catalog cleanup did not reintroduce the crash: one Desktop double-click on the single PID crossed the old boundary, retained that PID after eight seconds, produced no crash report, and showed real video. The foreground snapshot before Diagnostics focus was `2592 published / 2503 presented / 88 superseded`, with 2.7 ms maximum control queue delay and 12.3 ms maximum publish-to-present delay.
- The same live snapshot reveals a diagnostic ownership defect: visible video and thousands of published/presented frames coexist with `Video decode: 0 access units, 0 frames, 0 failures`. The decode row is therefore not current production evidence and must be wired to the actual generation-owned decode telemetry rather than inferred from presentation or left at a default snapshot.
- Follow-up source tracing corrects the preceding diagnosis: `NativeSessionVideoProcessor` already publishes the active generation-owned `VideoDecodePipelineSnapshot`. The UI mislabeled `droppedAccessUnitCount` as total access units and `decoderDroppedFrameCount` as total frames, so a healthy stream correctly produced zeroes under false labels. The fix adds independent submitted-access-unit and successful current-generation decoded-frame counters while retaining the drop/failure counters.
- The user's physical host result establishes that negating `NSEvent.deltaY` was wrong in this capture path; the earlier deterministic test encoded an unverified coordinate assumption. Preserve AppKit's observed delta on the Moonlight wire. The reported jump-with-complete-total-motion pattern is also consistent with AppKit's default mouse-event coalescing, so active capture now disables that upstream coalescing under replacement-safe global ownership while LuneX's bounded first serialized queue remains the explicit Moonlight-Qt-style coalescing point.
- Fresh focused R2 passed all 57 selected tests with warnings treated as errors. It exercises active-capture coalescing ownership and restoration, every moved/dragged Relative Y entry point, successful access-unit submission, current-generation VideoToolbox callback accounting, drop/failure separation, and AppModel snapshot aggregation; no live host, Keychain, Simulator, or product process was touched.
- The serial related matrix passed 405 of 406 tests with only the explicit live-host opt-in skipped and zero structured build diagnostics. Input delivery/backpressure/barriers, decoder generation recovery, session media ownership, Metal presentation, workflow aggregation, and bounded runtime Diagnostics all remain intact.

## 2026-08-31 pressed-drag window-exit crash

- The current `15:40:54` incident belongs to original PID `34362`, launched at `08:51:35`, and therefore matches the user's physical reproduction on pushed `61e4d47`. The pasted text belongs to obsolete PID `15431` from `07:51` and is not the current incident.
- The current main-thread stack is `ObservationTracking._AccessList.addAccess -> activeProductSessionOwner.getter -> productSessionOwnerOwnsActiveReservation -> consumeSessionControlEvent -> launchSelectedApp`. No `MacStreamInputCaptureView`, AppKit drag callback, coordinate mapper, ENet, Metal, or audio frame is on the faulting thread. Pointer exit is the reliable main-thread reentrancy trigger, while the defect is observing a private session reservation from long-lived asynchronous and synchronous runtime paths.
- `activeProductSessionOwner` has no direct View binding and is always paired with observable launch/session/workspace/render/input/audio state changes. Excluding that MainActor-confined token from Observation removes the invalid transient access dependency without weakening session-generation validation or hiding user-visible state.
- Focused R1 confirms the private reservation-token change compiles through the full test target and that an out-of-surface pressed drag remains a finite sample with its button snapshot. This is deterministic evidence only; it does not prove that the exact pushed product survives the user's physical drag-out gesture.
