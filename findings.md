...
- 2026-08-26 17:41 CST，经过direct XCTest环境继承验证后，catalog-only live gate对固定endpoint只发出一次`/serverinfo`，在5秒内timeout。该结果只证明当时`tanmy-white`不可达；未发送catalog、launch、resume、cancel、stop或input。host/settings/catalog/Debug identity四个文件的SHA和权限前后相同，不能将较早的缓存catalog或先前busy响应替代为本轮live通过。
